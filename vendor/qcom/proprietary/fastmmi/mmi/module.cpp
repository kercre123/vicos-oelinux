...
bool is_autoswitch_module(module_info * mod) {
    return is_bluetooth_module(mod) || is_gps_module(mod) || is_sim_module(mod) || is_storage_module(mod)
        || is_wifi_module(mod) || is_battery_module(mod);
}
...
void initial_screen(module_info * mod) {
    ...
    if(curlay != NULL) {
        ...
        if(tv != NULL) {
            ...
            ALOGI("Trans : %s to %s", mod->module, module_name);
...
static void send_cmd(module_info * mod, int cmd, const char *subcmd) {
    ...
    if(mod == NULL) {
        ALOGE("%s,Invalid command, mod is NULL", __FUNCTION__);
        return;
    }

    if(mod->socket_fd < 0) {
        ALOGE("%s,socket fd  is invalid", __FUNCTION__);
        return;
    }
    ...
    ALOGI("SEND COMMAND [%s]: cmd[%d], through sock:%d, to process id:%d, parameter:%s ", mod->module, cmd,
          mod->socket_fd, mod->pid, parameter.c_str());
...
void module_cleanup(module_info * mod) {
    ...
    ALOGI("%s[%s]:end,g_module_tid=%d", __FUNCTION__, mod->module, g_module_tid);
...
void update_main_status() {
    ...
    if(btn == NULL) {
        ALOGE("Main layout no status button\n");
        return;
    }
...
static bool wait_for_headset(module_info * mod) {
    ...
    while(!inserted) {
        ALOGI("waiting for headset insert ...");
...
static void *launch_module(void *module) {
    ...
    ALOGD("===launch module: %s\n", mod->module);
    if(!pre_check(mod)) {
        ALOGE("===launch module: %s, pre check failed!\n", mod->module);
        return NULL;
    }
...
void module_exec_pcba(module_info * mod) {
    ...
    time(&(mod->start_time));
    mod->running_state = MODULE_RUNNING;
    ALOGI("start to exec pcba: %s", mod->module);
    if(is_lcd_module(mod)) {
#ifdef ANDROID
        if(is_lcd_show_image(mod)) {
