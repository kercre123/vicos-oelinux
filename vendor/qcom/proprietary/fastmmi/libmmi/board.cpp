...
int get_subtype_id(char *buf, int size) {
    ...
    if(0 != get_product_name(buf, size)) {
        return -1;
    }
    return 0;
}
...
int get_product_name(char *product, int size) {
    ...
    int found = 0;
    FILE *fp;
    char str[] = "ro.product.name=";
    char *addr;

    memset(product, 0, size);
    // find out the information of ro.product.name
    if((fp = fopen("/build.prop", "r")) != NULL) {
        while(fgets(tmp, sizeof(tmp), fp) != NULL){
            if(addr = strstr(tmp, str)){
                memset(buf, 0, 128);
                strncpy(buf, addr+strlen(str), strlen(addr)-strlen(str));
                trim(buf);
                break;
            }
        }
