...
#else
  property_get_bt("wlan.driver.ath", ath6kl_supported, 0);
#endif
...
int property_get_bt(const char *key, char *value, const char *default_value)
{
    ...
    fprintf(stderr, "%s: property_get_bt: key(%s) has value: %s\n", __func__, key, value);
    if (bytes_read) {
        return 0;
    } else {
...
int property_set_bt(const char *key, const char *value)
{
    ...
    fprintf(stderr, "%s: property_set_bt: setting key(%s) to value: %s\n", __func__, key, value);
    ret = send(bt_prop_socket, prop_string, strlen(prop_string), 0);
    return 0;
}
#endif
