...
void set_debug_properties() {
    ...
    ALOGV("%s: property_get_bt: wc_transport.force_special_byte: %d", __func__, ret);
#endif
    if (ret) {
...
#else
    ret = property_get_bt("wc_transport.out_ringbuf_log", value, NULL);
    ALOGV("%s: property_get_bt: wc_transport.ringbuf: %d", __func__, ret);
#endif
    ...
    ALOGV("%s: property_get: wc_transport.ringbuf: %d", __func__, ret);
#else
    ret = property_get_bt("wc_transport.in_ringbuf_log", value, NULL);
    ALOGV("%s: property_get_bt: wc_transport.ringbuf: %d", __func__, ret);
#endif
...
#else
    ret = property_get_bt("persist.service.bdroid.fwsnoop", value, "false");
    ALOGV("%s: property_get_bt: persist.service.bdroid.fwsnoop: %d", __func__, ret);
#endif

    enabled = (strcmp(value, "true") == 0) ? TRUE : FALSE;
...
static int establish_remote_socket(char *name)
{
    ...
    ALOGE("establish_remote_socket: addr.sun_path = %s", addr.sun_path);
...
#endif
        ret = do_write(fd_transport, buf, LEN_PACKET_TO_SOC);
        if (ret < 0) {
...
int main() {
    ...
    openlog ("wcnss_filter", LOG_CONS | LOG_PID | LOG_NDELAY, LOG_LOCAL1);
#endif
...
bt_thread_fail:
    ...
    property_set_bt("wc_transport.start_hci", "false");
    if(bt_prop_socket > 0)
    {
        ALOGE("%s:calling close of bt_prop_socket fd = %d", __func__,bt_prop_socket);
...
#else
    property_get_bt("wc_transport.clean_up", cleanup, "0");
#endif
    clean = atoi(cleanup);
...
#else
         property_set_bt("wc_transport.ref_count", ref_count);
#endif
      }
    }
    else {
...
//        cleanup_and_suicide();
    ...
    if(bt_prop_socket > 0)
    {
        ALOGE("%s:calling shutdown of fd = %d", __func__,bt_prop_socket);
        shutdown(bt_prop_socket, SHUT_RDWR);
        close(bt_prop_socket);
        bt_prop_socket = -1;
    }
    ...
    ALOGE("%s:Done with this Life!!!",__func__);
    exit(0);
}
