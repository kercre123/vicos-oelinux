...
extern "C"
{
    int32_t test1_init( const struct vaapi_source_info *sourceInfo, void **handle);
    ...
    int32_t test1_enroll_obj(void *handle, struct vaapi_enrollment_info *enroll_info);
    int32_t test1_disenroll_obj(void *handle, const char *obj_id);

    int32_t test1_get_event_count(void *handle, uint32_t *event_count);
    int32_t test1_get_events(void *handle, struct vaapi_event *events, uint32_t num_events);
    int32_t test1_get_object_count(void *handle, uint32_t *object_count);
    int32_t test1_get_objects(void *handle, vaapi_object *objects, uint32_t obj_count);
    int32_t test1_get_target_FPS(void *handle, uint8_t *target_fps);

    int32_t test1_is_event_supported(void *handle, vaapi_event_type type, uint8_t *is_supported);
    int32_t test1_is_depends_on_other_event(void *handle, vaapi_event_type type, uint8_t *is_depended);
}
