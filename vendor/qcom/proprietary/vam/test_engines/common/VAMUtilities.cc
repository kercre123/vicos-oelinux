...
uint64_t getTimeNow()
{
    ...
    return 1000000LL * (long long)res.tv_sec + res.tv_nsec/1000;
}
...
int getCalibratedParams(const vaapi_scene_calibration *caliInfo, int64_t *params)
{
    ...
    float h_mat[9] = { sx2, sxy, sx, sxy, sy2, sy, sx, sy, (float)caliInfo->object_size };
    float h_inv_mat[9];
    float p[3] = { sxz, syz, sz };
    OCMatrixInverse3x3_1((float *)h_mat, (float *)h_inv_mat);
    float a = 0;
    float b = 0;
    float c = 0;
    float * abc = (float*)params;
    for (int i = 0; i < 3; i++)
    {
        a += h_inv_mat[i] * p[i];
        b += h_inv_mat[3 + i] * p[i];
