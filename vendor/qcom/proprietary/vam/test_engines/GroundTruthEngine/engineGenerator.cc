...
extern "C"
{
    int32_t GTRenderer_init( const struct vaapi_source_info *sourceInfo, void **handle);
    ...
    int32_t GTRenderer_enroll_obj(void *handle, struct vaapi_enrollment_info *enroll_info);
    ...
    int32_t GTRenderer_get_events(void *handle, struct vaapi_event *events, uint32_t num_events);
    int32_t GTRenderer_get_object_count(void *handle, uint32_t *object_count);
    int32_t GTRenderer_get_objects(void *handle, vaapi_object *objects, uint32_t obj_count);
    int32_t GTRenderer_get_target_FPS(void *handle, uint8_t *target_fps);

    int32_t GTRenderer_is_event_supported(void *handle, vaapi_event_type type, uint8_t *is_supported);
    int32_t GTRenderer_is_depends_on_other_event(void *handle, vaapi_event_type type, uint8_t *is_depended);
}
...
void *jsonHandle = NULL;
struct metadata_content_t *json_metadata = NULL;
int32_t GTRenderer_init(const struct vaapi_source_info *pSourceInfo, void **handle)
{
    if(pSourceInfo == NULL)
        return VAM_NULLPTR;
    ...
    MetadataParserStatus ret = metadata_parser_init(GTFilePrefix.c_str(), VAM_FILE, 0, 0, &jsonHandle);
    if(ret != MP_OK)
...
int32_t GTRenderer_process(void *handle, struct vaapi_frame_info *pFrameInfo)
{
    if(pFrameInfo == NULL || pFrameInfo->frame_l_data[0] == NULL)
    {
    ...
    MetadataParserStatus ret = metadata_parser_get_next_frame(jsonHandle, &metadata_str);
    if(metadata_str == NULL)
	VAM_ERROR("GTRenderer_init error(%d): metadata_parser_get_next_frame failed\n", (int)ret);
    else
    {
        ret = metadata_parser_parse_metadata_frame(jsonHandle, metadata_str, &json_metadata);
    }
