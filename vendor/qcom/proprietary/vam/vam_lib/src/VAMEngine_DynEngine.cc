...
VAMEngine_DynEngine::VAMEngine_DynEngine(const std::string libName, const struct vaapi_source_info &sourceInfo):
    ...
    _pEngineHandle(nullptr)
{
    ...
    if (_pEngineLibHandle == nullptr) {
        VAM_LOGE("VAMEngine_DynEngine::VAMEngine_DynEngine: load (%s) failed error = %s\n", libName.c_str(), dlerror());
//        throw VAM_ENGINE_DYLIB_NOT_EXIST;
    ...
    // link symbol
    _pEngineModule = reinterpret_cast<vaapi_alg_module_t *>(dlsym(_pEngineLibHandle, "VA_ALG_INFO_SYM"));
    if (_pEngineModule == nullptr) {
        VAM_LOGE("VAMEngine_DynEngine::VAMEngine_DynEngine: Can't find symbol in (%s)\n", libName.c_str());
        dlclose(_pEngineLibHandle);
//        throw VAM_ENGINE_SYMBOL_ERROR;
    }

    _pEngineModule->methods->init(&sourceInfo, &_pEngineHandle);

    uint8_t targetFPS = 0;
    _pEngineModule->methods->get_target_FPS(_pEngineHandle, &targetFPS);
    _targetFPS = static_cast<int>(targetFPS);
    if (_targetFPS > 61) {
        VAM_LOGE("VAMEngine_DynEngine error: found invalid target FPS %d"
    ...
    for (int eventType = 0; eventType < vaapi_event_size; eventType++) {
        uint8_t isDepend = 0;
        _pEngineModule->methods->is_depends_on_other_event(_pEngineHandle, (vaapi_event_type)eventType, &isDepend);
    ...
    if (_isDependsOnOtherEvent == false) {
        ...
        VAM_LOGD("is depends on other event: %s", (_isDependsOnOtherEvent) ? "yes" : "no");
        for (int eventType = 0; eventType < vaapi_event_size; eventType++) {
            uint8_t isDepend = 0;
...
bool VAMEngine_DynEngine::isEventSupported(vaapi_event_type type)
{
    ...
    uint8_t isSupported = 0;
    int32_t ret = _pEngineModule->methods->is_event_supported(_pEngineHandle, type, &isSupported);
...
int VAMEngine_DynEngine::_workerFunc(struct vaapi_frame_info *p)
{
    ...
    uint32_t objCount = 0;
    int ret = _pEngineModule->methods->get_object_count(_pEngineHandle, &objCount);
    if (ret == VAM_OK && objCount > 0) {
        ...
        int error = _pEngineModule->methods->get_objects(_pEngineHandle, pObjects, objCount);

        if (error) {
            VAM_LOGE("VAMEngine_DynEngine error(%d): get_meta failed\n", objCount);
        }
    ...
    if (ret == VAM_OK && eventCount  > 0) {
        ...
        int32_t error = _pEngineModule->methods->get_events(_pEngineHandle, pEvents, eventCount);

        if (error) {
            VAM_LOGE("VAMEngine_DynEngine error(%d): get_event failed\n", error);
        }
