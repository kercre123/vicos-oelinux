...
{
    if (newPTS - _lastProcessedPTS < 1000000 / _targetFPS) {
        return false;
    }
    ...
    while (_this->_isStop == false) {
        ...
        memset(&frameInfo, 0, sizeof(vaapi_frame_info));
        {
            ...
            if (_this->_isStop == false && _this->_frameDataList.size() == 0) {
                // timeout: 20 sec
                std::cv_status ret = _this->_putFrameCond.wait_for(m, std::chrono::milliseconds(20000));
        ...
        if (_this->_isTimeToPutFrame(frameInfo.time_stamp)) {
            ...
            int processTimeInMS = static_cast<int>(getTimeNow() - startTime) / 1000;
...
int VAMEngineBase::_processCallback(vaapi_frame_info *pFrameInfo, bool isUpdated)
{
    ...
    _pMetadataCBFunc(this, pFrameInfo->time_stamp, isUpdated, _pVAManager, _objList);
