    ...
    debugLevel(0)
{
    ...
    _VASimConfigMap["<Dynamic VA library folder>"] = configType_dynVAEngineFolder;
    _VASimConfigMap["<Dynamic VA library data folder>"] = configType_dynVAEngineDataFolder;
    ...
    _VASimConfigMap["<VAM Ext Rule Config File>"] = configType_VAMExtRuleConfigFile;
...
int VASimConfig::parseArgv(int argc, char **argv)
{
    for(int i=0; i<argc; i++)
    {
        ...
        if(arg == "-i" || (arg == "-I" && argv[i+1] != NULL))
        {
            inputFileNameList.clear();
            inputFileNameList.push_back(argv[i+1]);
        }

        if(arg == "-mo" || (arg == "-MO" && argc > (i+1) && argv[i+1] != NULL))
            MetadataOutputFolder = argv[i+1];

        if(arg == "-eo" || (arg == "-EO" && argc > (i+1) && argv[i+1] != NULL))
            EventOutputFolder = argv[i+1];

        if(arg == "-r" || (arg == "-R" && argc < (i+1) && argv[i+1] != NULL))
            VAMRuleConfigFileName = argv[i+1];

        if(arg == "-rext" || (arg == "-REXT" && argc < (i+1) && argv[i+1] != NULL))
            VAMExtRuleConfigFileName = argv[i+1];

        if(arg == "-dl" || (arg == "-DL" && argc > (i+1) && argv[i+1] != NULL))
            DynVAEngineFolder = argv[i+1];

        if(arg == "-dd" || (arg == "-DD" && argc > (i+1) && argv[i+1] != NULL))
            DynVAEngineDataFolder = argv[i+1];

        if(arg == "--FREnrollmentFileName" && argc > (i+1) && argv[i+1] != NULL)
            FREnrollmentFileName = argv[i+1];

        if(arg == "--FREnrollmentFolder" && argc > (i+1) && argv[i+1] != NULL)
        ...
        if(arg == "--filmStripNum" && argc > (i+1) && argv[i+1] != NULL)
        {
            std::stringstream ss;
            ss << argv[i+1];
            ss >> filmStripNum;
        ...
        //printf("VASimConfig::parseArgv: \"%s\"\n", arg.c_str());
    }
...
{
    std::cout << "===VASimConfig=============================begin" << std::endl;
    ...
    std::cout << "  VAMRuleConfigFileName:" << VAMRuleConfigFileName << std::endl;
    std::cout << "  VAMExtRuleConfigFileName:" << VAMExtRuleConfigFileName << std::endl;
    ...
    std::cout << "  DynVAEngineDataFolder:" << DynVAEngineDataFolder << std::endl;
    std::cout << "  inputFileNameList size:" << inputFileNameList.size() << std::endl;
    for(size_t idx=0; idx<inputFileNameList.size(); idx++)
        std::cout << "    file " << idx << ": " << inputFileNameList[idx] << std::endl;
...
int VASimConfig::_validateConfig()
{
    ...
    if(inputFileNameList.size() != videosPerRow * videoRowsPerColumn)
        printf("VASimConfig Warning: input file size(%d) is not equal to video view %d x %d",
                (int)inputFileNameList.size(), videosPerRow, videoRowsPerColumn);
