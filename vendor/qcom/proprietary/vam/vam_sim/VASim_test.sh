...
doVASim()
{
    ...
    printThis "  output metadata : ${outputFolder}/metadata/m*"
    ...
    if [[ ${Enable_DryRun_Only} = "enable" ]] ; then
        printThis " ${VASim_Dir}/bin/VASim -c ${VASim_Config} -i ${targetVideoFile} -r ${targetJSONRuleConfig} -mo ${outputFolder}/metadata/m -eo ${outputFolder}/event/ -dl ${VASim_Dir}/engine_libs -dd ${VASim_Dir}/engine_data --display=\"${Enable_Renderer}\" --snapshot=\"${Enable_Snapshot}\" --enableFR=\"${Enable_FR}\" --filmStripNum ${FilmStripNum} --FREnrollmentFileName \"${VASim_FR_Enrollment_File}\" --FREnrollmentFolder \"${VASim_FR_Enrollment_Folder}\" \
	 ...
	 -mo ${outputFolder}/metadata/m\
	 -eo ${outputFolder}/event/\
	 -dl /usr/lib64/VAMEngines/\
...
doVAMReport()
{
    ...
    if [[ -n ${grounTruthFilePrefix} ]] ; then
        ...
        if [[ ${Enable_DryRun_Only} = "enable" ]] ; then
            printThis "${VASim_Dir}/bin/VAMReport ${grounTruthFilePrefix} ${outputFolder}/metadata/m 0 0 1 ${outputFolder}/report/report 200 0 2>&1 | tee ${outputFolder}/log/VASimReport.log"
        else
            echo "[${printf_prefix}] [`date +"%Y/%m/%d %H:%M:%S"`] VAMReport begin." >> ${outputFolder}/log/VAMReport.log
            /usr/bin/VAMReport ${grounTruthFilePrefix} ${outputFolder}/metadata/m 0 0 1 ${outputFolder}/report/report 200 0 2>&1 | tee -a ${outputFolder}/log/VAMReport.log
