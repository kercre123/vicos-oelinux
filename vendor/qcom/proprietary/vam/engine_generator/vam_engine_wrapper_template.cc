...
extern "C"
{
    int32_t va_engine_init( const struct vaapi_source_info *sourceInfo, void **handle);
    ...
    int32_t va_engine_enroll_obj(void *handle, struct vaapi_enrollment_info *enroll_info);
    ...
    int32_t va_engine_get_events(void *handle, struct vaapi_event *events, uint32_t num_events);
    int32_t va_engine_get_object_count(void *handle, uint32_t *object_count);
    int32_t va_engine_get_objects(void *handle, vaapi_object *objects, uint32_t obj_count);
    int32_t va_engine_get_target_FPS(void *handle, uint8_t *desired_fps);

    int32_t va_engine_is_event_supported(void *handle, vaapi_event_type type, uint8_t *is_supported);
    int32_t va_engine_is_depends_on_other_event(void *handle, vaapi_event_type type, uint8_t *is_depended);
}
...
int32_t va_engine_init(const struct vaapi_source_info *pSourceInfo, void **handle)
{
...
int32_t va_engine_get_objects(void *handle, vaapi_object *objects, uint32_t obj_count)
{
    ...
    objects[0].pos.x = (int) (50.0 * cos(t)) + 300;
    objects[0].pos.y = (int) (50.0 * sin(t)) + 300;
    ...
    objects[1].pos.x = (int) (50.0 * cos(-t)) + 400;
    objects[1].pos.y = (int) (50.0 * sin(-t)) + 400;
