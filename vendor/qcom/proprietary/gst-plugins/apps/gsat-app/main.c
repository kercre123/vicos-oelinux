...
typedef struct _CustomData {
  ...
  guint count, cmd_count,total_cmd_count, concurrency_flag;
...
int parseconfig(gchar *fp, CustomData *data)
{
    ...
    if (!g_file_test (fp, G_FILE_TEST_EXISTS)) {
        g_print ("File not found to open\n");
        return 0;
    }
...
static gboolean onfly_effect (CustomData *data) {
    ...
    /*Move the SVA pipe to PAUSE again to be ready to receive new KWD + Command */
      ...
      g_print ("PVR: onfly_effect- setting to 1 \n");
      }
      else
      {
          g_object_set (data->pb_enhancer, "strength", 999, NULL);
          g_print ("PVR: onfly_effect- setting to 999 \n");
      }
      i = !i;
      return 1;
}
...
static gint create_playback_pipeline(CustomData *data, gint session_id){
    ...
    gchar *pipe_str;
      ...
      if(g_strcmp0(data->session_info[session_id]->queue,"1")==0){
          ...
          pipe_str = g_strdup_printf ("filesrc location=%s ! qtdemux name=demux demux.audio_0 ! queue ! aacparse ! qahwsink", data->session_info[session_id]->FileLoc);
      ...
      char *argv[])
{
 ...
 GstElement *sva_source, *sva_filter, *sva_sink;
  GMainLoop *main_loop;
  GstStateChangeReturn ret;
  gint i, r, session_count=0, iteration_count,dur=10;
  ...
  for(j=iterations;j>0;j--){
      ...
      for (session_count =0; session_count <= data.count; session_count ++){
       ...
       if(data.concurrency_flag == 0){
           g_timeout_add_seconds_full (G_PRIORITY_HIGH, dur, (GSourceFunc)action_section, &data, NULL);
       }
