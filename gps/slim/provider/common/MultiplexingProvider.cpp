...
@retval void
*/
void MultiplexingProvider::routeIndicationInBatch
(
  slimServiceEnumT eService,
...
)
{
  ...
  if ((tDispatchTimestamp > 0ull) && (tCurrentTimestamp >= tDispatchTimestamp))
  {
     SLIM_LOGD("routeIndication eService = %d tDispatchTimestamp=%" PRIu64 " ms. \
        tCurrentTimestamp = %" PRIu64 " ms. sensorData.samples_len = %d. \n", eService, \
        tDispatchTimestamp, tCurrentTimestamp, sensorData.samples_len);
     routeIndication(SlimSensorTypeToService(sensorData.sensorType), sensorData);
     sensorData.samples_len = 0;
     tDispatchTimestamp = 0ull;
  }
  else
#endif
  if (sensorData.samples_len == wBatchCount ||
  ...
  if (tCurrentTimestamp > tDispatchTimestamp  ||
      sensorData.samples_len >= SLIM_SENSOR_MAX_SAMPLE_SETS)
  {
     SLIM_LOGD("routeIndication eService = %d tDispatchTimestamp=%" PRIu64 " ms. \
        tCurrentTimestamp = %" PRIu64 " ms. sensorData.samples_len = %d. \n", eService, \
