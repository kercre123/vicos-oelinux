   ...
   for (i = 0; i < MAX_NUM_UNIQUE_CRITERIA; i++)
   {
      ...
      if (criteria->isUsed == true)
      {
        LOC_LOGI ("%s index at %d, isNew = %d, mask 0x%x, provider source = %d,"
         ...
         ulp_data.quipc_provider_info.new_request_active |= criteria->isNew;
...
int ulp_brain_select_providers ()
{
   ...
   else
   {
      ...
      if ((ulp_data.low_accuracy_request_active == true) &&
          (ulp_data.gnp_provider_info.enabled == false) && (!ulp_data.emergency_request))
      {
         ulp_data.gnss_provider_info.selected_to_run = true;
...
SIDE EFFECTS
   N/A
===========================================================================*/
int ulp_brain_process_gnss_sv_report  (const LocGnssSvStatus* svStatusPtr)
{
   int              sv_index;
   const LocGnssSvInfo* svInfoPtr = NULL;

   gnss_sv_report_cnt++;

   ulp_data.gnss_provider_info.strong_sv_cnt = 0;

   svInfoPtr = &(svStatusPtr->gnss_sv_list[0]);
   for (sv_index = 0; sv_index < svStatusPtr->num_svs; sv_index++)
   {
      if ((LOC_GNSS_CONSTELLATION_GPS == svInfoPtr->constellation ||
          LOC_GNSS_CONSTELLATION_GLONASS == svInfoPtr->constellation) &&
          (svInfoPtr->c_n0_dbhz >= (float) ULP_STRONG_SV_SNR_TH))
      {
                                            ...
                                            const void* gps_location_ext)
{
   ...
   if (report_hybrid_position == true || ulp_data.gps_provider_request_active == true)
   {
      ...
      ulp_msg_send_generic_position_report(status, tech_mask, gps_location_ptr,
                                           gps_location_extended_ptr, gps_location_ext);
   }
                                            ...
                                            const GpsLocationExtended* dr_location_extended_ptr)
{
   ...
   if(ulp_data.dr_provider_request_active == true )
   {
      ...
      ulp_msg_send_generic_position_report (status, tech_mask, &location_copy, &location_extended_copy, NULL);
   }
      ...
      if (ulp_data.zpp_request_active == true)
      {
         ...
         if (0 != location_ptr->position_source)
         {
            ...
            ulp_msg_send_generic_position_report(status, tech_mask,
                                                 location_ptr, &location_extended_dummy, NULL);
