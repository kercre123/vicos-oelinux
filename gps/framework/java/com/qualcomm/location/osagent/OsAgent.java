...
public class OsAgent
{
    ...
    private void broadcastCountryIntent(String action, boolean status)
    {
        String COUNTY_SELECT_STATUS = "COUNTY_SELECT_STATUS";
        Intent intent = new Intent();
        intent.setAction(action);
        intent.putExtra(COUNTY_SELECT_STATUS, status);
        ...
        if (mEnableAutoEula) {
            ...
            if(mDeviceInRegulatedArea != isDevInRegulatedArea) {
                ...
                if(0 != (mDeviceInRegulatedArea & DEVICE_IN_REGULATED_REGION_KNOWN)) {
                    boolean regulated;
                    regulated = (0 != (mDeviceInRegulatedArea & DEVICE_IN_REGULATED_REGION_AT_EMBARGOED));
                    XTProxy.setFeatureAllowed(mContext, !regulated);

                    regulated = (0 != (mDeviceInRegulatedArea & DEVICE_IN_REGULATED_REGION_AT_REGIONAL_NLP));
                    broadcastCountryIntent(COUNTRY_SELECT_ACTION, regulated);
                }
            }
        }
    }
