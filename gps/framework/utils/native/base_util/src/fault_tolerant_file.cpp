...
namespace qc_loc_fw
{
#define WAKE_LOCK_ID_LEN 128

static const char *const TAG = "FTFile";

static unsigned long long getWakeLockId()
{
  pthread_t thread_id = pthread_self();
  return ((unsigned long long)thread_id);
}

void wakeLockAcquire()
{
  unsigned long long wakelock_id = getWakeLockId();
  char shell_command[WAKE_LOCK_ID_LEN + 50] = {0};
  char wakelock_id_str[WAKE_LOCK_ID_LEN] = { 0 };
  snprintf(wakelock_id_str, WAKE_LOCK_ID_LEN, "%llu", wakelock_id);
  strlcat(shell_command, "echo qc_loc_fw_lock", sizeof(shell_command));
  strlcat(shell_command, wakelock_id_str, sizeof(shell_command));
  strlcat(shell_command, " > /sys/power/wake_lock", sizeof(shell_command));
  log_debug(TAG, "%s: executing shell command: %s", __FUNCTION__, shell_command);
  if (system(shell_command))
  {
    log_error (TAG, "%s: executing shell command: %s failed", __FUNCTION__, shell_command);
  }
}
...
static void wakeLockRelease()
{
  ...
  strlcat(shell_command, "echo qc_loc_fw_lock", sizeof(shell_command));
  strlcat(shell_command, wakelock_id_str, sizeof(shell_command));
  strlcat(shell_command, " > /sys/power/wake_unlock", sizeof(shell_command));
  log_debug(TAG, "%s: executing shell command: %s", __FUNCTION__, shell_command);
  if (system(shell_command))
  {
    log_error (TAG, "%s: executing shell command: %s failed", __FUNCTION__, shell_command);
