...
static map<IzatStreamType, const char *> providerNameLookUpTable = {
...
static void onJavaClassLoad (JNIEnv* env, jclass clazz) {
    ...
    (
        clazz,
        "onLocationChanged",
        "(Ljava/lang/String;JJDDZDZFZFZFS)V"
    );
...
)
{
    ...
    if(location->mNetworkPositionSource == IZAT_NETWORK_POSITION_FROM_WIFI)
         positionSource = 1;
    else if(location->mNetworkPositionSource == IZAT_NETWORK_POSITION_FROM_CELL)
         positionSource = 0;
