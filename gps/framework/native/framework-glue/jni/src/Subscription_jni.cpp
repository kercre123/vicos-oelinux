...
static void instanceDeinit(JNIEnv *env, jobject obj)
{
    ...
    // delete the framework action request object
    FrameworkActionRequest::destroyInstance();
}
                ...
                jlong scan_start_timestamp, jlong scan_recv_timestamp, jint error_code)
{
    ...
    do {
        BtLeDeviceScanDetailsDataItem btle_device_data_item;

        btle_device_data_item.mValidSrnData = is_valid;
        btle_device_data_item.mErrorCause = error_code;
        if (is_valid) {
            jbyte * mac_arr = 0;
            mac_arr = env->GetByteArrayElements(btle_mac_array, 0);
            if (mac_arr != NULL) {
                for(int i=0;i<SRN_MAC_ADDRESS_LENGTH;i++) {
                    btle_device_data_item.mApSrnMacAddress[i] = mac_arr[i];
                }
            }
            btle_device_data_item.mApSrnRssi = rssi;
            btle_device_data_item.mApSrnTimestamp = btle_scan_req_timestamp;
            btle_device_data_item.mRequestTimestamp = scan_start_timestamp;
            btle_device_data_item.mReceiveTimestamp = scan_recv_timestamp;
            ...
            jlong scan_start_timestamp, jlong scan_recv_timestamp, jint error_code)
{
    ...
    do {
        BtDeviceScanDetailsDataItem btclassic_device_data_item;

        btclassic_device_data_item.mValidSrnData = is_valid;
        btclassic_device_data_item.mErrorCause = error_code;
        if (is_valid) {
            jbyte * mac_arr = 0;
            ...
            if (mac_arr != NULL) {
                for(int i=0;i<SRN_MAC_ADDRESS_LENGTH;i++) {
                    btclassic_device_data_item.mApSrnMacAddress[i] = mac_arr[i];
                }
            }
            btclassic_device_data_item.mApSrnRssi = rssi;
            btclassic_device_data_item.mApSrnTimestamp = bt_scan_req_timestamp;
            btclassic_device_data_item.mRequestTimestamp = scan_start_timestamp;
            btclassic_device_data_item.mReceiveTimestamp = scan_recv_timestamp;
