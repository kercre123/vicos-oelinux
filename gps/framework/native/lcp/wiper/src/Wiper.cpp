        ...
        do {
            ...
            if((dataItemIdList.size() != 0) && (timerToRun != NULL)) {
                ...
                if (gnssSrnDataReq.srnRequest)
                {
                    ...
                    mInstance->mIzatContext->mOSObserverObj->subscribe(dataItemIdList, mInstance);
                    mInstance->mIzatContext->mOSObserverObj->turnOn(dataItemId, 0);
                ...
                else
                {
                    ...
                    mInstance->mIzatContext->mOSObserverObj->turnOff(dataItemId);
                    mInstance->mIzatContext->mOSObserverObj->unsubscribe(dataItemIdList, mInstance);
                    timerToRun->stop();
                }
                result = 0;
            }
...
void Wiper::injectBtClassicScanData::proc() const
{
    ...
    if (btDevScanMapSize != 0) {
        ...
        for (it=mWiperObj->mBtClassicDevicesMap.begin(); it!= mWiperObj->mBtClassicDevicesMap.end(); ++it) {
            BtAccessPointInfo btApInfo = it->second;
                ...
                switch (dataItem->getId())
                {
                    ...
                    case BTLE_SCAN_DATA_ITEM_ID:
                    {
                        BtLeDeviceScanDetailsDataItem *btleDev =
                                    reinterpret_cast<BtLeDeviceScanDetailsDataItem*>(dataItem);
                        ...
                        if (btleDev->mValidSrnData) {
                            ...
                            LOC_LOGD ("LocTech-Value :: BTLE device scan"
                                      " request time: %lld, start time: %lld, recv time: %lld",
                            ...
                            dataItemIdList.push_back(BTLE_SCAN_DATA_ITEM_ID);
                            // turnoff and unsubscribe when the stopScan request comes in.
                            mInstance->mIzatContext->mOSObserverObj->turnOff(BTLE_SCAN_DATA_ITEM_ID);
                            mWiperObj->mIzatContext->mOSObserverObj->unsubscribe(
                                                                        dataItemIdList, mWiperObj);
                    ...
                    case BT_SCAN_DATA_ITEM_ID:
                    {
                        BtDeviceScanDetailsDataItem *btClassicDev =
                                        reinterpret_cast<BtDeviceScanDetailsDataItem*>(dataItem);
                        ...
                        if (btClassicDev->mValidSrnData) {
                            ...
                            dataItemIdList.push_back(BT_SCAN_DATA_ITEM_ID);
                            // turnoff and unsubscribe when the stopScan request comes in.
                            mInstance->mIzatContext->mOSObserverObj->turnOff(BT_SCAN_DATA_ITEM_ID);
                            mWiperObj->mIzatContext->mOSObserverObj->unsubscribe(dataItemIdList, mWiperObj);
