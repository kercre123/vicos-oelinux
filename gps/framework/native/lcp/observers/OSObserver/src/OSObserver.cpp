...
inline OSObserver :: HandleTurnOn :: HandleTurnOn
 (OSObserver * parent, const DataItemId dit,const int timeOut)
:
HandleMsgBase (parent), mDataItemId (dit), mTimeOut (timeOut)
{}

inline OSObserver :: HandleTurnOff :: HandleTurnOff
 (OSObserver * parent, const DataItemId dit)
:
...
void OSObserver :: HandleTurnOn :: proc () const {
    int result = 0;
    ENTRY_LOG ();
    do {
        // Send action turn on to framework
        this->mParent->mIzatContext->mFrameworkActionReqObj->turnOn(mDataItemId, mTimeOut);
    } while (0);
    EXIT_LOG_WITH_ERROR ("%d", result);
}

void OSObserver :: HandleTurnOff :: proc () const {
    int result = 0;
    ENTRY_LOG ();
    do {
        // Send action turn off to framework
        this->mParent->mIzatContext->mFrameworkActionReqObj->turnOff(mDataItemId);
    } while (0);
    EXIT_LOG_WITH_ERROR ("%d", result);
}
...
void OSObserver :: turnOn (DataItemId dit, int timeOut)
{
    int result = 0;
    ENTRY_LOG ();
    do {
        ...
        if (citer == mActiveRequestCount.end ()) {
            ...
            // Send action turn on to framework
            mIzatContext->mMsgTask->sendMsg (new  (nothrow) HandleTurnOn (this, dit, timeOut));
        } else {
...
void OSObserver :: turnOff (DataItemId dit)
{
    int result = 0;
    ENTRY_LOG ();
    do {
        ...
        if (citer != mActiveRequestCount.end ()) {
            ...
            if(citer->second == 0) {
                ...
                // Send action turn off to framework
                mIzatContext->mMsgTask->sendMsg (new  (nothrow) HandleTurnOff (this, dit));
            }
        } else {
            // Not found in map
