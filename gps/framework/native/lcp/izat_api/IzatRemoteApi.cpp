...
void SvInfoUpdaterBase::handleMsg(InPostcard * const in_msg) {
    ...
    if ((in_msg != nullptr) &&
        ...
        GpsLocationExtended locExtended;
        memset(&gnssSvStatus, 0, sizeof(gnssSvStatus));
        memset(&locExtended, 0, sizeof(locExtended));
        const void* blob = NULL;
        size_t length = 0;
        in_msg->getBlob ("GNSSSVSTATUS", &blob, &length);
        gnssSvStatus = *(LocGnssSvStatus *)blob;
        in_msg->getBlob ("LOCEXTENDED", &blob, &length);
        locExtended = *(GpsLocationExtended *)blob;

        if (mClientInfo) {
            //Send SVInfo to remote clients
            mClientInfo->svReportCb(&gnssSvStatus, &locExtended, (void*)mClientData);
            //generate and send NMEA
            if(mClientInfo->nmeaCb){
                std::vector<std::string> nmeaArraystr;
                loc_nmea_generate_sv(gnssSvStatus, locExtended, nmeaArraystr);
                struct timeval tv;
                gettimeofday(&tv,  NULL);
                int64_t now = tv.tv_sec * 1000LL + tv.tv_usec / 1000;
                UlpNmea nmeaStr;
...
void WiFiDBUpdater::pushWiFiDB(vector<APLocationData>* l_data_ptr,
    ...
    if (mNotifierProxy) {
        ...
        if (nullptr != card) {
            ...
            for (vector<APLocationData>::iterator it = l_data_ptr->begin(); it != l_data_ptr->end(); ++it) {
                ...
                if (0 != (ap_loc->valid_mask & APLocationData::WIFIDBUPDATER_APLOC_HORIZONTAL_ERR_VALID)) {
                    locCard->addFloat("HORIZONTAL_ERROR", ap_loc->horizontal_error);
                }
                if (0 != (ap_loc->valid_mask & APLocationData::WIFIDBUPDATER_APLOC_RELIABILITY_VALID)) {
            ...
            for (vector<APSpecialInfo>::iterator it = s_data_ptr->begin(); it != s_data_ptr->end(); ++it) {
...
void WiFiDBUpdater::handleMsg(InPostcard * const in_msg) {
    ...
    if (nullptr != in_msg) {
        if (0 == in_msg->getString(sInfoTag, &info)) {
            if (0 == strncmp(WiFiDBUpdater::sAPListTag, info, sizeof(WiFiDBUpdater::sAPListTag) - 1) &&
                ...
                for (uint32_t ii = 0; ii < ap_count; ++ii) {
                    ...
                    if (0 == in_msg->getCard(WiFiDBUpdater::sAPListTag, &ap_info_card, ii) &&
                        ...
                        if (ap_info.ssid_valid_byte_count > AP_SSID_PREFIX_LENGTH) {
                            ap_info.ssid_valid_byte_count = AP_SSID_PREFIX_LENGTH;
                        }
