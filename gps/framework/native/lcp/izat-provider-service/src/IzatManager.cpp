...
int32 IzatManager::addOdcpiRequest(const IOdcpiRequest *request)
{
    ...
    if (request) {
        ...
        LOC_LOGI
        (
            "LocTech-Value :: Interval: %lld, Emergency %d",
             request->getInterval (),
...
void IzatManager::handleMsg(InPostcard * const in_card)
{
    ...
    do {
        ...
        if (strcmp(req, "PASSIVE-LOCATION") == 0) {
            ...
            if (0x0 != listensto) {
                mIzatContext.mMsgTask->sendMsg(
                    new(nothrow) startPassiveSvInfoListenerMsg(
                        this, from, listensto));
            }
        }
        result = 0;
...
void IzatManager::printUlpLocation (UlpLocation * ulpLocation, GpsLocationExtended* locationExtended, enum loc_sess_status status) {
    if (ulpLocation) {
        LOC_LOGD
        (
...
void IzatManager::location_callback (UlpLocation* location, GpsLocationExtended* locationExtended, enum loc_sess_status status) {
    ENTRY_LOG();
    int result = -1;
    do {
        ...
        mIzatManager->mIzatContext.mMsgTask->sendMsg(
            new (nothrow)locationCallbackMsg(mIzatManager, location, locationExtended, (IzatLocationStatus)status));
        result = 0;
    } while(0);
    EXIT_LOG_WITH_ERROR("%d", result);
}
...
void IzatManager::svinfo_callback (const LocGnssSvStatus *gnssSvStatus,
    ...
    do {
        ...
        BREAK_IF_ZERO(1, mIzatManager);
        BREAK_IF_ZERO(2, gnssSvStatus);
        BREAK_IF_ZERO(3, locationExtended);

        mIzatManager->mIzatContext.mMsgTask->sendMsg(
            new (nothrow)svInfoCallbackMsg(mIzatManager, gnssSvStatus, locationExtended, source));
        result = 0;
    } while(0);
    EXIT_LOG_WITH_ERROR("%d", result);
}
...
IzatLocation IzatManager::izatLocationFromUlpLocation(const UlpLocation *ulpLoc, const GpsLocationExtended* locExtended) {
    ...
    if (NULL != ulpLoc) {
        ...
        res.mPosition_source = ulpLoc->position_source;
        ...
        if (locExtended->flags & GPS_LOCATION_EXTENDED_HAS_VERT_UNC) {
            res.mHasVert_unc = true;
            res.mVert_unc = locExtended->vert_unc;
        }
        if (locExtended->flags & GPS_LOCATION_EXTENDED_HAS_SPEED_UNC) {
            res.mHasSpeed_unc = true;
            res.mSpeed_unc = locExtended->speed_unc;
        }
        if (locExtended->flags & GPS_LOCATION_EXTENDED_HAS_BEARING_UNC) {
            res.mHasBearing_unc = true;
            res.mBearing_unc = locExtended->bearing_unc;
        }
        if (locExtended->flags & GPS_LOCATION_EXTENDED_HAS_HOR_RELIABILITY) {
            res.mHasHorizontal_reliability = true;
            res.mHorizontal_reliability = locExtended->horizontal_reliability;
        }
        if (locExtended->flags & GPS_LOCATION_EXTENDED_HAS_VERT_RELIABILITY) {
            res.mHasVertical_reliability = true;
            res.mVertical_reliability = locExtended->vertical_reliability;
        }
        ...
        if (locExtended->flags & GPS_LOCATION_EXTENDED_HAS_SV_SOURCE_INFO) {
            res.mHasSvSourceInfo = true;
            res.mSvSourceInfo = locExtended->sv_source;
        }
...
void IzatManager::startPassiveNmeaListenerMsg::proc() const {
    ...
    IzatPassiveNmeaListener::startPassiveNmeaListener(
      mClientName, mListensTo, mIzatManager, mIzatManager->mIzatContext.mIPCMessagingProxyObj);
