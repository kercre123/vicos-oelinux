...
private:
    ...
    static void svinfo_callback (const LocGnssSvStatus *gnssSvStatus,
    ...
    struct svInfoCallbackMsg : public LocMsg {
        ...
        IzatSvInfo mSvInfo;
        uint16_t mSource;
        inline svInfoCallbackMsg(IzatManager *mgrObj, const LocGnssSvStatus *gnssSvStatus,
            const GpsLocationExtended *locExtended, uint16_t source)
            :mIzatManager(mgrObj), mSource(source) {
            mSvInfo.mGnssSvStatus = *gnssSvStatus;
            mSvInfo.mLocExtended = *locExtended;
        }
