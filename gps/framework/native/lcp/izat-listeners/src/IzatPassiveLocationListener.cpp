...
)
{
    ...
    if ((NULL != izatLoc) && (NULL != locExtended)) {
        ...
        if (izatLoc->mHasSvSourceInfo) {
            locExtended->flags |= GPS_LOCATION_EXTENDED_HAS_SV_SOURCE_INFO;
            locExtended->sv_source = izatLoc->mSvSourceInfo;
        }
        ...
        if (izatLoc->mHasPositionDynamics) {
            ...
            locExtended->bodyFrameData.bodyFrameDatamask = izatLoc->mHasPositionDynamics;
