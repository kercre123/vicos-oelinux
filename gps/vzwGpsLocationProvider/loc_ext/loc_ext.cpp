...
--------   ---      ---------------------------------------------------
04/25/16   hahariha  If confidence value is less than 68%, set horizontal
                     confidence to 68% since accuracy value is scaled to
                     68% in location HAL.
...
static void local_sv_status_callback(LocGpsSvStatus* sv_status, void* sv_dop);
...
int loc_ext_init()
{
    ...
    if (NULL == context) {
        context = new ContextBase(
                        new MsgTask((LocThread::tCreate)clientCallbacks.create_thread_cb,
                                    "loc_ext"),
...
{
    ENTRY_LOG();
    LocGpsPositionRecurrence recurrence = isSingleShot ?
    ...
    // override the default setting.  VZW does not want to share fixes
    bool sharePosition = false;

    LocPosMode params(locMode, recurrence, fix_frequency * 1000,
                      100, timeOut_sec * 1000, sharePosition, credentials, "VERIZON");
...
void loc_ext_delete_aiding_data(LocGpsAidingData f)
{
    ENTRY_LOG();
...
static void* parsePositionExt(void* data)
{
    ...
    if (NULL != data) {
      ...
      if (locationExt == NULL) {
        ...
        {
            locationExt->flags    |= VZW_GPS_LOCATION_HAS_CONFIDENCE;
            // If confidence value is less than 68%, then accuracy value will be scaled to 68%.
            // confidence in location HAL. So for reported confidence less than 68%, updating
            // confidence_horizontal to 68.
            locationExt->confidence_horizontal = (location_report_ptr->horConfidence < 68)? 68 :
                                                                location_report_ptr->horConfidence;
        }
...
static void local_location_callback(UlpLocation* location, void* ext)
{
    ...
    if (NULL != location && NULL != ext) {
        ...
        LocGpsStatus gs ;
        gs.size = sizeof(gs);
        gs.status = LOC_GPS_STATUS_SESSION_END;
