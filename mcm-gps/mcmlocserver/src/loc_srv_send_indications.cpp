//=============================================================================
// FILE: loc_srv_send_send_indications.c
//
// DESCRIPTION:
// Send Indications
//
...
void loc_srv_send_sv_status_ind (LocGpsSvStatus *sv_info) {
    ...
    loc_srv_conv_gps_sv_status(sv_info, &payload.sv_info);
    for( ; (index < LOC_SRV_MAX_CLIENT_HANDLES); index++) {
        if(client_info =
...
void loc_srv_send_nmea_ind (LocGpsUtcTime timestamp, const char* nmea, int length){
    ...
    memcpy((void*)payload.nmea, (const void *)nmea, payload.length);
...
void loc_srv_send_gps_ni_ind (LocGpsNiNotification *notification) {
    ...
    loc_srv_conv_gps_ni_notification(notification, &payload.notification);
    for( ; (index < LOC_SRV_MAX_CLIENT_HANDLES); index++) {
        if(client_info =
