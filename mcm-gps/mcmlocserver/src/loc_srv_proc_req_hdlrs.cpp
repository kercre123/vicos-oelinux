//=============================================================================
// FILE: loc_srv_proc_req_hdlrs.c
//
// DESCRIPTION:
// Location service "process request" handler implementation
//
...
int loc_set_position_mode_proc_req_hdlr (mcm_gps_position_mode_t_v01 mode,
    ...
    LocGpsPositionMode m;
    LocGpsPositionRecurrence r;

    LOC_SRV_NULL_CHECK(loc_srv_get_gps_iface_ptr());

    loc_srv_conv_mcm_gps_position_mode(&mode, &m);
    loc_srv_conv_mcm_gps_position_recurrence(&recurrence, &r);

    rc = loc_srv_get_gps_iface_ptr()->set_position_mode(
                   m,
                   r,
                   min_interval,
                   preferred_accuracy,
...
int loc_start_nav_proc_req_hdlr (){
    ...
    int rc = LOC_SRV_FAILURE;
    LOC_SRV_NULL_CHECK(loc_srv_get_gps_iface_ptr());
    rc = loc_srv_get_gps_iface_ptr()->start();
...
int loc_stop_nav_proc_req_hdlr (){
    ...
    int rc = LOC_SRV_FAILURE;
    LOC_SRV_NULL_CHECK(loc_srv_get_gps_iface_ptr());
    rc = loc_srv_get_gps_iface_ptr()->stop();
...
int loc_delete_aiding_data_proc_req_hdlr (mcm_gps_aiding_data_t_v01 flags){
    ...
    loc_srv_get_gps_iface_ptr()->delete_aiding_data(f);
...
int loc_inject_time_proc_req_hdlr (int64_t time,
    ...
    int rc = LOC_SRV_FAILURE;
    rc = loc_extended_inject_time( (LocGpsUtcTime) time,
                                                time_reference,
                                                uncertainty);
...
int loc_inject_location_proc_req_hdlr (double latitude,
    ...
    int rc = LOC_SRV_FAILURE;
    LOC_SRV_NULL_CHECK(loc_srv_get_gps_iface_ptr());
    rc = loc_srv_get_gps_iface_ptr()->inject_location(latitude,
                                                      longitude,
                                                      accuracy);
...
int loc_agps_data_conn_open_proc_req_hdlr (mcm_agps_t_v01 agps_type,
    ...
    AGpsExtType a;
    AGpsBearerType b;
    loc_srv_conv_mcm_agps_type(&agps_type, &a);
    loc_srv_conv_mcm_agps_bearer(&bearer_type, &b);
    rc = loc_extended_agps_open(a, apn, b);
...
int loc_agps_data_conn_failed_proc_req_hdlr (mcm_agps_t_v01 agps_type){
    ...
    AGpsExtType a;
    loc_srv_conv_mcm_agps_type(&agps_type, &a);
    rc = loc_extended_agps_open_failed(a);
...
int loc_agps_set_server_proc_req_hdlr (mcm_agps_t_v01 agps_type,
    ...
    if(!host_name)
        return rc;
    loc_srv_conv_mcm_agps_type(&agps_type, &a);
    rc = loc_extended_agps_set_server(a, host_name, port);
...
int loc_agps_ril_update_network_availability_proc_req_hdlr (int32_t available,
    ...
    int rc = LOC_SRV_SUCCESS;
    LOC_SRV_NULL_CHECK(loc_srv_get_gps_iface_ptr());
    LOC_SRV_NULL_CHECK(loc_srv_get_agps_ril_iface_ptr());
    loc_srv_get_agps_ril_iface_ptr()->update_network_availability(available, apn);
