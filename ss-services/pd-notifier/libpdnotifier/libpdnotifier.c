		...
		void *user_data)
{
	...
	strncpy(handle->serviceName, serviceName,
			sizeof(handle->serviceName) - 1);
	memset(handle->clientName, '\0', sizeof(handle->clientName));
	strncpy(handle->clientName, clientName,
			sizeof(handle->clientName) - 1);
		...
		void *hndl)
{
	...
	ack.service_name[sizeof(ack.service_name) - 1] = '\0';
	strncpy(ack.service_name,
			msg.service_name,
			sizeof(ack.service_name) - 1);
