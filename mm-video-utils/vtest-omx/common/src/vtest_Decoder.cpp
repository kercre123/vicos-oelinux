...
OMX_ERRORTYPE Decoder::FillDoneCallback(OMX_IN OMX_HANDLETYPE hComponent,
    ...
    if (pHeader->nFlags & OMX_BUFFERFLAG_EXTRADATA) {
        result = pThis->ParseExtraData(pHeader);
        FAILED2(result, pThis->SetError(),
                "Error in processing extra data buffer %p", pBuffer);
...
OMX_ERRORTYPE Decoder::ParseExtraData(OMX_BUFFERHEADERTYPE *pHeader) {
    ...
    while (pExtra && ((OMX_U8*)pExtra < (pBaseAdd + nBuffer_size)) &&
        ...
        switch ((unsigned long)pExtra->eType)
        {
            case OMX_ExtraDataInterlaceFormat:
            {
                ...
                if (m_fInterlaceRef)
                {
                    ...
                    if (sInterlaceFormat != NULL)
                    {
                        ...
                        {
                            VTEST_MSG_ERROR("ExtraData Mismatch with Reference: Interlace Frame# %u!",m_nFrameDoneCount);
                            result = OMX_ErrorUndefined;
                        }
                    }
                }
            ...
            case OMX_ExtraDataFrameInfo:
            {
                ...
                /* TODO: Process Recovery Point SEI Extradata */
...
#ifndef NATIVE_WINDOW_SINK_DISABLE
    if ((strstr(m_pSink->Name(), "Native") != NULL) && (pBuff != MAP_FAILED) && handle)
    {
        if(munmap(pBuff, handle->size))
            VTEST_MSG_ERROR("munmap failed for pBuff : %p", pBuff);
    }
#endif
    return result;
}
