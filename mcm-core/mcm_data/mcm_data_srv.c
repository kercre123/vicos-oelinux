...
)
{
  ...
  {
    MCM_DATA_LOG_ERROR("%s", "DSD service object not found");
    MCM_DATA_LOG_ERROR("%s", "Continuing without DSD Service support");
    goto BAIL;
  }

  /* Using QMI_CLIENT_QMUX_RMNET_INSTANCE_0 as we are currently
  using mcm-core only on MDM targets. This has to be set based on target
  in case mcm-core is used on LA in future.*/
