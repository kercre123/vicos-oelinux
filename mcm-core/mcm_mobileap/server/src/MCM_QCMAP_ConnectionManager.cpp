...
FILE:  MCM_QCMAP_ConnectionManager.cpp

SERVICES:
   MCM QCMAP Connection Manager Implementation
...
/*===========================================================================
  ...
  08/30/13   cp         IOE Initial Module
===========================================================================*/
#include <fstream>
#include <iostream>
...
#define HOSTAPD_CFG_PATH "/data/misc/wifi/hostapd.conf"
#define A5_IP_ADDR "192.168.225.1"
#define A5_SUB_NET_MASK "255.255.255.0"

#define A5_RMNET_SUB_NET_MASK "255.255.255.128"

#define HOSTAPD_CFG_PATH_SSID2 "/data/misc/wifi/hostapd-eth1.conf"
...
/* Client sockets */
extern unsigned int qcmap_cb_sockfd;

static void MCM_MOBILEAP_ConnectionManager_callback
(
...
)
{
  ...
  MCM_QCMAP_ConnectionManager *manager;
  mcm_mobileap_event_buffer_t event_buffer;
  int numBytes = 0, len;
  sockaddr_un client_endpt;

  LOG_MSG_INFO1("MCM_MOBILEAP_ConnectionManager_callback: Event Received %d handle %d", event, handle, 0);
  ...
  event_buffer.qcmap_handle = handle;
  event_buffer.evt = event;
  event_buffer.user_data = qcmap_cm_cb_user_data;
  if ((numBytes = sendto(qcmap_cb_sockfd, (void *)&event_buffer, sizeof(mcm_mobileap_event_buffer_t), 0,
...
MCM_QCMAP_ConnectionManager::MCM_QCMAP_ConnectionManager
(
...
boolean MCM_QCMAP_ConnectionManager::IPv4v6Only()
{
...
boolean MCM_QCMAP_ConnectionManager::IoEMode()
{
...
boolean MCM_QCMAP_ConnectionManager::TetheringOnly()
{
  if (this->cfg.nat_config.tethering_only)
  {
...
void MCM_QCMAP_ConnectionManager::MCM_MOBILEAP_handle_callback_events
(
  int 			  handle,				/* MobileAP Application id */
  qcmap_cm_event_e	event,				  /* Type of MobileAP Event  */
  void			   *mgr /* Call back user data	 */
...
)
{
  ...
  if (mgr != NULL)
  {
    manager = (MCM_QCMAP_ConnectionManager *)mgr;
  }
  else
  {
    return;
...
/*-----------------------------------------------------------------------
Posting the signal for QTI to make it aware that LAN  has come up.
-----------------------------------------------------------------------*/
...
----------------------------------------------------------------------*/
  ...
  {
    manager->EnableWLAN(&mcm_err_num);
    if(manager->GetAutoconnect(&mcm_err_num))
    {
      manager->ConnectBackHaul(&mcm_err_num);
    }
  }
  break;
...
-----------------------------------------------------------------------*/
  if(manager->GetQtiState() && manager->GetAutoconnect(&mcm_err_num))
  {
    manager->ConnectBackHaul(&mcm_err_num);
  }
  }
...
-------------------------------------------------------------------- */
  ...
  case QCMAP_CM_EVENT_DISABLED:
    ...
    manager->DisableWLAN(&mcm_err_num);
    break;
  default:
...
boolean MCM_QCMAP_ConnectionManager::Enable(int *handle, void *cb_user_data, mcm_error_t_v01 *mcm_err_num)
{
  ...
  ((mcm_mobileap_svc_handle_type *)cb_user_data)->Mgr = this;

  this->qcmap_cm_handle = *handle = qcmap_cm_enable(&this->cfg, MCM_MOBILEAP_ConnectionManager_callback, cb_user_data, &qcmap_cm_errno);
  *mcm_err_num = mcm_mobileap_errno[qcmap_cm_errno];
...
boolean MCM_QCMAP_ConnectionManager::Disable(mcm_error_t_v01 *mcm_err_num)
{
  int ret = 0, qcmap_cm_errno = QCMAP_CM_ESUCCESS;
...
boolean MCM_QCMAP_ConnectionManager::EnableTethering()
{
...
boolean MCM_QCMAP_ConnectionManager::EnableWLAN
(
  mcm_error_t_v01 *mcm_err_num
)
{
  ...
  char devmode[16] = { '\0' };
  char devname[16];
  int s, i, qcmap_cm_error = QCMAP_CM_ESUCCESS, dev;
  struct ifreq buffer;
  int lte_frequency = -1;
  boolean ret_val;
  char scratch_buf[16];
  ...
  ds_system_call(command, strlen(command));
#endif
  *mcm_err_num = mcm_mobileap_errno[qcmap_cm_error];
...
boolean MCM_QCMAP_ConnectionManager::EnableIPV6Forwarding()
{
  char command[MAX_COMMAND_STR_LEN];
  char devname[16];
  int qcmap_cm_error;
...
boolean MCM_QCMAP_ConnectionManager::DisableWLAN
(
  mcm_error_t_v01 *mcm_err_num
)
{
  ...
  struct in_addr addr;
  char devname[16];
  int qcmap_cm_error = QCMAP_CM_ESUCCESS;
...
boolean MCM_QCMAP_ConnectionManager::EnableStaMode
(
...
/*-------------------------------------------------------------------------*/
  ...
  if (!this->qcmap_enable)
  {
    ...
    *mcm_err_num = MCM_ERROR_INVALID_HANDLE_V01;
    return FALSE;
  }
  ...
  if( this->sta_iface_index < 0 )
  {
    ...
    *mcm_err_num = MCM_ERROR_INVALID_INDEX_V01;
    return false;
  }
  ...
  if( !ConnectSTA(conn_type,&static_ip_config, mcm_err_num) )
  {
  ...
  *mcm_err_num = mcm_mobileap_errno[qcmap_cm_errno];
  ...
  if ( false == DisconnectBackHaul(mcm_err_num) )
  {
...
boolean MCM_QCMAP_ConnectionManager::DisableStaMode
(
  mcm_error_t_v01 *mcm_err_num
)
{
...
/*-------------------------------------------------------------------------*/
  ...
  *mcm_err_num = mcm_mobileap_errno[qcmap_cm_errno];
  ...
  {
    if ( false == ConnectBackHaul(mcm_err_num) )
    {
...
boolean MCM_QCMAP_ConnectionManager::EnableNATonA5()
{
  char command[MAX_COMMAND_STR_LEN];
  struct in_addr addr;
  int i = 0;
...
/*-------------------------------------------------------------------------*/
  ...
  {
    AddDMZOnA5(this->cfg.nat_config.dmz_ip, &mcm_err_num);
  }

  /* Config SNAT */
  for (i = 0; i < this->cfg.nat_config.num_port_fwding_entries; i++)
  {
      AddSNATEntryOnA5(&this->cfg.nat_config.port_fwding_entries[i], &mcm_err_num);
  }
  ...
  SetIPSECVpnPassThroughOnA5(this->cfg.nat_config.enable_ipsec_vpn_pass_through, &mcm_err_num);
  SetL2TPVpnPassThroughOnA5(this->cfg.nat_config.enable_l2tp_vpn_pass_through, &mcm_err_num);
  SetPPTPVpnPassThroughOnA5(this->cfg.nat_config.enable_pptp_vpn_pass_through, &mcm_err_num);

  return true;
}

boolean MCM_QCMAP_ConnectionManager::DisableNATonA5()
{
  char command[MAX_COMMAND_STR_LEN];
  char devname[16];
  int qcmap_cm_error;
...
boolean MCM_QCMAP_ConnectionManager::ReadConfigFromXML()
{
...
boolean MCM_QCMAP_ConnectionManager::ReadConfigFromFirewallXML()
{
...
boolean MCM_QCMAP_ConnectionManager::WriteConfigToXML()
{
  if ((this->xml_path) && (this->xml_path[0] != '\0') &&
...
boolean MCM_QCMAP_ConnectionManager::WriteConfigToFirewallXML()
{
  if ((this->cfg.nat_config.firewall_config_file) && (this->cfg.nat_config.firewall_config_file[0] != '\0') &&
...
boolean MCM_QCMAP_ConnectionManager::ConnectBackHaul
(
  mcm_error_t_v01 *mcm_err_num
)
{
...
boolean MCM_QCMAP_ConnectionManager::EnableDNS()
{
  int qcmap_cm_errno = QCMAP_CM_ESUCCESS;
  int ret_val;
  int rc = 0;
  ...
  if (qcmap_cm_get_ipv4_net_conf(this->qcmap_cm_handle, &public_ip, &pri_dns_addr, &sec_dns_addr, &qcmap_cm_errno) == QCMAP_CM_SUCCESS)
  {
    if (pri_dns_addr)
    {
      ...
      snprintf(command, MAX_COMMAND_STR_LEN, "echo 'nameserver %s' > /data/resolv.conf", inet_ntoa(addr));
...
boolean MCM_QCMAP_ConnectionManager::GetNatTypeChangeStatus()
{
  return this->qcmap_nat_type_change_in_progress;
}


boolean MCM_QCMAP_ConnectionManager::DisconnectBackHaul(mcm_error_t_v01 *mcm_err_num)
{
  int qcmap_cm_errno = QCMAP_CM_ESUCCESS;
  int ret_val;
...
boolean MCM_QCMAP_ConnectionManager::AddStaticNatEntry
(
...
)
{
  ...
  for (i = 0; i < this->cfg.nat_config.num_port_fwding_entries; i++)
  {
        ...
        (nat_entry->port_fwding_global_port== this->cfg.nat_config.port_fwding_entries[i].port_fwding_global_port))
    {
      ...
      *mcm_err_num = MCM_ERROR_NO_EFFECT_V01;
      return false;
    }
  }
  ...
  if (this->cfg.nat_config.num_port_fwding_entries == QCMAP_MAX_SNAT_ENTRIES_V01)
  {
    ...
    *mcm_err_num = MCM_ERROR_INSUFFICIENT_RESOURCES_V01;
    return false;
  }
  ...
  {
    if(!AddSNATEntryOnA5(nat_entry, mcm_err_num))
      return false;
  }
...
boolean MCM_QCMAP_ConnectionManager::DeleteStaticNatEntry
(
  qcmap_cm_port_fwding_entry_conf_t* nat_entry,
  mcm_error_t_v01 *mcm_err_num
)
{
  ...
  {
    if(!DeleteSNATEntryOnA5(nat_entry, mcm_err_num))
      return false;
  }
...
int MCM_QCMAP_ConnectionManager::GetStaticNatEntry
(
  qcmap_cm_port_fwding_entry_conf_t* nat_entry,
  int max_entries,
  mcm_error_t_v01 *mcm_err_num
...
boolean MCM_QCMAP_ConnectionManager::AddDMZ(uint32 dmz_ip, mcm_error_t_v01 *mcm_err_num)
{
  ...
  {
    if(!AddDMZOnA5(dmz_ip, mcm_err_num))
      return false;
  }
...
boolean MCM_QCMAP_ConnectionManager::DeleteDMZ(uint32 dmz_ip, mcm_error_t_v01 *mcm_err_num)
{
  ...
  if ( GetDMZ(&check_dmz, mcm_err_num) )
  {
    if (check_dmz != dmz_ip)
    {
      LOG_MSG_ERROR("DMZ requested for deletion is not same as original DMZ",0,0,0);
      *mcm_err_num = MCM_ERROR_NOT_SUPPORTED_V01;
      return false;
    }
  }
  ...
  {
    if(!DeleteDMZOnA5(this->cfg.nat_config.dmz_ip, mcm_err_num))
      return false;
  }
...
boolean MCM_QCMAP_ConnectionManager::SetIPSECVpnPassThrough(boolean enable, mcm_error_t_v01 *mcm_err_num)
{
  int qcmap_cm_errno = QCMAP_CM_ESUCCESS;
  int ret_val;
  ...
  {
    /* STA mode. */
    if (!SetIPSECVpnPassThroughOnA5(enable, mcm_err_num))
      return false;
  }
...
)
{
  ...
  if(this->sta_connected)
  {
    *vpn_value= this->cfg.nat_config.enable_ipsec_vpn_pass_through;
    return TRUE;
  }

  /* call CM lib to get NAT type */
  ret_val = qcmap_cm_vpn_op_get_ipsec(vpn_value, &qcmap_cm_errno);
  *mcm_err_num = mcm_mobileap_errno[qcmap_cm_errno];
  if (ret_val != QCMAP_CM_SUCCESS)
  {
    return false;
...
boolean MCM_QCMAP_ConnectionManager::SetL2TPVpnPassThrough(boolean enable, mcm_error_t_v01 *mcm_err_num)
{
  int qcmap_cm_errno = QCMAP_CM_ESUCCESS;
  int ret_val;
  uint8 vpn_value = (enable) ? 1 : 0;
  ...
  {
    /* STA mode. */
    if (!SetL2TPVpnPassThroughOnA5(enable, mcm_err_num))
      return false;
  }
...
)
{
  ...
  if(this->sta_connected)
  {
    *vpn_value= this->cfg.nat_config.enable_l2tp_vpn_pass_through;
    return TRUE;
  }

  /* call CM lib to get NAT type */
  ret_val = qcmap_cm_vpn_op_get_l2tp(vpn_value, &qcmap_cm_errno);
  *mcm_err_num = mcm_mobileap_errno[qcmap_cm_errno];
  if (ret_val != QCMAP_CM_SUCCESS)
  {
    return false;
...
boolean MCM_QCMAP_ConnectionManager::SetPPTPVpnPassThrough(boolean enable, mcm_error_t_v01 *mcm_err_num)
{
  int qcmap_cm_errno = QCMAP_CM_ESUCCESS;
  int ret_val;
  uint8 vpn_value = (enable) ? 1 : 0;
  ...
  {
    /* STA mode. */
    if (!SetPPTPVpnPassThroughOnA5(enable, mcm_err_num))
      return false;
  }
...
)
{
  ...
  if (this->sta_connected)
  {
    *vpn_value= this->cfg.nat_config.enable_pptp_vpn_pass_through;
    return TRUE;
  }

  /* call CM lib to get NAT type */
  ret_val = qcmap_cm_vpn_op_get_pptp(vpn_value, &qcmap_cm_errno);
  *mcm_err_num = mcm_mobileap_errno[qcmap_cm_errno];
  if (ret_val != QCMAP_CM_SUCCESS)
  {
    return false;
...
boolean MCM_QCMAP_ConnectionManager::SetFirewall
(
  boolean enable,
...
)
{
  ...
  if (enable)
  {
    ...
    else
    {
      ...
      ret_val = this->SetFirewallOnA5(enable, pkts_allowed, mcm_err_num);
    }
  }
  else
  {
...
int MCM_QCMAP_ConnectionManager::SetFirewallOnA5
(
  boolean enable,
  boolean pkts_allowed,
  mcm_error_t_v01 *mcm_err_num
...
boolean MCM_QCMAP_ConnectionManager::SetAutoconnect(boolean enable, mcm_error_t_v01 *mcm_err_num)
{
  int qcmap_cm_errno = QCMAP_CM_ESUCCESS;
  int ret_val;
...
boolean MCM_QCMAP_ConnectionManager::GetAutoconnect(mcm_error_t_v01 *mcm_err_num)
{
  return this->cfg.wan_config.auto_connect;
}

boolean MCM_QCMAP_ConnectionManager::SetRoaming(boolean enable, mcm_error_t_v01 *mcm_err_num)
{
  int qcmap_cm_errno = QCMAP_CM_ESUCCESS;
  int ret_val;
...
boolean MCM_QCMAP_ConnectionManager::GetRoaming(mcm_error_t_v01 *mcm_err_num)
{
  return this->cfg.wan_config.roaming;
}

boolean MCM_QCMAP_ConnectionManager::GetWWANStatistics(
...
boolean MCM_QCMAP_ConnectionManager::ResetWWANStatistics(
                                   ip_version_enum_type ip_family,
                                   mcm_error_t_v01 *mcm_err_num)
{
  int ret_val;
...
boolean MCM_QCMAP_ConnectionManager::StartDHCPD()
{
  char command[MAX_COMMAND_STR_LEN];
  char scratch_buf[16];
  in_addr start, end;
...
boolean MCM_QCMAP_ConnectionManager::StopDHCPD()
{
...
boolean MCM_QCMAP_ConnectionManager::StartHostAPD(int interface)
{
  char command[MAX_COMMAND_STR_LEN];
...
boolean MCM_QCMAP_ConnectionManager::StopHostAPD(int intf)
{
  pid_t pid_hostapd;
...
void MCM_QCMAP_ConnectionManager::StartMcastDaemon(void)
{
  ds_system_call("pimd", strlen("pimd"));
}
void MCM_QCMAP_ConnectionManager::StopMcastDaemon(void)
{
  ds_system_call("killall -15 pimd", strlen("killall -15 pimd"));
}
void MCM_QCMAP_ConnectionManager::ReStartMcastDaemon(void)
{
  ds_system_call("pimd -l", strlen("pimd -l"));
}

boolean MCM_QCMAP_ConnectionManager::AddExtdFireWallEntry
(
...
boolean MCM_QCMAP_ConnectionManager::ModifyExtdFirewallEntryOnA5
(
...
)
{
  ...
  if(firewall_entry->filter_spec.ip_vsn == IP_V4)
  {
    ...
    if(firewall_entry->filter_spec.ip_hdr.v4.field_mask & IPFLTR_MASK_IP4_SRC_ADDR)
    {
      ...
      strlcpy(ip_addr, inet_ntoa(addr), 16);
      strlcpy(netmask_addr, inet_ntoa(subnet), 16);
...
boolean MCM_QCMAP_ConnectionManager::GetFireWallHandleList
(
  qcmap_cm_extd_firewall_conf_t* extd_firewall_conf,
  mcm_error_t_v01 *mcm_err_num
)
...
boolean MCM_QCMAP_ConnectionManager::GetExtdFireWallEntry
(
  qcmap_cm_extd_firewall_conf_t* extd_firewall_conf,
  mcm_error_t_v01 *mcm_err_num
)
{
  ...
  if(!this->sta_connected)
  {
    ...
    QCMAP_CM_LOG("Received firewall handle from svc hdlr : %d",extd_firewall_conf->extd_firewall_entry.firewall_handle);
    ret_val = qcmap_cm_extd_firewall_op(this->qcmap_cm_handle,
...
boolean MCM_QCMAP_ConnectionManager::DeleteExtdFireWallEntry
(
  qcmap_cm_extd_firewall_conf_t* extd_firewall_entry,
  mcm_error_t_v01 *mcm_err_num
)
{
  int qcmap_cm_errno = QCMAP_CM_ESUCCESS;
    ...
    for (i = 0; i < this->cfg.nat_config.num_extd_firewall_entries; i++)
    {
          ...
          this->cfg.nat_config.extd_firewall_entries[i].extd_firewall_entry.firewall_handle)
      {
        QCMAP_CM_LOG("Found matching handle %d and %d index is %d",
                     ...
                     this->cfg.nat_config.extd_firewall_entries[i].extd_firewall_entry.firewall_handle, i );
        extd_firewall_entry->extd_firewall_handle.ip_family =
                     this->cfg.nat_config.extd_firewall_entries[i].extd_firewall_entry.filter_spec.ip_vsn;
        break;
      }
    }
...
boolean MCM_QCMAP_ConnectionManager::AddSNATEntryOnA5
(
  qcmap_cm_port_fwding_entry_conf_t* nat_entry,
  mcm_error_t_v01 *mcm_err_num
...
boolean MCM_QCMAP_ConnectionManager::DeleteSNATEntryOnA5
(
  qcmap_cm_port_fwding_entry_conf_t* nat_entry,
  mcm_error_t_v01 *mcm_err_num
)
...
boolean MCM_QCMAP_ConnectionManager::SetIPSECVpnPassThroughOnA5
(
  boolean enable,
  mcm_error_t_v01 *mcm_err_num
)
...
boolean MCM_QCMAP_ConnectionManager::SetL2TPVpnPassThroughOnA5
(
  boolean enable,
  mcm_error_t_v01 *mcm_err_num
)
...
boolean MCM_QCMAP_ConnectionManager::SetPPTPVpnPassThroughOnA5
(
  boolean enable,
  mcm_error_t_v01 *mcm_err_num
)
...
boolean MCM_QCMAP_ConnectionManager::ConnectSTA
(
  qcmap_sta_connection_e conn_type,
  qcmap_sta_static_ip_config * static_ip_config,
  mcm_error_t_v01 *mcm_err_num
...
/*-------------------------------------------------------------------------*/
  ...
  if( this->cfg.lan_config.interface[this->sta_iface_index].enable_supplicant )
  {
     ...
     *mcm_err_num = MCM_ERROR_INTERNAL_V01;
      return false;
    }
  ...
  else
  {
    ...
    ds_system_call(command, strlen(command));
    QCMAP_CM_LOG("%s\n", command);
    /* Populate the /data/resolv.conf file with DNS address provided. */
    addr.s_addr = htonl(static_ip_config->dns_addr);
    snprintf(command, MAX_COMMAND_STR_LEN,
             "echo 'nameserver %s' > /data/resolv.conf", inet_ntoa(addr));
    ds_system_call(command, strlen(command));
    QCMAP_CM_LOG("%s\n", command);
  }
  /*
   * Just check whether the STA Interface is UP. No need to add the DNS address
   * to /data/resolv.conf file. It will taken care by dhcpcd.
   */
...
boolean MCM_QCMAP_ConnectionManager::DisconnectSTA()
{
  char command[MAX_COMMAND_STR_LEN];
/*-------------------------------------------------------------------------*/
  QCMAP_CM_LOG_FUNC_ENTRY();
...
boolean MCM_QCMAP_ConnectionManager::GetStaIP(char *staIP, int staIPBufLen, char *netMask, int netMaskBufLen)
{
  int s, ret;
  struct ifreq buffer;
  struct sockaddr *sa;
...
)
{
  ...
  if (qcmap_cm_get_state() != QCMAP_CM_WAN_CONNECTED)
  {
    ...
    *mcm_err_num = MCM_ERROR_NO_NETWORK_FOUND_V01;
    return false;
  }
...
boolean MCM_QCMAP_ConnectionManager::ChangeNatType
(
...
/*-------------------------------------------------------------------------*/
  ...
  if(this->GetNatType(&cur_nat_type, mcm_err_num))
  {
...
boolean MCM_QCMAP_ConnectionManager::GetNatType
(
  qcmap_cm_nat_type *cur_nat_type,
  mcm_error_t_v01 *mcm_err_num
)
...
boolean MCM_QCMAP_ConnectionManager::SetNatType()
{
  int qcmap_cm_errno = QCMAP_CM_ESUCCESS;
  int ret_val;
  boolean ret = TRUE;
...
boolean MCM_QCMAP_ConnectionManager::SendWanConfig()
{
...
void MCM_QCMAP_ConnectionManager::SetQtiState
(
  boolean enable
)
{
...
boolean MCM_QCMAP_ConnectionManager::GetQtiState()
{
  /* This state variable tracks the current QTI link state. */
  return this->cfg.lan_config.qti_link_up;
}
boolean MCM_QCMAP_ConnectionManager::GetIPV6PrefixInfo()
{
  ...
  {
    LOG_MSG_ERROR("GetIPV6PrefixInfo: sendto failed: %d\n", errno, 0, 0);
    ret = false;
    goto bail;
  }
  ...
  {
    LOG_MSG_ERROR("GetIPV6PrefixInfo: recvmsg failure: %d\n", errno, 0, 0);
    ret = false;
    goto bail;
  }
  else
...
void MCM_QCMAP_ConnectionManager::RemoveIPV6Address()
{
  char command[MAX_COMMAND_STR_LEN];
