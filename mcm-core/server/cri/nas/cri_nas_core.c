...
qmi_error_type_v01 cri_nas_init_client_state()
{
    ...
    cri_nas_nw_network_time_info =
        util_memory_alloc(sizeof(*cri_nas_nw_network_time_info));
    cri_nas_nw_lte_sib16_network_time_info =
        util_memory_alloc(sizeof(*cri_nas_nw_lte_sib16_network_time_info));
...
void cri_nas_cleanup_client_state()
{
    ...
    if(cri_nas_nw_network_time_info)
    {
       util_memory_free((void**) &cri_nas_nw_network_time_info);
    }

    if(cri_nas_nw_lte_sib16_network_time_info)
    {
       util_memory_free((void**) &cri_nas_nw_lte_sib16_network_time_info);
    }
}
...
void cri_nas_core_nas_network_time_ind_handler(int qmi_service_client_id,
                                                 nas_network_time_ind_msg_v01 *ind_msg)
{
    if(ind_msg)
    {
        if(cri_nas_nw_network_time_info)
        {
            CRI_STORE_MAND_TLV(cri_nas_nw_network_time_info->universal_time,
                               ind_msg->universal_time);
            CRI_STORE_OPT_TLV(cri_nas_nw_network_time_info->time_zone,
                              ind_msg->time_zone);
            CRI_STORE_OPT_TLV(cri_nas_nw_network_time_info->daylt_sav_adj,
                              ind_msg->daylt_sav_adj);
            CRI_STORE_OPT_TLV(cri_nas_nw_network_time_info->radio_if,
                              ind_msg->radio_if);
        }
    }
}

void cri_nas_core_nas_lte_sib16_network_time_ind_handler(int qmi_service_client_id,
                                                 nas_lte_sib16_network_time_ind_msg_v01 *ind_msg)
{
    if(ind_msg)
    {
        if(cri_nas_nw_lte_sib16_network_time_info)
        {
            CRI_STORE_OPT_TLV(cri_nas_nw_lte_sib16_network_time_info->lte_sib16_acquired,
                              ind_msg->lte_sib16_acquired);
            CRI_STORE_OPT_TLV(cri_nas_nw_lte_sib16_network_time_info->universal_time,
                              ind_msg->universal_time);
            CRI_STORE_OPT_TLV(cri_nas_nw_lte_sib16_network_time_info->abs_time,
                              ind_msg->abs_time);
            CRI_STORE_OPT_TLV(cri_nas_nw_lte_sib16_network_time_info->leap_sec,
                              ind_msg->leap_sec);
            CRI_STORE_OPT_TLV(cri_nas_nw_lte_sib16_network_time_info->time_zone,
                              ind_msg->time_zone);
            CRI_STORE_OPT_TLV(cri_nas_nw_lte_sib16_network_time_info->daylt_sav_adj,
                              ind_msg->daylt_sav_adj);
        }
    }
}
    ...
    switch(message_id)
    {
        ...
        case QMI_NAS_SYSTEM_SELECTION_PREFERENCE_IND_MSG_V01:
            cri_nas_core_system_selection_ind_handler(qmi_service_client_id,
                                                        ind_data);
            cri_nas_ind_id = CRI_NAS_DEVICE_STATE_INFO_IND;
            break;
        ...
        case QMI_NAS_NETWORK_TIME_IND_MSG_V01:
            cri_nas_core_nas_network_time_ind_handler(qmi_service_client_id,
                                                        ind_data);
            cri_nas_ind_id = CRI_NAS_NETWORK_TIME_IND;
            break;

        case QMI_NAS_LTE_SIB16_NETWORK_TIME_IND_V01:
            cri_nas_core_nas_lte_sib16_network_time_ind_handler(qmi_service_client_id,
                                                        ind_data);
            cri_nas_ind_id = CRI_NAS_LTE_SIB16_NETWORK_TIME_IND;
            break;
...
uint8_t cri_nas_is_current_plmn(uint32_t mcc, uint32_t mnc)
{
    ...
    if ( mcc == atoi(curr_mcc) && mnc == atoi(curr_mnc))
    {
        ret = TRUE;
    }
    return ret;
...
void cri_nas_update_voice_network_info()
{
    ...
    if ( cri_nas_core_rte_info.cri_nas_voice_rte == CRI_NAS_RTE_GSM )
    {
        ...
        {
            snprintf(lac, sizeof(lac),
                "%d",
                 cri_nas_core_sys_info->gsm_sys_info.threegpp_specific_sys_info.lac);

            strlcpy(cri_nas_rte_gsm_reg_info[CRI_NAS_RTE_SERVICE_VOICE]->lac,lac,sizeof(cri_nas_rte_gsm_reg_info[CRI_NAS_RTE_SERVICE_VOICE]->lac));
        }
        ...
        if ( cri_nas_core_sys_info->gsm_sys_info.threegpp_specific_sys_info.cell_id_valid == TRUE )
        {
            ...
            strlcpy(cri_nas_rte_gsm_reg_info[CRI_NAS_RTE_SERVICE_VOICE]->cid,cid,sizeof(cri_nas_rte_gsm_reg_info[CRI_NAS_RTE_SERVICE_VOICE]->cid));
        }
    }
    ...
    if ( cri_nas_core_rte_info.cri_nas_voice_rte == CRI_NAS_RTE_WCDMA )
    {
        if ( cri_nas_core_sys_info->wcdma_sys_info.threegpp_specific_sys_info.lac_valid == TRUE )
        {
            ...
            strlcpy(cri_nas_rte_wcdma_reg_info[CRI_NAS_RTE_SERVICE_VOICE]->lac,lac,sizeof(cri_nas_rte_wcdma_reg_info[CRI_NAS_RTE_SERVICE_VOICE]->lac));
        }
        ...
        if ( cri_nas_core_sys_info->wcdma_sys_info.threegpp_specific_sys_info.cell_id_valid == TRUE )
        {
            ...
            strlcpy(cri_nas_rte_wcdma_reg_info[CRI_NAS_RTE_SERVICE_VOICE]->cid,cid,sizeof(cri_nas_rte_wcdma_reg_info[CRI_NAS_RTE_SERVICE_VOICE]->cid));
        }
        if ( cri_nas_core_sys_info->wcdma_sys_info.wcdma_specific_sys_info.psc_valid == TRUE )
        {
            ...
            strlcpy(cri_nas_rte_wcdma_reg_info[CRI_NAS_RTE_SERVICE_VOICE]->psc,psc,sizeof(cri_nas_rte_wcdma_reg_info[CRI_NAS_RTE_SERVICE_VOICE]->psc));
        }
    }
    ...
    if ( cri_nas_core_rte_info.cri_nas_voice_rte == CRI_NAS_RTE_CDMA )
    {
        if ( cri_nas_core_sys_info->cdma_sys_info.cdma_specific_sys_info.cdma_sys_id_valid == TRUE )
        {
            snprintf(sid,sizeof(sid),
                "%d",
                 cri_nas_core_sys_info->cdma_sys_info.cdma_specific_sys_info.cdma_sys_id);

            strlcpy(cri_nas_rte_cdma_reg_info[CRI_NAS_RTE_SERVICE_VOICE]->sid,sid,sizeof(cri_nas_rte_cdma_reg_info[CRI_NAS_RTE_SERVICE_VOICE]->sid));
        }
        if ( cri_nas_core_sys_info->cdma_sys_info.cdma_specific_sys_info.network_id_valid == TRUE )
        {
            snprintf(nid,sizeof(nid),
                "%d",
                 cri_nas_core_sys_info->cdma_sys_info.cdma_specific_sys_info.network_id);

            strlcpy(cri_nas_rte_cdma_reg_info[CRI_NAS_RTE_SERVICE_VOICE]->nid, nid,sizeof(cri_nas_rte_cdma_reg_info[CRI_NAS_RTE_SERVICE_VOICE]->nid));
        }
    }
    ...
    if ( cri_nas_core_rte_info.cri_nas_voice_rte == CRI_NAS_RTE_LTE)
    {
        if ( cri_nas_core_sys_info->lte_sys_info.threegpp_specific_sys_info.lac_valid == TRUE )
        {
            ...
            strlcpy(cri_nas_rte_lte_reg_info[CRI_NAS_RTE_SERVICE_VOICE]->lac,lac,sizeof(cri_nas_rte_lte_reg_info[CRI_NAS_RTE_SERVICE_VOICE]->lac));
        }
        ...
        if ( cri_nas_core_sys_info->lte_sys_info.threegpp_specific_sys_info.cell_id_valid == TRUE )
        {
            ...
            strlcpy(cri_nas_rte_lte_reg_info[CRI_NAS_RTE_SERVICE_VOICE]->cid,cid,sizeof(cri_nas_rte_lte_reg_info[CRI_NAS_RTE_SERVICE_VOICE]->cid));
        }
        if ( cri_nas_core_sys_info->lte_sys_info.lte_specific_sys_info.tac_valid == TRUE )
        {
            ...
            strlcpy(cri_nas_rte_lte_reg_info[CRI_NAS_RTE_SERVICE_VOICE]->tac,tac,sizeof(cri_nas_rte_lte_reg_info[CRI_NAS_RTE_SERVICE_VOICE]->tac));
        }
    }
...
void cri_nas_update_data_network_info()
{
    ...
    if ( cri_nas_core_rte_info.cri_nas_voice_rte == CRI_NAS_RTE_GSM )
    {
        if ( cri_nas_core_sys_info->gsm_sys_info.threegpp_specific_sys_info.lac_valid == TRUE )
        {
            ...
            strlcpy(cri_nas_rte_gsm_reg_info[CRI_NAS_RTE_SERVICE_DATA]->lac,lac,sizeof(cri_nas_rte_gsm_reg_info[CRI_NAS_RTE_SERVICE_DATA]->lac));
        }
        ...
        if ( cri_nas_core_sys_info->gsm_sys_info.threegpp_specific_sys_info.cell_id_valid == TRUE )
        {
            ...
            strlcpy(cri_nas_rte_gsm_reg_info[CRI_NAS_RTE_SERVICE_DATA]->cid,cid,sizeof(cri_nas_rte_gsm_reg_info[CRI_NAS_RTE_SERVICE_DATA]->cid));
        }
    }
    ...
    if ( cri_nas_core_rte_info.cri_nas_voice_rte == CRI_NAS_RTE_WCDMA )
    {
        if ( cri_nas_core_sys_info->wcdma_sys_info.threegpp_specific_sys_info.lac_valid == TRUE )
        {
            ...
            strlcpy(cri_nas_rte_wcdma_reg_info[CRI_NAS_RTE_SERVICE_DATA]->lac,lac,sizeof(cri_nas_rte_wcdma_reg_info[CRI_NAS_RTE_SERVICE_DATA]->lac));
        }
        ...
        if ( cri_nas_core_sys_info->wcdma_sys_info.threegpp_specific_sys_info.cell_id_valid == TRUE )
        {
            ...
            strlcpy(cri_nas_rte_wcdma_reg_info[CRI_NAS_RTE_SERVICE_DATA]->cid,cid,sizeof(cri_nas_rte_wcdma_reg_info[CRI_NAS_RTE_SERVICE_DATA]->cid));
        }
        if ( cri_nas_core_sys_info->wcdma_sys_info.wcdma_specific_sys_info.psc_valid == TRUE )
        {
            snprintf(psc,sizeof(psc),
            ...
            strlcpy(cri_nas_rte_wcdma_reg_info[CRI_NAS_RTE_SERVICE_DATA]->psc,psc,sizeof(cri_nas_rte_wcdma_reg_info[CRI_NAS_RTE_SERVICE_DATA]->psc));
        }
    }
    ...
    if ( cri_nas_core_rte_info.cri_nas_voice_rte == CRI_NAS_RTE_CDMA )
    {
        if ( cri_nas_core_sys_info->cdma_sys_info.cdma_specific_sys_info.cdma_sys_id_valid == TRUE )
        {
            ...
            strlcpy(cri_nas_rte_cdma_reg_info[CRI_NAS_RTE_SERVICE_DATA]->sid,sid,sizeof(cri_nas_rte_cdma_reg_info[CRI_NAS_RTE_SERVICE_DATA]->sid));
        }
        if ( cri_nas_core_sys_info->cdma_sys_info.cdma_specific_sys_info.network_id_valid == TRUE )
        {
            snprintf(nid,sizeof(nid),
            ...
            strlcpy(cri_nas_rte_cdma_reg_info[CRI_NAS_RTE_SERVICE_DATA]->nid, nid,sizeof(cri_nas_rte_cdma_reg_info[CRI_NAS_RTE_SERVICE_DATA]->nid));
        }
    }
    ...
    if ( cri_nas_core_rte_info.cri_nas_voice_rte == CRI_NAS_RTE_LTE)
    {
        if ( cri_nas_core_sys_info->lte_sys_info.threegpp_specific_sys_info.lac_valid == TRUE )
        {
            ...
            strlcpy(cri_nas_rte_lte_reg_info[CRI_NAS_RTE_SERVICE_DATA]->lac,lac,sizeof(cri_nas_rte_lte_reg_info[CRI_NAS_RTE_SERVICE_DATA]->lac));
        }
        ...
        if ( cri_nas_core_sys_info->lte_sys_info.threegpp_specific_sys_info.cell_id_valid == TRUE )
        {
            ...
            strlcpy(cri_nas_rte_lte_reg_info[CRI_NAS_RTE_SERVICE_DATA]->cid,cid,sizeof(cri_nas_rte_lte_reg_info[CRI_NAS_RTE_SERVICE_DATA]->cid));
        }
        if ( cri_nas_core_sys_info->lte_sys_info.lte_specific_sys_info.tac_valid == TRUE )
        {
            snprintf(tac,sizeof(tac),
            ...
            strlcpy(cri_nas_rte_lte_reg_info[CRI_NAS_RTE_SERVICE_DATA]->tac,tac,sizeof(cri_nas_rte_lte_reg_info[CRI_NAS_RTE_SERVICE_DATA]->tac));
        }
    }
...
cri_nas_tech_reg_status_type cri_nas_retrieve_reg_state(uint8_t is_voice)
{
    ...
    // If there is no registration info, consider RTE as unknown.
    if ( reg_info == NULL )
    {
        rte = CRI_NAS_RTE_UNKNOWN;
    }
    ...
    switch(rte)
    {
        ...
        default:
            reg_status = CRI_NAS_TECH_REG_STATUS_UNKNOWN;
            break;
    }
...
cri_nas_tech_reg_status_type cri_nas_is_considered_registered(void)
{
...
cri_core_error_type cri_nas_set_pref_mode_request_handler( cri_core_context_type cri_core_context,
                                                            uint8_t pref_mode_valid,
                                                            uint32_t pref_mode,
                                                            uint8_t roaming_pref_valid,
                                                            ...
                                                            uint32_t *is_changed)
{
    ...
    if ( pref_mode_valid )
    {
        is_auto = cri_nas_is_auto_mode_enabled(pref_mode);
        qmi_pref_mode = cri_nas_get_qmi_pref_mode(pref_mode);
        ret_val = cri_nas_get_mode_preferrence(&curr_pref_mode);

        if ( ret_val == QMI_ERR_NONE_V01 &&
    ...
    // if mode_pref/roam_pref valid send request to modem
    if ( nas_set_pref_mode_req_msg.mode_pref_valid ||
         nas_set_pref_mode_req_msg.roam_pref_valid )
    {
        ...
        if ( rule_data != NULL )
        {
            if ( pref_mode_valid )
            {
                rule_data->pref_mode_valid = TRUE;
                rule_data->pref_mode = qmi_pref_mode;
            }
            if ( roaming_pref_valid )
            {
                rule_data->roaming_pref_valid = TRUE;
                rule_data->roaming_pref = roaming_pref;
            }
        }
...
cri_core_error_type cri_nas_get_current_mcc_mnc(char *mcc, char *mnc)
{
    ...
    switch(rat)
    {
        case CRI_NAS_RTE_CDMA:
            cdma_reg_info = (cri_nas_rte_cdma_reg_info_type*)reg_info;
            strlcpy(mcc, cdma_reg_info->reg_info.mcc,CRI_NAS_MCC_MNC_MAX+1);
            strlcpy(mnc, cdma_reg_info->reg_info.mnc,CRI_NAS_MCC_MNC_MAX+1);
            break;

        case CRI_NAS_RTE_HDR:
            hdr_reg_info = (cri_nas_rte_hdr_reg_info_type*)reg_info;
            strlcpy(mcc, hdr_reg_info->reg_info.mcc,CRI_NAS_MCC_MNC_MAX+1);
            strlcpy(mnc, hdr_reg_info->reg_info.mnc,CRI_NAS_MCC_MNC_MAX+1);
            break;

        case CRI_NAS_RTE_GSM:
            gsm_reg_info = (cri_nas_rte_gsm_reg_info_type*)reg_info;
            strlcpy(mcc, gsm_reg_info->reg_info.mcc,CRI_NAS_MCC_MNC_MAX+1);
            strlcpy(mnc, gsm_reg_info->reg_info.mnc,CRI_NAS_MCC_MNC_MAX+1);
            break;

        case CRI_NAS_RTE_WCDMA:
            wcdma_reg_info = (cri_nas_rte_wcdma_reg_info_type*)reg_info;
            strlcpy(mcc, wcdma_reg_info->reg_info.mcc,CRI_NAS_MCC_MNC_MAX+1);
            strlcpy(mnc, wcdma_reg_info->reg_info.mnc,CRI_NAS_MCC_MNC_MAX+1);
            break;

        case CRI_NAS_RTE_TDSCDMA:
            tdscdma_reg_info = (cri_nas_rte_tdscdma_reg_info_type*)reg_info;
            strlcpy(mcc, tdscdma_reg_info->reg_info.mcc,CRI_NAS_MCC_MNC_MAX+1);
            strlcpy(mnc, tdscdma_reg_info->reg_info.mnc,CRI_NAS_MCC_MNC_MAX+1);
            break;

        case CRI_NAS_RTE_LTE:
            lte_reg_info = (cri_nas_rte_lte_reg_info_type*)reg_info;
            strlcpy(mcc, lte_reg_info->reg_info.mcc,CRI_NAS_MCC_MNC_MAX+1);
            strlcpy(mnc, lte_reg_info->reg_info.mnc,CRI_NAS_MCC_MNC_MAX+1);
            break;
...
)
{
    ...
    UTIL_LOG_MSG("qmi_client_error: %d",(int)qmi_client_error );
            ...
            else
            {
                ...
                else
                {
                    cri_nas_decode_operator_name_in_little_endian(short_eons,
                                                                ...
                                                                get_plmn_resp.eons_plmn_name_3gpp.plmn_short_name_len);
                    UTIL_LOG_MSG("short eons derived from plmn %s",short_eons);
                }
                    ...
                    else
                    {
                        cri_nas_decode_operator_name_in_little_endian(long_eons,
                                                                    ...
                                                                    get_plmn_resp.eons_plmn_name_3gpp.plmn_long_name_len);
                        UTIL_LOG_MSG("long eons derived from plmn %s",long_eons);
                    }
                }
            }
        }
...
void* cri_nas_core_nw_scan_helper(void *nw_scan_resp_msg)
{
    ...
    if ( qmi_response )
    {
        if ( qmi_response->nas_3gpp_network_info_valid )
        {
...
//                    rat = qmi_response->nas_network_radio_access_technology[nw_cnt].rat;
                ...
                UTIL_LOG_MSG("Network #%d - %s/%s/%d/%d",
                                iter_i+1,
                                long_eons,
                                short_eons,
                                network_status,
                ...
                if ( long_eons && short_eons )
                {
                    strlcpy(nw_scan_result->long_eons[iter_i],long_eons,CRI_NAS_OPERATOR_MAX_EONS_LEN);
                    strlcpy(nw_scan_result->short_eons[iter_i],short_eons,CRI_NAS_OPERATOR_MAX_EONS_LEN);
                }
                strlcpy(nw_scan_result->mcc[iter_i],qmi_nw_info_mcc_str,CRI_NAS_MCC_MNC_MAX+1);
                strlcpy(nw_scan_result->mnc[iter_i],qmi_nw_info_mnc_str,CRI_NAS_MCC_MNC_MAX+1);
                                            ...
                                            uint8_t is_nw_sel_in_progress)
{
    ...
    // Due to issue in Nikel Modem PL, delta for lte_snr is not working.
    // Irrespective of Delta value we are getting SIG_INFO_IND. Triton, Dime don't have this issue
    // Treshold list works fine. So only for LTE SNR we are using threshold list.
    sig_info2_req_msg.lte_snr_threshold_list_valid = TRUE;
    sig_info2_req_msg.lte_snr_threshold_list_len = CRI_NAS_SNR_THRESHOLD_ENTRIES_NUM;
    for( iter_i = 0; iter_i < CRI_NAS_SNR_THRESHOLD_ENTRIES_NEG_NUM ; iter_i++ )
    {
        sig_info2_req_msg.lte_snr_threshold_list[iter_i] = CRI_NAS_SNR_THRESHOLD_ENTRIES_NEG_START_VALUE  + iter_i * CRI_NAS_SNR_THRESHOLD_ENTRIES_NEG_INC_VALUE;
    }
    for( iter_i = CRI_NAS_SNR_THRESHOLD_ENTRIES_NEG_NUM ; iter_i < CRI_NAS_SNR_THRESHOLD_ENTRIES_NUM; iter_i++ )
    {
        sig_info2_req_msg.lte_snr_threshold_list[iter_i] = CRI_NAS_SNR_THRESHOLD_ENTRIES_POS_START_VALUE  + (iter_i - CRI_NAS_SNR_THRESHOLD_ENTRIES_NEG_NUM) * CRI_NAS_SNR_THRESHOLD_ENTRIES_POS_INC_VALUE ;
    }
    ...
    sig_info2_req_msg.wcdma_rssi_delta = CRI_NAS_RSSI_DELTA;
    ...
    if ( cri_error == CRI_ERR_NONE_V01 )
    {
        ...
        nwreg_indications_qmi_request.reg_lte_sib16_network_time       = always_enable_action; // only when online?
...
cri_core_error_type cri_nas_network_selection_handler(cri_core_context_type cri_core_context, const cri_nas_nw_selection_request_type *selection_info, const void *user_data, hlos_resp_cb_type hlos_resp_cb)
{
    uint32_t mcc;
    uint32_t mnc;
    uint32_t rat;
    ...
    if ( cri_nas_rules_generic_rule_data != NULL )
    {
        cri_nas_rules_generic_rule_data->dummy_var = 0x00FF;
    }
    else
    {
        return CRI_ERR_NO_MEMORY_V01;
    }
...
uint8 cri_nas_core_sib16_acquired_status()
{
    ...
    if(cri_nas_nw_lte_sib16_network_time_info)
    {
        sib16_acquired = cri_nas_nw_lte_sib16_network_time_info->lte_sib16_acquired_valid?
                            cri_nas_nw_lte_sib16_network_time_info->lte_sib16_acquired:FALSE;
    }

    return sib16_acquired;
}

qmi_error_type_v01 cri_nas_core_fetch_update_nitz_time_info()
{
    qmi_error_type_v01 err_code;
    nas_get_network_time_resp_msg_v01 *nas_get_network_time_resp_msg;

    err_code = QMI_ERR_INTERNAL_V01;
    nas_get_network_time_resp_msg = NULL;

    nas_get_network_time_resp_msg = util_memory_alloc(sizeof(*nas_get_network_time_resp_msg));

    if(nas_get_network_time_resp_msg)
    {
        err_code = cri_core_qmi_send_msg_sync(cri_nas_core_retrieve_client_id(),
                                              QMI_NAS_GET_NETWORK_TIME_REQ_MSG_V01,
                                              NULL,
                                              NIL,
                                              nas_get_network_time_resp_msg,
                                              sizeof(*nas_get_network_time_resp_msg),
                                              CRI_CORE_MINIMAL_TIMEOUT);

        if(QMI_ERR_NONE_V01 == cri_core_retrieve_err_code(err_code,
                                                          &nas_get_network_time_resp_msg->resp))
        {
            if(cri_nas_nw_network_time_info)
            {
                memset(cri_nas_nw_network_time_info,0,sizeof(*cri_nas_nw_network_time_info));
                if(nas_get_network_time_resp_msg->nas_3gpp_time_valid)
                {
                    CRI_STORE_MAND_TLV(cri_nas_nw_network_time_info->universal_time,
                               nas_get_network_time_resp_msg->nas_3gpp_time.universal_time);
                    cri_nas_nw_network_time_info->time_zone_valid = TRUE;
                    CRI_STORE_MAND_TLV(cri_nas_nw_network_time_info->time_zone,
                               nas_get_network_time_resp_msg->nas_3gpp_time.time_zone);
                    cri_nas_nw_network_time_info->daylt_sav_adj_valid = TRUE;
                    CRI_STORE_MAND_TLV(cri_nas_nw_network_time_info->daylt_sav_adj,
                               nas_get_network_time_resp_msg->nas_3gpp_time.daylt_sav_adj);
                    cri_nas_nw_network_time_info->radio_if_valid = TRUE;
                    CRI_STORE_MAND_TLV(cri_nas_nw_network_time_info->radio_if,
                               nas_get_network_time_resp_msg->nas_3gpp_time.radio_if);
                }

                if(nas_get_network_time_resp_msg->nas_3gpp2_time_valid)
                {
                    CRI_STORE_MAND_TLV(cri_nas_nw_network_time_info->universal_time,
                               nas_get_network_time_resp_msg->nas_3gpp2_time.universal_time);
                    cri_nas_nw_network_time_info->time_zone_valid = TRUE;
                    CRI_STORE_MAND_TLV(cri_nas_nw_network_time_info->time_zone,
                               nas_get_network_time_resp_msg->nas_3gpp2_time.time_zone);
                    cri_nas_nw_network_time_info->daylt_sav_adj_valid = TRUE;
                    CRI_STORE_MAND_TLV(cri_nas_nw_network_time_info->daylt_sav_adj,
                               nas_get_network_time_resp_msg->nas_3gpp2_time.daylt_sav_adj);
                    cri_nas_nw_network_time_info->radio_if_valid = TRUE;
                    CRI_STORE_MAND_TLV(cri_nas_nw_network_time_info->radio_if,
                               nas_get_network_time_resp_msg->nas_3gpp2_time.radio_if);
                }
            }
            else
            {
        ...
        util_memory_free((void**)&nas_get_network_time_resp_msg);
    }
...
{
    qmi_error_type_v01 err_code;
    nas_get_lte_sib16_network_time_resp_msg_v01 *nas_get_lte_sib16_network_time_resp_msg;

    err_code = QMI_ERR_INTERNAL_V01;
    nas_get_lte_sib16_network_time_resp_msg = NULL;

    nas_get_lte_sib16_network_time_resp_msg = util_memory_alloc(sizeof(*nas_get_lte_sib16_network_time_resp_msg));

    if(nas_get_lte_sib16_network_time_resp_msg)
    {
        err_code = cri_core_qmi_send_msg_sync(cri_nas_core_retrieve_client_id(),
                                              QMI_NAS_GET_LTE_SIB16_NETWORK_TIME_REQ_MSG_V01,
                                              NULL,
                                              NIL,
                                              nas_get_lte_sib16_network_time_resp_msg,
                                              sizeof(*nas_get_lte_sib16_network_time_resp_msg),
                                              CRI_CORE_MINIMAL_TIMEOUT);

        if(QMI_ERR_NONE_V01 == cri_core_retrieve_err_code(err_code,
                                                          &nas_get_lte_sib16_network_time_resp_msg->resp))
        {
            if(cri_nas_nw_lte_sib16_network_time_info)
            {
                memset(cri_nas_nw_lte_sib16_network_time_info,0,sizeof(*cri_nas_nw_lte_sib16_network_time_info));
                CRI_STORE_OPT_TLV(cri_nas_nw_lte_sib16_network_time_info->lte_sib16_acquired,
                              nas_get_lte_sib16_network_time_resp_msg->lte_sib16_acquired);
                CRI_STORE_OPT_TLV(cri_nas_nw_lte_sib16_network_time_info->universal_time,
                              nas_get_lte_sib16_network_time_resp_msg->universal_time);
                CRI_STORE_OPT_TLV(cri_nas_nw_lte_sib16_network_time_info->abs_time,
                              nas_get_lte_sib16_network_time_resp_msg->abs_time);
                CRI_STORE_OPT_TLV(cri_nas_nw_lte_sib16_network_time_info->leap_sec,
                              nas_get_lte_sib16_network_time_resp_msg->leap_sec);
                CRI_STORE_OPT_TLV(cri_nas_nw_lte_sib16_network_time_info->time_zone,
                              nas_get_lte_sib16_network_time_resp_msg->time_zone);
                CRI_STORE_OPT_TLV(cri_nas_nw_lte_sib16_network_time_info->daylt_sav_adj,
                              nas_get_lte_sib16_network_time_resp_msg->daylt_sav_adj);
            }
            else
            {
                err_code = QMI_ERR_INTERNAL_V01;
        ...
        util_memory_free((void**)&nas_get_lte_sib16_network_time_resp_msg);
    }
...
void cri_nas_core_fill_nitz_time_resp(uint8_t *nitz_time_valid, char *nitz_time, uint8_t *abs_time_valid, uint64_t *abs_time, uint8_t *leap_sec_valid, int8_t *leap_sec)
{
    ...
    do
    {
        ...
        if(cri_nas_nw_lte_sib16_network_time_info && (cri_nas_nw_lte_sib16_network_time_info->universal_time_valid || cri_nas_nw_lte_sib16_network_time_info->abs_time_valid) )
        {
            UTIL_LOG_MSG("\nLTE_SIB16 info available\n");
            if(cri_nas_nw_lte_sib16_network_time_info->universal_time_valid)
            {
                UTIL_LOG_MSG("\nLTE_SIB16 Universal time available\n");
                if(cri_nas_nw_lte_sib16_network_time_info->time_zone_valid)
                {
                    time_zone = cri_nas_nw_lte_sib16_network_time_info->time_zone;
                    if (0 > time_zone)
                    {
                        time_zone *= -1;
                        time_zone_west = TRUE;
                ...
                if (cri_nas_nw_lte_sib16_network_time_info->daylt_sav_adj_valid)
                {
                    daylight=cri_nas_nw_lte_sib16_network_time_info->daylt_sav_adj;
                }

                *nitz_time_valid = TRUE;
                snprintf( nitz_time, CRI_NAS_NITZ_STR_BUF_MAX, "%02d/%02d/%02d,%02d:%02d:%02d%c%02d,%02d",
                        (int) cri_nas_nw_lte_sib16_network_time_info->universal_time.year%100,
                        (int) cri_nas_nw_lte_sib16_network_time_info->universal_time.month,
                        (int) cri_nas_nw_lte_sib16_network_time_info->universal_time.day,
                        (int) cri_nas_nw_lte_sib16_network_time_info->universal_time.hour,
                        (int) cri_nas_nw_lte_sib16_network_time_info->universal_time.minute,
                        (int) cri_nas_nw_lte_sib16_network_time_info->universal_time.second,
                        time_zone_west ? '-' : '+',
                        (int) time_zone,
                        (int) daylight);
            }

            if(cri_nas_nw_lte_sib16_network_time_info->abs_time_valid)
            {
                UTIL_LOG_MSG("\nLTE_SIB16 Absolute time available\n");
                *abs_time_valid = TRUE;
                *abs_time = cri_nas_nw_lte_sib16_network_time_info->abs_time;
            }

            if(cri_nas_nw_lte_sib16_network_time_info->leap_sec_valid)
            {
                UTIL_LOG_MSG("\nLTE_SIB16 Leap sec available\n");
                *leap_sec_valid = TRUE;
                *leap_sec = cri_nas_nw_lte_sib16_network_time_info->leap_sec;
            }
        }
        else if(!cri_nas_sib16_acquired_status() && cri_nas_nw_network_time_info)
        {
            UTIL_LOG_MSG("\n Network Time info available\n");
            if(cri_nas_nw_network_time_info->time_zone_valid)
            {
                time_zone = cri_nas_nw_network_time_info->time_zone;
                if (0 > time_zone)
                {
                    time_zone *= -1;
                    time_zone_west = TRUE;
            ...
            if (cri_nas_nw_network_time_info->daylt_sav_adj_valid)
            {
                daylight=cri_nas_nw_network_time_info->daylt_sav_adj;
            }

            *nitz_time_valid = TRUE;
            snprintf( nitz_time, CRI_NAS_NITZ_STR_BUF_MAX, "%02d/%02d/%02d,%02d:%02d:%02d%c%02d,%02d",
                        (int) cri_nas_nw_network_time_info->universal_time.year%100,
                        (int) cri_nas_nw_network_time_info->universal_time.month,
                        (int) cri_nas_nw_network_time_info->universal_time.day,
                        (int) cri_nas_nw_network_time_info->universal_time.hour,
                        (int) cri_nas_nw_network_time_info->universal_time.minute,
                        (int) cri_nas_nw_network_time_info->universal_time.second,
                        time_zone_west ? '-' : '+',
                        (int) time_zone,
                        (int) daylight);
        }
...
uint8 cri_nas_core_fill_network_time_ind(uint8_t *nitz_time_valid, char *nitz_time, uint8_t *abs_time_valid, uint64_t *abs_time, uint8_t *leap_sec_valid, int8_t *leap_sec)
{
    ...
    do
    {
        ...
        sib16_acquired = cri_nas_sib16_acquired_status();
        if(!sib16_acquired)
        {
            if(cri_nas_nw_network_time_info->time_zone_valid)
            {
