...
)
{
    ...
    uint8_t is_e911_call_active = FALSE;
    uint8_t is_e911_call_ended = FALSE;
    cri_voice_e911_state_t_v01 e911_state_init;
    ...
    if( NULL != call_status_ind_ptr && NULL != call_list )
    {
        ...
        for ( i = 0; i < call_status_ind_ptr->call_info_len; i++ )
        {
            ...
            switch ( iter_qmi_call_info->call_state )
            {
                ...
                case CALL_STATE_END_V02:
                    ...
                    if( (CALL_TYPE_EMERGENCY_V02 ==iter_qmi_call_info->call_type) ||
                        (CALL_TYPE_ECALL_V02 ==iter_qmi_call_info->call_type)
                      )
                    {
                        is_e911_call_ended = TRUE;
                    }
                ...
                case CALL_STATE_ORIGINATING_V02:
                    ...
                    {
                        is_e911_call_active = TRUE;
                        e911_state_init = CRI_VOICE_E911_ACTIVE_V01;
                    }
        ...
        if(hlos_ind_cb_func_ptr)
        {
            ...
            QCRIL_LOG_INFO("is_e911_call_active %d", is_e911_call_active);
            if(is_e911_call_active)
            {
                hlos_ind_cb_func_ptr(
                    CRI_VOICE_E911_STATE_IND,
                    (void*)&e911_state_init,
                    sizeof(e911_state_init));
            }

            QCRIL_LOG_INFO("is_e911_call_ended %d", is_e911_call_ended);
            if(is_e911_call_ended)
            {
...
)
{
    ...
    if (info_rec_ind_ptr)
    {
        if (CALL_WAITING_NEW_CALL_V02 != info_rec_ind_ptr->call_waiting)
        {
            ...
            if (call_obj_ptr == NULL && 254 != info_rec_ind_ptr->call_id)
            {
...
void cri_voice_ind_hdlr_dtmf_ind(
    int qmi_service_client_id,
    const voice_dtmf_ind_msg_v02  *dtmf_ind_ptr
)
{
    hlos_ind_cb_type hlos_ind_cb_func_ptr = cri_core_retrieve_hlos_ind_cb(qmi_service_client_id);

    cri_dtmf_ind_type cri_dtmf_ind;

    if (NULL != dtmf_ind_ptr) {

        cri_dtmf_ind.dtmf_info.call_id          = dtmf_ind_ptr->dtmf_info.call_id;
        cri_dtmf_ind.dtmf_info.digit_buffer_len = dtmf_ind_ptr->dtmf_info.digit_buffer_len;
        memcpy(cri_dtmf_ind.dtmf_info.digit_buffer, dtmf_ind_ptr->dtmf_info.digit_buffer, dtmf_ind_ptr->dtmf_info.digit_buffer_len);
