...
void* core_handler_thread_proc(void* core_handler_thread_proc_param)
{
    ...
    core_flow_control_req_decision  flow_control_decision;
    ...
    while(1)
    {
        ...
        while(NULL == core_handler_data)
        {
            ...
            else
            {
                ...
                {
                    flow_control_decision = core_flow_control_check_event(core_handler_data);
                    switch (flow_control_decision)
                    {
                        case CORE_FLOW_CONTROL_REQ_DEC_HOLD:
                            /* do not process the message for now */
                            core_handler_data->is_flow_control_approved = FALSE;
                            break;

                        case CORE_FLOW_CONTROL_REQ_DEC_PROCESS:
                            /* continue to process the message. */
                            core_handler_data->is_flow_control_approved = TRUE;
                            core_handler_data->is_processed = TRUE;
                            break;

                        default:
                            /* delete message from core queue */
                            // TODO: We shouldn't simply delete event, should send response to client as well.
                            core_handler_remove_event(core_handler_data);
                            core_handler_data = NULL;
                            break;
                    }
                    if ( core_handler_data && core_handler_data->is_processed == FALSE )
                    {
                        // should continue with next request if the request is not processed.
                        core_handler_data = NULL;
                        continue;
                    }
                }
        ...
        if(core_handler_data)
        {
            ...
            if(event_handler)
            {
                UTIL_LOG_MSG("Event Handler: %x", event_handler);
                UTIL_LOG_MSG("core_handler_data->event_data: %x", core_handler_data->event_data);
...
int core_queue_find_for_processing_evaluator(util_list_node_data_type *to_be_found_data)
{
    ...
    if(to_be_found_data && to_be_found_data->user_data)
    {
        ...
        if(FALSE == core_handler_data->is_processed && TRUE == core_handler_data->is_flow_control_approved)
        {
            ret = TRUE;
        }
    }
                           ...
                           void *event_data)
{
    ...
    if(core_handler_data)
    {
        // Mark request as flow control approved while adding to core queue.
        // while processing, flow control handlers will check if the request is eligible
        // for processing this request or not and based on flow control criteria.
