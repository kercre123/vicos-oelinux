...
static int qti_enable_mobile_ap()
{
  ...
  snprintf(command, QTI_MAX_COMMAND_STR_LEN,"echo QTI:Enable mobileap\
           > /dev/kmsg");
...
static int qti_disable_mobile_ap()
{
  ...
  snprintf(command, QTI_MAX_COMMAND_STR_LEN, "echo QTI:Disable \
           Mobileap > /dev/kmsg");
  ds_system_call(command, strlen(command));
/*---------------------------------------------------------------------------
...
FUNCTION QTI_TETHERED_LINK_UP()

DESCRIPTION

  This function sends a message to QCMAP setup the TETHERED link for RNDIS/ECM
  tethering
...
==========================================================================*/
static int qti_tethered_link_up
(
  qti_interface_e interface
)
{
  qcmap_msgr_usb_link_up_req_msg_v01   qcmap_qti_tethered_link_up_req_msg;
  qcmap_msgr_usb_link_up_resp_msg_v01  qcmap_qti_tethered_link_up_resp_msg;
  qmi_client_error_type                qmi_error;
  char                                 command[QTI_MAX_COMMAND_STR_LEN];
/*--------------------------------------------------------------------------*/

  LOG_MSG_INFO1("Setup TETHERED link start", 0, 0, 0);
  snprintf(command, QTI_MAX_COMMAND_STR_LEN, "echo QTI:Setup TETHERED link \
           > /dev/kmsg");
  ds_system_call(command, strlen(command));
  memset(&qcmap_qti_tethered_link_up_req_msg,
         0,
         sizeof(qcmap_msgr_usb_link_up_req_msg_v01));

  memset(&qcmap_qti_tethered_link_up_resp_msg,
         0,
         sizeof(qcmap_msgr_usb_link_up_resp_msg_v01));

/*-------------------------------------------------------------------------
  Setup TETHERED link - RNDIS/ECM/ETH
---------------------------------------------------------------------------*/
  qcmap_qti_tethered_link_up_req_msg.mobile_ap_handle =
                                         qti_qcmap_conf->qti_mobile_ap_handle;


  qcmap_qti_tethered_link_up_req_msg.usb_link = interface;

  qmi_error = qmi_client_send_msg_sync(qti_qcmap_conf->qti_qcmap_msgr_handle,
                                       QMI_QCMAP_MSGR_USB_LINK_UP_REQ_V01,
                                       &qcmap_qti_tethered_link_up_req_msg,
                                       sizeof(qcmap_msgr_usb_link_up_req_msg_v01),
                                       &qcmap_qti_tethered_link_up_resp_msg,
                                       sizeof(qcmap_msgr_usb_link_up_resp_msg_v01),
                                       QTI_QMI_MSG_TIMEOUT_VALUE);

  if (qmi_error == QMI_TIMEOUT_ERR || qmi_error != QMI_NO_ERR ||
      qcmap_qti_tethered_link_up_resp_msg.resp.result != QMI_NO_ERR)
  {
    LOG_MSG_ERROR("Setup TETHERED link failed. Error values: %d , %d",
        qmi_error, qcmap_qti_tethered_link_up_resp_msg.resp.error,0);
    return QTI_FAILURE;
  }

  LOG_MSG_INFO1("Setup TETHERED link succeeded", 0, 0, 0);
...
FUNCTION QTI_TETHERED_LINK_DOWN()

DESCRIPTION

  This function sends a message to QCMAP to bring down the RNDIS/ECM TETHERED link
...
==========================================================================*/
static int qti_tethered_link_down
(
  qti_interface_e interface
)
{
  qcmap_msgr_usb_link_down_req_msg_v01   qcmap_qti_tethered_link_down_req_msg;
  qcmap_msgr_usb_link_down_resp_msg_v01  qcmap_qti_tethered_link_down_resp_msg;
  qmi_client_error_type                  qmi_error;
  char                                   command[QTI_MAX_COMMAND_STR_LEN];
/*--------------------------------------------------------------------------*/

  LOG_MSG_INFO1("Tear down TETHERED link start", 0, 0, 0);
  snprintf(command, QTI_MAX_COMMAND_STR_LEN, "echo QTI:TETHERED link down \
           > /dev/kmsg");
  ds_system_call(command, strlen(command));
  memset(&qcmap_qti_tethered_link_down_req_msg,
         0,
         sizeof(qcmap_msgr_usb_link_down_req_msg_v01));

  memset(&qcmap_qti_tethered_link_down_resp_msg,
         0,
...
/*-------------------------------------------------------------------------
  Setup TETHERED link - RNDIS/ECM/ETH
---------------------------------------------------------------------------*/
  qcmap_qti_tethered_link_down_req_msg.mobile_ap_handle =
                                         qti_qcmap_conf->qti_mobile_ap_handle;

  qcmap_qti_tethered_link_down_req_msg.usb_link = interface;

  qmi_error = qmi_client_send_msg_sync(qti_qcmap_conf->qti_qcmap_msgr_handle,
                                       QMI_QCMAP_MSGR_USB_LINK_DOWN_REQ_V01,
                                       &qcmap_qti_tethered_link_down_req_msg,
                                       sizeof(qcmap_msgr_usb_link_down_req_msg_v01),
                                       &qcmap_qti_tethered_link_down_resp_msg,
                                       sizeof(qcmap_msgr_usb_link_down_resp_msg_v01),
                                       QTI_QMI_MSG_TIMEOUT_VALUE);

  if (qmi_error == QMI_TIMEOUT_ERR || qmi_error != QMI_NO_ERR ||
      qcmap_qti_tethered_link_down_resp_msg.resp.result != QMI_NO_ERR)
  {
    LOG_MSG_ERROR("Bring down TETHERED link failed. Error values: %d , %d",
        qmi_error, qcmap_qti_tethered_link_down_resp_msg.resp.error,0);
    return QTI_FAILURE;
  }

  LOG_MSG_INFO1("Bring down TETHERED link succeeded", 0, 0, 0);
...
---------------------------------------------------------------------------*/
      ds_system_call("echo QTI:Processing LINK_UP > /dev/kmsg",
                     strlen("echo QTI:Processing LINK_UP > /dev/kmsg"));

      LOG_MSG_INFO1("Got TETHERED link up event on interface %d",interface, 0, 0);

/*--------------------------------------------------------------------------
  Enable QC Mobile AP when the first link gets enabled.
---------------------------------------------------------------------------*/
      if (qti_qcmap_conf->qcmap_link_ref_cnt == 0)
      {
...
---------------------------------------------------------------------------*/
      ret_val = qti_tethered_link_up(interface);
      if (ret_val==QTI_SUCCESS)
      {
...
/*--------------------------------------------------------------------------
  Increment the QCMAP TETHERED link count.
---------------------------------------------------------------------------*/
      qti_qcmap_conf->qcmap_link_ref_cnt++;

      LOG_MSG_INFO1("QCMAP TETHERED link count %d",
                    qti_qcmap_conf->qcmap_link_ref_cnt, 0, 0);

      ds_system_call("echo QTI:LINK_UP Processed > /dev/kmsg",
                     strlen("echo QTI:LINK_UP Processed > /dev/kmsg"));
...
/*--------------------------------------------------------------------------
  ...
  But we need to make sure we dont write too many messages to dmesg.
---------------------------------------------------------------------------*/
      ds_system_call("echo QTI:Processing LINK_DOWN > /dev/kmsg",
                     strlen("echo QTI:Processing LINK_DOWN > /dev/kmsg"));

      LOG_MSG_INFO1(" Got TETHERED link down event", 0, 0, 0);
/*--------------------------------------------------------------------------
  Bring down RNDIS/ECM tethering
---------------------------------------------------------------------------*/
      ret_val = qti_tethered_link_down(interface);
      if (ret_val==QTI_SUCCESS)
      {
      ...
      LOG_MSG_INFO1("QCMAP USB link count %d",
                    qti_qcmap_conf->qcmap_link_ref_cnt, 0, 0);
/*--------------------------------------------------------------------------
  Disable QC Mobile AP
---------------------------------------------------------------------------*/
      ...
      ds_system_call("echo QTI:LINK_DOWN Processed > /dev/kmsg",
                     strlen("echo QTI:LINK_DOWN Processed > /dev/kmsg"));
...
/*-------------------------------------------------------------------------
  ...
  since QCMAP needs to handles the tethered link up and down messages generated.
