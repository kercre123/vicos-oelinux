...
tftp_server_create_info_file (void)
{
  ...
  snprintf (info_file, sizeof (info_file), "%s%c%s",
            path, OS_PATH_SEPARATOR, TFTP_SERVER_INFO_FILEPATH);
...
tftp_get_real_filepath(struct tftp_server_client_info_type *client_info)
{
  ...
  pkt_filename_len = tftp_strlen (pkt_filename);
                         ...
                         enum tftp_server_instance_id_type instance)
{
  ...
  {
    TFTP_LOG_ERR ("rcvd invalid request [%d] [%d] [%d]",
                  instance, rx_data_size, rx_pkt->opcode);
    return;
  }
  ...
  TFTP_LOG_INFO ("rcvd request [%d] [%d] [%d] [%d] [%d]",
    instance, rx_data_size, rx_pkt->opcode, proc_id, port_id);
...
tftp_server (int argc, const char *argv[])
{
  ...
  /* Do this before we drop privilege */
  tftp_folders_upgrade_uid();
  ...
  while (!server_info->stop_server)
  {
    ...
    poll_res = tftp_socket_poll (pollfd_array + 1,
                                 TFTP_SERVER_INSTANCE_ID_MAX - 1, -1);
...
static int32
tftp_server_attempt_autodir(const char *fname, uint16_t mode,
                            int32_t uid, int32_t gid)
{
  int32_t ret;
  ret = tftp_os_auto_mkdir (fname, mode, uid, gid);
...
static int32
tftp_server_file_open (struct tftp_server_client_info_type *client_info)
{
  ...
  handle = &client_info->file_handle;
  path = file_path;
  ...
  else if (proto->type == TFTP_REQUEST_TYPE_WRQ)
  {
    ...
    else
    {
      ...
      TFTP_LOG_INFO ("Using temp file[%s] for atomic put", temp_file_path);
    }
  }
  ...
  is_ramdump_file = tftp_server_folders_check_if_ramdump_file (path);
  if (is_ramdump_file == 1)
  {
    file_mode_bits = TFTP_SHARED_FILE_MODE;
    dir_mode_bits  = TFTP_SHARED_DIR_MODE;
    gid = TFTP_RAMDUMP_GID;
    uid = TFTP_RAMDUMP_UID;
  }

  open_res = tftp_os_file_open_helper (path, oflags, file_mode_bits, uid, gid,
                                       handle, file_mode);
  if (open_res < 0)
  {
    if (proto->type == TFTP_REQUEST_TYPE_RRQ)
    ...
    else
    {
      if (open_res != -ENOENT)
      {
        TFTP_LOG_INFO ("open failed2: [%d] [%s] [%s]",
                       open_res, strerror(errno), path);
        return open_res;
      }
      mkdir_res = tftp_server_attempt_autodir (path, dir_mode_bits, uid, gid);
      if (mkdir_res == 0)
      {
        open_res = tftp_os_file_open_helper (path, oflags, file_mode_bits,
                                             uid, gid, handle, file_mode);
        if (open_res < 0)
        {
          if (open_res != -ENOENT)
          {
            TFTP_LOG_ERR("Could not create file after autodir.");
            return open_res;
          }
          ...
          /* Try to create folders in the data partition */
          (void) tftp_server_folders_init();
          TFTP_LOG_INFO("Creating dirs done.");
      ...
      struct tftp_server_client_info_type *client_info)
{
      ...
      ++option_idx)
  {
    ...
    switch (req_pkt->options.options[option_idx].option_id)
    {
      ...
      {
        proto->timeout_in_ms = TFTP_SERVER_MIN_TIMEOUT_IN_MS;
      ...
      case TFTP_PKT_OPTION_ID_RSIZE:
      {
        ...
        options->options[option_idx].option_id = TFTP_PKT_OPTION_ID_RSIZE;
      ...
      struct tftp_server_client_info_type *client_info)
{
  ...
  TFTP_LOG_INFO ("OACK options : [%d, %d, %d, %d, %d, %d, %d, %d]",
    options->options[0].option_value,
    options->options[1].option_value,
    options->options[2].option_value,
    options->options[3].option_value,
    options->options[4].option_value,
    options->options[5].option_value,
    options->options[6].option_value,
    options->options[7].option_value,
    options->options[8].option_value);
}
...
tftp_server_send_data_cb (void *param, uint16 block_number, uint8 **data_buf)
{
  ...
  *data_buf = info->data_buffer;
  ...
  result = tftp_file_seek (info->file_handle, seek_posn, SEEK_SET);
  if (result != 0)
  {
    goto End;
  }
  ...
  /* rsize of 0 is treated as full file read */
  if (info->proto.rsize > 0)
  {
    uint32 read_so_far = (block_number - 1) * info->proto.block_size;
    read_size = info->proto.rsize - read_so_far;
    if (read_size > info->proto.block_size)
    {
      read_size = info->proto.block_size;
    }
  }

  result = tftp_file_read (info->file_handle, info->data_buffer, read_size);
...
End:
  ...
  if (info->file_handle != NULL)
  {
    /* Flush the data. */
    tftp_file_close (info->file_handle);
    info->file_handle = NULL;
  }

  TFTP_LOG_INFO ("RRQ stats : sent_size = %u total-blocks = %d "
  ...
  TFTP_LOG_INFO ("RRQ time stats : Total : [TX, RX] = [%u, %u]",
                con_debug_info->total_send_time_delta,
                con_debug_info->total_recv_time_delta);

  TFTP_LOG_INFO ("RRQ time stats : Tx/Rx [Min, Max] = [%u, %u, %u, %u]",
  ...
  {
    TFTP_LOG_INFO ("RRQ was successfully processed.");
  }
                          ...
                          uint8 *data_buf, uint32 data_buf_size)
{
  ...
  result = tftp_file_write (info->file_handle, data_buf, data_buf_size);
                           ...
                           uint8 *data_buf, uint32 data_buf_size)
{
  ...
  {
    /* Flush the data. */
    (void) tftp_file_close (info->file_handle);
    info->file_handle = NULL;
  }
  ...
  if (info->is_atomic_put)
  {
    ...
    {
      ret_val = result;
      TFTP_LOG_INFO ("Rename failed for atomic put error=%d[%s], [%s to %s]",
...
End:
  ...
  TFTP_LOG_INFO ("WRQ stats : total-blocks = %d : total-bytes = %d : %d"
  ...
  TFTP_LOG_INFO ("WRQ time stats : Total : [TX, RX] = [%u, %u, %u]",
                con_debug_info->total_send_time_delta,
                con_debug_info->total_recv_time_delta);

  TFTP_LOG_INFO ("WRQ time stats : Tx [Min, Max] = [%u, %u]",
                con_debug_info->min_send_time_delta,
                con_debug_info->max_send_time_delta);

  TFTP_LOG_INFO ("WRQ time stats : Rx [Min, Max] = [%u, %u]",
                con_debug_info->min_recv_time_delta,
                con_debug_info->max_recv_time_delta);
...
tftp_thread_unlink_handler(struct tftp_server_client_info_type *info)
{
  ...
  if (unlink_res < 0)
  {
    ...
    TFTP_LOG_ERR ("unlink failed [%d] [%s]", unlink_res, strerror(unlink_res));
    tftp_protocol_send_errno_as_error_packet (proto, error_val);
    return;
  }

  TFTP_LOG_INFO ("unlink was success");
...
tftp_thread_req_handler (struct tftp_server_client_info_type *client_info)
{
  ...
  open_res = tftp_server_file_open (client_info);
  if (open_res < 0)
  {
    int32 error_val = -open_res;
    TFTP_LOG_ERR ("open failed : [%d] [%s]", open_res, strerror(open_res));
    tftp_protocol_send_errno_as_error_packet (proto, error_val);
    goto End;
  }
  ...
  if (client_info->data_buffer == NULL)
  {
    ...
    TFTP_LOG_ERR ("Malloc returned NULL");
    goto Error;
  }
...
End:
  ...
  if (client_info->file_handle != NULL)
  {
    tftp_file_close (client_info->file_handle);
    client_info->file_handle = NULL;
  }
