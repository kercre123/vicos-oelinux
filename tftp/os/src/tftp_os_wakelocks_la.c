...
#include <unistd.h>
  ...
  #endif /* ifndef CAP_NET_BIND_SERVICE */
...
static struct tftp_os_la_wakelock_struct tftp_os_la_wakelock;
...
static void drop_excess_capabilities (void)
{
  ...
  for (cap = 0; prctl (PR_CAPBSET_READ, cap, 0, 0, 0) >= 0; cap++)
  {
    ...
    if ((cap == CAP_SETUID) || (cap == CAP_SETGID) ||
        (cap == CAP_BLOCK_SUSPEND)|| (cap == CAP_NET_BIND_SERVICE))
    {
      continue;
    }
...
{
  int ret;
  gid_t gid_list[] = {AID_RFS, AID_RFS_SHARED, AID_SYSTEM, AID_WAKELOCK};
  ...
  ret = setgroups (gid_list_size, gid_list);
  if (ret < 0)
  {
...
tftp_os_wakelocks_init(void)
{
  ...
  tftp_os_la_wakelock.write_size = tftp_strlen (WAKE_LOCK_STRING);
