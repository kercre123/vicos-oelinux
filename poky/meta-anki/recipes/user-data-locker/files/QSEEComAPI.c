...
struct qcom_keymaster_handle* initialize_keymaster_handle() {

	//Initializing the handle
    struct qcom_keymaster_handle* handle = malloc(sizeof(struct qcom_keymaster_handle));
    memset(handle, 0, sizeof(struct qcom_keymaster_handle));

	//Getting the QSEECom library
    handle->libhandle = dlopen("/usr/lib/libQseeComApi.so", RTLD_NOW);
    if (!handle->libhandle) {
        perror("[-] Failed to load QSEEComAPI");
        return NULL;
    }
