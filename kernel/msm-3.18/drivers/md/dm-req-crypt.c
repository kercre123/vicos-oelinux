	...
	if (total_bytes_in_req > REQ_DM_512_KB) {
       ...
       __rq_for_each_bio(_bio, clone) {
               bio_for_each_segment_all(_bvec, _bio, i) {
                       if (_bvec && _bvec->bv_len > remaining_size) {
                               page = NULL;
                               while (page == NULL) {
                               ...
                               _bvec->bv_page = page;
                               _bvec->bv_offset = 0;
                               remaining_size = PAGE_SIZE - _bvec->bv_len;
                               if (remaining_size < 0)
                                       BUG();
                       } else {
                               _bvec->bv_page = page;
                               _bvec->bv_offset = PAGE_SIZE - remaining_size;
                               remaining_size = remaining_size -  _bvec->bv_len;
                       }
               }
       }
		...
		__rq_for_each_bio(_bio, clone) {
			bio_for_each_segment_all(_bvec, _bio, i) {
				if (_bvec->bv_offset == 0) {
					mempool_free(_bvec->bv_page, req_page_pool);
					_bvec->bv_page = NULL;
				} else {
					_bvec->bv_page = NULL;
				}
			    ...
			    int error, union map_info *map_context)
{
	...
	if (rq_data_dir(clone) == WRITE) {
		__rq_for_each_bio(_bio, clone) {
			bio_for_each_segment_all(_bvec, _bio, i) {
				if (req_io->should_encrypt && _bvec->bv_offset == 0) {
					mempool_free(_bvec->bv_page, req_page_pool);
					_bvec->bv_page = NULL;
				} else {
					_bvec->bv_page = NULL;
