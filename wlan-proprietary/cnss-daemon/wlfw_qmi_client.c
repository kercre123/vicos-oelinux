...
#define DEFAULT_BDF_FILE	"/firmware/image/bdwlan.bin"
#define BDF_FILE		"/firmware/image/bdwlan.b"
...
static int wlfw_send_ind_register_req(uint64_t *fw_status)
{
	...
	wsvc_printf_dbg("%s: result %d, error %d, fw_status_valid %u, fw_status 0x%llx",
			__func__,
			resp.resp.result,
			resp.resp.error,
			resp.fw_status_valid,
			(unsigned long long) resp.fw_status);
	return rc;
}
...
static int wlfw_send_bdf_download_req(void)
{
	...
	if (file == NULL) {
		wsvc_printf_err("%s: Fail to open BDF file %s", __func__,
				filename);
		rc = -1;
		goto end;
	}
...
static int wlfw_send_mac_addr_req(void)
{
	...
	wsvc_printf_dbg("Sending MAC address request");

	memset(&req, 0, sizeof(req));
	memset(&resp, 0, sizeof(resp));

	if (gdata.mac_addr.mac_addr_valid) {
	...
	gdata.mac_addr.send_count++;

	wsvc_printf_dbg("%s: result %d, error %d, send_count %u", __func__,
			resp.resp.result,
			resp.resp.error,
			gdata.mac_addr.send_count);
			    ...
			    void *err_cb_data)
{
	...
	gdata.state &= ~(CNSS_WLFW_QMI_CONNECTED | CNSS_MSA_READY);
...
void *wlfw_service_request(void *arg)
{
	...
	if (fw_status & QMI_WLFW_ALREADY_REGISTERED_V01) {
		if (!(fw_status & QMI_WLFW_FW_READY_V01)) {
			wsvc_printf_err("FW in bad state 0x%llx", (unsigned long long int )fw_status);
			goto skip_mac_addr;
		}
...
skip_cal_report:
	wlfw_send_mac_addr_req();

skip_mac_addr:
	while (svc_running) {
		pthread_mutex_lock(&(gdata.mutex));
		gettimeofday(&delta, NULL);
		abstime.tv_sec = delta.tv_sec + WLFW_TIMEOUT_MS/1000;
		rc = pthread_cond_timedwait(&(gdata.cond),
					    &(gdata.mutex), &abstime);
		pthread_mutex_unlock(&(gdata.mutex));
		if (rc != 0) {
			continue;
...
void *dms_service_request(void *arg)
{
	...
	if (rc != QMI_NO_ERR) {
		wsvc_printf_err("Failed to get WLAN MAC address");
		 ...
		 gdata.mac_addr.send_count) {
		wsvc_printf_dbg("Re-sending MAC address request, count %u",
				gdata.mac_addr.send_count);
		wlfw_send_mac_addr_req();
	}
