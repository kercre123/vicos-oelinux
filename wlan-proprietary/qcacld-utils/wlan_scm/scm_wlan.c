...
[WLAN_STA_1]   = {"/data/misc/wifi/wpa_supplicant.conf", "",                    ""},
[WLAN_SAP_1]   = {"/data/misc/wifi/hostapd.conf",        "/tmp/scm/sap1_entropy",   ""},
[WLAN_SAP_2]   = {"/data/misc/wifi/hostapd-wlan1.conf",  "/tmp/scm/sap2_entropy",   ""},
[WLAN_SAP_3]   = {"/data/misc/wifi/hostapd3.conf",       "/tmp/scm/sap3_entropy",   ""},
};
...
#define HOSTAPD_STARTUP_TIME_MAX 40
	...
	scm_logd("SAP startup in %d ms", retry*100);
...
int scm_wlan_load_daemon(struct scm_data *sd, struct wlan_if_data *wif)
{
	...
	case WLAN_STA_INT:
		if (sd->wlan_cfg == WLAN_DRV_CFG_1_SAP_1_STA_BRIDGE_MODE ||
			...
			snprintf(cmd, SCM_SYS_CMD_MAX,
				"/usr/sbin/wpa_supplicant -b %s -i wlan%d -c %s -Dnl80211 %s -P /tmp/scm/wif%d.pid -B",
				BRIDGE_IFACE, wif->num, wif->d_cfg->conf_file,
				wif->d_cfg->ext_cmd_args, wif->num);
		}else {
			snprintf(cmd, SCM_SYS_CMD_MAX,
				"/usr/sbin/wpa_supplicant -i wlan%d -c %s -Dnl80211 %s -P /tmp/scm/wif%d.pid -B",
	...
	case WLAN_SAP_3:
		...
		// So copy to /tmp and update
		scm_main_sys_cmd_res(NULL, 0, "cp %s /tmp/scm/%s; sync",
				wif->d_cfg->conf_file, wif->d_cfg->conf_file);
		scm_main_sys_cmd_res(NULL, 0,
				"sed -i '/\\<interface=/c\\interface=wlan%d' /tmp/scm/%s",
				wif->num, wif->d_cfg->conf_file);
		snprintf(cmd, SCM_SYS_CMD_MAX,
				"/usr/sbin/hostapd /tmp/scm/%s -e %s %s -P/tmp/scm/wif%d.pid  -B",
	...
	ret = 0;
err:
	scm_main_sys_cmd_res(res, sizeof(res), "cat /tmp/scm/wif%d.pid", wif->num);
...
int scm_wlan_enable(struct scm_data *sd, int wlan_drv_cfg)
{
	...
	system("killall -9 wlan_services");
