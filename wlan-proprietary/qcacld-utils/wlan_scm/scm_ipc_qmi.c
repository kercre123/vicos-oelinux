					...
					void* req, int req_len)
{
	...
	for (i = 0; i < req_msg->wlan_if_type_len &&
		...
		if (wif) {
			...
			scm_logi("New IF_Name: wlan%d %s", wif->num,
					scm_wlan_if_type_str(wif->type));
			j++;
		}
	}
					...
					void* req, int req_len)
{
	...
	for (i = 0; i < WLAN_IF_TYPE_MAX; i++) {
		...
		if (WLAN_IS_STA(sd->wif[i]->type)) {
			if (sd->act_sta_wif != sd->wif[i])
				continue;
		}
		resp_msg.wlan_concurrency_cfg[j].wlan_if_type =
				scm_ipc_s2q_wlan_if_type(sd->wif[i]->type);
		resp_msg.wlan_concurrency_cfg[j].wlan_if_num = sd->wif[i]->num;
		scm_logi("IF_Name: wlan%d %s",  sd->wif[i]->num,
					scm_wlan_if_type_str(sd->wif[i]->type));
