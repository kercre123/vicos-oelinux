...
status_t BnQvopCallback::onTransact(
  ...
  case QVOP_RES_ERROR:
  {
    logfile_print_v("%s: %s", QVOP_FN, "QVOP_RES_ERROR");
  ...
  {
    logfile_print_v("%s: %s", QVOP_FN, "QVOP_RES_ENROLL");
    CHECK_INTERFACE(IQvopCallback, data, reply);
  ...
  {
    logfile_print_v("%s: %s", QVOP_FN, "QVOP_RES_MATCH_FOUND");
    CHECK_INTERFACE(IQvopCallback, data, reply);
...
#ifdef QVOP_DEBUG
    logfile_print_v("%s: onMatchFound: index=%d, sentence_score=%lf, verification_score=%lf, spoofScore=%f",
  ...
  {
    logfile_print_v("%s: %s", QVOP_FN, "QVOP_RES_MATCH_NOT_FOUND");
    CHECK_INTERFACE(IQvopCallback, data, reply);
    int32_t error = data.readInt32();
    int32_t index = data.readInt32();
    double sentence_score = data.readDouble();
...
#ifdef QVOP_DEBUG
    logfile_print_v("onMatchNotFound: error=%d, index=%d, sentence_score=%lf, verification_score=%lf, spoofScore=%f",
  ...
  {
    logfile_print_v("%s: %s", QVOP_FN, "QVOP_RES_DELETE_USER");
    CHECK_INTERFACE(IQvopCallback, data, reply);
    int32_t status = data.readInt32();
    char const* user = data.readCString();
  ...
  {
    logfile_print_v("%s: %s", QVOP_FN, "QVOP_RES_DELETE_ID");
    CHECK_INTERFACE(IQvopCallback, data, reply);
    int32_t status = data.readInt32();
    char const* user = data.readCString();
    char const* id = data.readCString();
  ...
  {
    logfile_print_v("%s: %s", QVOP_FN, "QVOP_RES_DELETE_ALL");
    CHECK_INTERFACE(IQvopCallback, data, reply);
    int32_t status = data.readInt32();
