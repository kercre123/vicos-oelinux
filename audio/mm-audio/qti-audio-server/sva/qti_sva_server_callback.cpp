...
public:
    ...
    int recognition_cb(struct sound_trigger_recognition_event *event,
                        void *cookie1, void *cookie2) {
        ALOGV("%s %d", __func__,__LINE__);
        Parcel data, reply;
        data.writeInterfaceToken(IRecognitionCallback::getInterfaceDescriptor());
        uint32_t size = sizeof(struct sound_trigger_recognition_event)
                                    + event->data_size;
        data.writeUint32(size);
        android::Parcel::WritableBlob blob;
        data.writeBlob(size, false, &blob);
        memset(blob.data(), 0x0, size);
        ALOGD("size %d, offset %d, data_size %d", size, event->data_offset,
                    event->data_size);
        memscpy(blob.data(), (size - event->data_size), event,
                    (size - event->data_size));
        memscpy(((char *)blob.data()) + event->data_offset, event->data_size,
                    ((char *)event) + event->data_offset, event->data_size);
        blob.release();
...
int32_t QTIRecognitionCallback::onTransact(uint32_t code,
    ...
    switch(code) {
        case ON_RECOGNITION_EVENT: {
            ...
            void *cookie2 = NULL;
            uint32_t blob_size;
            struct sound_trigger_recognition_event *rec_event = NULL;
            android::Parcel::ReadableBlob blob;
            data.readUint32(&blob_size);
            data.readBlob(blob_size, &blob);
            rec_event = (struct sound_trigger_recognition_event *)calloc(1, blob_size);
            if (rec_event == NULL) {
                ALOGE("%s: Could not allocate memory for sound model", __func__);
                return -ENOMEM;
            }
            int size = sizeof(sound_trigger_recognition_event);
            memscpy(rec_event, size, blob.data(), size);
            memscpy(((char *)rec_event) + rec_event->data_offset,
                        (blob_size - size), ((char *)blob.data())
                        + rec_event->data_offset, (blob_size - size));
            blob.release();
            data.read(&cookie1, sizeof(qsthw_recognition_callback_t *));
            data.read(&cookie2, sizeof(qsthw_recognition_callback_t *));
            status = recognition_cb(rec_event, cookie1, cookie2);
            break;
        }
        default:
