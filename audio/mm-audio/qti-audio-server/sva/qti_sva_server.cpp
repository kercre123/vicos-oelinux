...
static void recognition_event_cb(
    ...
    struct sound_trigger_recognition_event *qsthw_event;
    qsthw_event = (struct sound_trigger_recognition_event *)event;
    ...
    cb_binder->recognition_cb(qsthw_event,
...
int32_t qti_sva_server::onTransact(uint32_t code,
    ...
    switch(code) {
        ...
        case GET_PARAM_DATA: {
            ...
            data.read(&payload_size, sizeof(qsthw_get_param_payload_t));
            ...
            reply->writeUint32(param_data_size);
            android::Parcel::WritableBlob blob;
        ...
        case PREPARE_FOR_READ:
        {
            ...
            return prepare_for_buf_read((n_sound_model *)n_stream);
        }
        break;
        case STOP_BUFFERING: {
    ...
    while (1) {
        ...
        switch(shmem_data->cmd) {
            case READ_BUFF:
            {
                ...
                shmem_data->status = 0;
                ALOGV("%s:Taking cmd mutex", __func__);
                pthread_mutex_lock(&shmem_data->c_mutex);
                ALOGV("%s:Taken cmd mutex", __func__);
