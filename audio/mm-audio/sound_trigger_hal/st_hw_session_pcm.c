    ...
    sound_trigger_sound_model_type_t sm_type)
{
    ...
    p_pcm_ses->pcm = pcm_open(SOUND_CARD, p_pcm_ses->pcm_id,
...
int st_hw_pcm_init()
{
    ...
    DLSYM(pcm_data.ffv_lib_handle, ffv_init, status);
    if (status)
        goto exit;
    DLSYM(pcm_data.ffv_lib_handle, ffv_deinit, status);
    if (status)
        goto exit;
    DLSYM(pcm_data.ffv_lib_handle, ffv_process, status);
    if (status)
        goto exit;
    DLSYM(pcm_data.ffv_lib_handle, ffv_read, status);
    if (status)
        goto exit;
    DLSYM(pcm_data.ffv_lib_handle, ffv_get_param, status);
    if (status)
        goto exit;
    DLSYM(pcm_data.ffv_lib_handle, ffv_set_param, status);
    if (status)
        goto exit;
    DLSYM(pcm_data.ffv_lib_handle, ffv_register_event_callback, status);
    if (status)
        goto exit;
