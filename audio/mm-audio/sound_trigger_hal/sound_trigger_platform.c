...
#define ST_PARAM_KEY_TRANSIT_TO_WDSP_WAIT_TIME "transit_to_wdsp_wait_time"
...
static void platform_stdev_set_default_config(struct platform_data *platform)
{
    ...
    stdev->transit_to_wdsp_wait_time = DEFAULT_TRANSIT_TO_WDSP_WAIT_TIME_SEC;
...
static int platform_stdev_load_sm_lib(struct st_vendor_info *sm_info, const char *name)
{
    ...
    sm_info->generate_st_phrase_recognition_event_v2 =
        (smlib_generate_sound_trigger_phrase_recognition_event_t)
        dlsym(sm_info->smlib_handle,
              "generate_sound_trigger_phrase_recognition_event_v2");

    if ((error = dlerror()) != NULL) {
        ALOGE("%s: dlsym error %s for generate_sound_trigger_phrase_recognition_event_v2",
              __func__, error);
        ret = -EINVAL;
        goto error_exit;
    }
    ...
    if ((error = dlerror()) != NULL) {
        ALOGE("%s: dlsym error %s for generate_sound_trigger_recognition_config_payload_v2",
              __func__, error);
        ret = -EINVAL;
        goto error_exit;
    }
    ...
    if ((error = dlerror()) != NULL) {
        ALOGE("%s: dlsym error %s for generate_sound_trigger_phrase_recognition_event_v3",
                 __func__, error);
        goto error_exit;
    }
...
)
{
    ...
    err = str_parms_get_int(parms, ST_PARAM_KEY_TRANSIT_TO_WDSP_WAIT_TIME, &value);
    if (err >= 0) {
        str_parms_del(parms, ST_PARAM_KEY_TRANSIT_TO_WDSP_WAIT_TIME);
        stdev->transit_to_wdsp_wait_time = value;
    }
...
static int string_to_fluence_type(const char *str, st_fluence_type_t *type)
{
    ...
    if (!strncmp(str, "FLUENCE", sizeof("FLUENCE"))) {
...
void *platform_stdev_init(sound_trigger_device_t *stdev)
{
    ...
    /* Check if ISV vendor_uuid is present  */
...
)
{
    ...
    int channel_count = my_data->codec_backend_cfg.channel_count;
...
)
{
    ...
    *backend_cfg_change = false;
    ...
    if (channel_count != my_data->codec_backend_cfg.channel_count) {
        if (my_data->codec_backend_cfg.channel_count != SOUND_TRIGGER_CHANNEL_MODE_OCT &&
            ...
            if (channel_count == SOUND_TRIGGER_CHANNEL_MODE_OCT) {
                mixer_ctl_set_enum_by_string(ctl, "Eight");
            } else if (channel_count == SOUND_TRIGGER_CHANNEL_MODE_HEX) {
                mixer_ctl_set_enum_by_string(ctl, "Six");
            } else if (channel_count == SOUND_TRIGGER_CHANNEL_MODE_QUAD) {
