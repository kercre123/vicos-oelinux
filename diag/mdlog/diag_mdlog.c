...
static void parse_args(int argc, char **argv)
{
	...
	while ((command = getopt_long(argc, argv, "f:m:l:o:p:s:w:n:t:x:g:cdkebrah", longopts, NULL))
			!= -1) {
...
int main(int argc, char *argv[])
{
	...
	if (peripheral_mask)
		...
		if (err) {
			DIAG_LOGE("diag: Kernel not supporting UPD logging, err: %d, errno: %d\n",
				 err, errno);
			return -1;
		} else {
			DIAG_LOGE("diag: Kernel supports UPD logging, err: %d\n",
				 err);
		}
