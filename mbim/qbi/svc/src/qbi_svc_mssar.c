...
#define MIN_HYSTERESIS_TIMER 1
#define MAX_HYSTERESIS_TIMER 5
...
/*-------------------------------------------------------------------------*/
  ...
  {
    QBI_LOG_D_1("Timer created with ID = %d", cache->timer.timerid);
    return TRUE;
  }
  return FALSE;
}/* qbi_sar_timer_init */
...
/*-------------------------------------------------------------------------*/
  ...
  if (QBI_SVC_MBIM_MS_SAR_CONTROL_MODE_OS == req->mode ||
    QBI_SVC_MBIM_MS_SAR_BACKOFF_STATUS_DISABLED == req->backoff_status)
  {
    ...
    qbi_svc_mssar_config_q_req(txn);
  }
  else
  {
...
/*-------------------------------------------------------------------------*/
  ...
  if (cache->notification_enabled == QBI_SVC_MBIM_MS_SAR_TRANSMISSION_NOTIFICATION_ENABLED)
  {
    ...
    QBI_LOG_D_1("Timer started in set req with ID = %d", cache->timer.timerid);
  }
  else
  {
...
/*-------------------------------------------------------------------------*/
  ...
  if (cache->notification_enabled)
  {
    /* Waking up the timer*/
    qbi_os_timer_set(&cache->timer, (cache->hysteresis_timer * 1000) + 500);
    QBI_LOG_D_1("Timer restarted in Ind with ID = %d", cache->timer.timerid);
...
/*-------------------------------------------------------------------------*/
  ...
  else
  {
    ...
    cache->hysteresis_timer = req->hysteresis_timer;
