...
static qbi_svc_bc_cache_s *qbi_svc_bc_cache_get
(
  const qbi_ctx_s *ctx
);
...
static uint32 qbi_svc_bc_max_sessions_get
(
  const qbi_ctx_s *ctx
);
...
static qbi_svc_action_e qbi_svc_bc_radio_state_q_req
(
  qbi_txn_s *txn
);
...
static qbi_svc_action_e qbi_svc_bc_radio_state_s_req
(
  qbi_txn_s *txn
);
...
static uint32 qbi_svc_bc_connect_combine_activation_states
(
...
static wds_auth_pref_mask_v01 qbi_svc_bc_connect_mbim_auth_pref_to_qmi_auth_pref
(
  uint32 auth_protocol
);

static wds_profile_auth_protocol_enum_v01 qbi_svc_bc_connect_mbim_auth_pref_to_qmi_auth_protocol
(
  uint32 auth_protocol
);
...
static boolean qbi_svc_bc_connect_is_session_ipv4_activated
(
  const qbi_ctx_s *ctx,
  uint32           session_id
);

static boolean qbi_svc_bc_connect_is_session_ipv6_activated
(
  const qbi_ctx_s *ctx,
  uint32           session_id
);
...
static qbi_svc_action_e qbi_svc_bc_connect_s_deactivate
(
  qbi_txn_s *txn
);
...
static uint32 qbi_svc_bc_connect_session_activation_state
(
  const qbi_ctx_s *ctx,
  uint32           session_id
);
...
/*-------------------------------------------------------------------------*/
  ...
  else
  {
    ...
    {
      QBI_LOG_D_0("Unable to read executor_slot_config NV. Setting default as 0");
...
/*-------------------------------------------------------------------------*/
  ...
  else if (!qmi_rsp->qos_format_valid || !qmi_rsp->link_prot_valid ||
  ...
  {
    QBI_LOG_D_0("Successfully configured data format");
...
/*-------------------------------------------------------------------------*/
  ...
  if (qmi_txn->parent->qmi_txns_pending == 0)
  {
    ...
    if ((cache->device_caps.data_class & QBI_SVC_BC_DATA_CLASS_3GPP_FAMILY) == 0 && 
	    !qbi_svc_bc_device_is_in_ftm(qmi_txn->ctx))
    {
      /* 3GPP2-only modems are no longer supported by QBI */
      QBI_LOG_E_0("Device doesn't support 3GPP!");
    }
    else
    {
...
)
{
  ...
  char provider_name_ascii[QBI_SVC_BC_PROVIDER_NAME_ASCII_MAX_LEN];
/*-------------------------------------------------------------------------*/
        ...
        plmn_name->plmn_short_name_len == 0)))
  {
    ...
    plmn_name_encoded = plmn_name->spn;
           ...
           ((plmn_name->plmn_long_name_enc ==
               NAS_CODING_SCHEME_CELL_BROADCAST_GSM_V01 &&
             plmn_name->plmn_long_name_len <= short_name_threshold) ||
    ...
    if (plmn_name_dcs == NAS_CODING_SCHEME_CELL_BROADCAST_GSM_V01)
    {
      if (plmn_name_len > sizeof(provider_name_ascii))
      {
        QBI_LOG_W_2("ProviderName will be truncated from %d to %d characters",
                    plmn_name_len, sizeof(provider_name_ascii));
        bytes_copied = sizeof(provider_name_ascii);
      }
      else
      {
...
/*-------------------------------------------------------------------------*/
  ...
  {
    if (qmi_rsp->call_end_reason == WDS_CER_AUTH_FAILED_V01)
    {
      QBI_LOG_I_0("Activation failed due to general authentication failure");
...
/*-------------------------------------------------------------------------*/
  ...
  else
  {
    ...
    qbi_txn_set_timeout(txn, QBI_SVC_BC_CONNECT_TIMEOUT_MS, NULL);
...
/*-------------------------------------------------------------------------*/
  ...
  if (result == TRUE && profile_settings->pdp_type_valid &&
...
/*-------------------------------------------------------------------------*/
  ...
  if (req->access_string.offset != 0)
  {
    ...
    if (!QBI_STRNCMP(apn_name, QBI_SVC_BC_CONNECT_LOOPBACK_APN_ASCII,
                     sizeof(apn_name)))
    {
...
/*-------------------------------------------------------------------------*/
  ...
  else
  {
    ...
    if (!qbi_svc_bc_connect_is_loopback(txn->ctx) &&
        qmi_rsp_ipv4->primary_DNS_IPv4_address_preference_valid)
    {
...
/*-------------------------------------------------------------------------*/
  ...
  else if (!qbi_svc_bc_connect_is_loopback(txn->ctx) &&
           qbi_svc_bc_connect_is_session_ipv6_activated(
             txn->ctx, req->session_id) &&
