...
/*-------------------------------------------------------------------------*/
  ...
  else if (!qbi_nv_store_cfg_item_read(qmi_txn->parent->ctx,
...
/*-------------------------------------------------------------------------*/
  ...
  {
    /*check is needed to avoid buffer overflow. 
    Valid cases are 
    1. physical slot 0 -> logical slot 0 (card length == 1) 
    2. physical slot 1 -> logical slot 0 (card length == 1) 
    3. physical slot 0 and 1 (for DSSA) -> logical slot 0 or 1 (card length == 2)*/
    QBI_LOG_E_1("Invalid Slot Index obtained from exec slot conf %d", 
      exec_slot_cfg.exec0_slot);
  }
  else
  {
    req = (qbi_svc_msuicc_open_channel_s_req_s *)qmi_txn->parent->req.data;
    if (qmi_rsp->card_mode[exec_slot_cfg.exec0_slot] == UIM_CARD_MODE_TELECOM_CARD_V01)
    {
    ...
    else if (qmi_rsp->card_mode[exec_slot_cfg.exec0_slot] == UIM_CARD_MODE_NON_TELECOM_CARD_V01)
    {
...
/*-------------------------------------------------------------------------*/
  ...
  {
    QBI_LOG_D_0("Sending first apdu req");
...
/*-------------------------------------------------------------------------*/
  ...
  if (qmi_rsp->resp.result == QMI_RESULT_FAILURE_V01 ||
      qmi_rsp->apdu_valid == FALSE)
  {
    rsp->status[0] = qmi_rsp->apdu[qmi_rsp->apdu_len - 1];
    rsp->status[1] = qmi_rsp->apdu[qmi_rsp->apdu_len];
    qmi_txn->parent->status = QBI_MBIM_STATUS_MS_SELECT_FAILED;
    QBI_LOG_E_2("Received error code %d from QMI, apdu valid %d",
      qmi_rsp->resp.error, qmi_rsp->apdu_valid);
  }
  else
  {
    info = (qbi_svc_msuicc_channel_req_info *)qmi_txn->parent->info;
...
/*-------------------------------------------------------------------------*/
  ...
  else if ((UIM_SLOT_1_V01 + exec_slot_cfg.exec0_slot) > MAX_SUPPORTED_SLOTS)
  {
    ...
    2. physical slot 1 -> logical slot 0 (card length == 1) 
    3. physical slot 0 and 1 (for DSSA) -> logical slot 0 or 1 (card length == 2)*/
    QBI_LOG_E_1("Invalid Slot Index obtained from exec slot conf %d", exec_slot_cfg.exec0_slot);
  }
  else
  {
...
/*-------------------------------------------------------------------------*/
  ...
  QBI_LOG_D_0("In qbi_svc_msuicc_reset_q_req");
...
/*-------------------------------------------------------------------------*/
  ...
  else if (qmi_rsp->card_mode_valid == FALSE) 
  {
    ...
    qmi_txn->parent->ctx, QBI_NV_STORE_CFG_ITEM_EXECUTOR_SLOT_CONFIG,
    &exec_slot_cfg, sizeof(qbi_svc_bc_ext_exec_slot_config_s)))
  {
    QBI_LOG_E_0("Missing executor slot mapping info");
  }
  ...
  else
  {
    ...
    if (qmi_rsp->card_mode[exec_slot_cfg.exec0_slot] == UIM_CARD_MODE_NON_TELECOM_CARD_V01)
    {
      rsp->pass_through_status = PASS_THROUGH_STATUS_ENABLED;
      QBI_LOG_D_3("card mode = %d pass through status = %d exec_slot_cfg.exec0_slot = %d",
      qmi_rsp->card_mode[exec_slot_cfg.exec0_slot],
      rsp->pass_through_status, exec_slot_cfg.exec0_slot);
    }
    else if (qmi_rsp->card_mode[exec_slot_cfg.exec0_slot] == UIM_CARD_MODE_TELECOM_CARD_V01)
    {
...
/*-------------------------------------------------------------------------*/
  ...
  QBI_LOG_D_0("In qbi_svc_msuicc_reset_s_req ");
  ...
  else
  {
    ...
    QBI_LOG_D_0("Evt reg success. Getting card status");
    qmi_req = (uim_get_card_status_req_msg_v01 *)
      qbi_qmi_txn_alloc_ret_req_buf(
        txn, QBI_QMI_SVC_UIM, QMI_UIM_GET_CARD_STATUS_REQ_V01,
...
/*-------------------------------------------------------------------------*/
  ...
  else
  {
    ...
    QBI_CHECK_NULL_PTR_RET_ABORT(qmi_req);
    qmi_req->slot = UIM_SLOT_1_V01 + exec_slot_cfg.exec0_slot;
    QBI_LOG_D_2("Slot requested to power down = %d exec_slot_cfg.exec0_slot = %d ",
      qmi_req->slot, exec_slot_cfg.exec0_slot);
...
/*-------------------------------------------------------------------------*/
  ...
  else if (!qbi_nv_store_cfg_item_read(ind->txn->ctx,
      QBI_NV_STORE_CFG_ITEM_EXECUTOR_SLOT_CONFIG,&exec_slot_cfg,
      sizeof(qbi_svc_bc_ext_exec_slot_config_s)))
  {
    QBI_LOG_E_0("Missing executor slot mapping info");
  ...
  else if (qmi_ind->card_status.card_info[exec_slot_cfg.exec0_slot].card_state == UIM_CARD_STATE_ERROR_V01)
  {
    ...
    if (req->pass_through_action == PASS_THROUGH_ACTION_ENABLE)
    {
      ...
      QBI_LOG_D_4("Non -telecom qmi_req->slot = %d card mode = %d req->pass_through_action = %d exec_slot_cfg.exec0_slot = %d",
       qmi_req->slot,
       qmi_req->card_mode,
       req->pass_through_action, exec_slot_cfg.exec0_slot);
    }
    else if(req->pass_through_action == PASS_THROUGH_ACTION_DISABLE)
    {
      ...
      QBI_LOG_D_4("Telecom qmi_req->slot = %d card mode = %d req->pass_through_action = %d exec_slot_cfg.exec0_slot = %d",
       qmi_req->slot,
       qmi_req->card_mode,
       req->pass_through_action, exec_slot_cfg.exec0_slot);
    }
  ...
  else if (qmi_ind->card_status.card_info[exec_slot_cfg.exec0_slot].card_state == UIM_CARD_STATE_PRESENT_V01)
  {
