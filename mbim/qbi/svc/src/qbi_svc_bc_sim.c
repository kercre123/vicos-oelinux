...
/*-------------------------------------------------------------------------*/
  ...
  else if (!qbi_nv_store_cfg_item_read(ctx,
  ...
  else
  {
          ...
          ctx, card_status, &primary_card_index, &primary_app_index))
    {
      ...
      /* NV Read and update index */
      primary_card_index = exec_slot_cfg.exec0_slot;
...
/*-------------------------------------------------------------------------*/
  ...
  else
  {
          ...
          qmi_txn->ctx, &qmi_rsp->card_status, &card_index, &app_index))
    {
      ...
      /* Received a card indication with information that modem has not provisioned the card
         This could be due to the following situations:
           - A card is inserted and ICCID has changed so provisioning is required from Host side
           - A card is inserted with cached ICCID. But no harm in provisioning from Host side
           - An eSIM profile has been enabled. So provisioning is required
         Note #1  In case eSIM profile is disabled then card will be in eSIM no profile state
         and no provisioning is required. Hence checking that num apps > 0 before attempting to
         provision 
         Note #2 In case slot mapping is on-going then there are transitory states provisioning 
         has not yet occurred but will occur in subsequent steps. So checking prov complete flag
         to attempting provisioning only if prov complete flag is set, i.e. slot mapping is not 
         already in progress*/
      if ((exec_slot_cfg.exec0_prov_complete == TRUE ) &&
          (qmi_rsp->card_status.card_info[exec_slot_cfg.exec0_slot].card_state ==
            UIM_CARD_STATE_PRESENT_V01) &&
          (qmi_rsp->card_status.card_info[exec_slot_cfg.exec0_slot].app_info_len >= 1))
      {
        txn = qbi_txn_alloc(qmi_txn->parent->ctx, QBI_SVC_ID_BC_EXT, 0, QBI_TXN_CMD_TYPE_INTERNAL,
                QBI_SVC_BC_EXT_MBIM_CID_MS_DEVICE_SLOT_MAPPING, 0, NULL);
        QBI_CHECK_NULL_PTR_RET_ABORT(txn);
        (void) qbi_svc_proc_action(txn, qbi_svc_bc_ext_provision_card(txn));
        return QBI_SVC_ACTION_ABORT;
      }
    }
    else
    ...
    info->num_apps = qmi_rsp->card_status.card_info[exec_slot_cfg.exec0_slot].app_info_len;
    QBI_LOG_D_3("exec_slot_cfg.exec0_slot =%d info->num_apps  = %d rsp->ready_state = %d",
      exec_slot_cfg.exec0_slot, info->num_apps, rsp->ready_state );
...
/*-------------------------------------------------------------------------*/
  ...
  {
    /* Telephone numbers are informational and optional, so don't fail the CID
       on any related error. If this failure is because the PBM cache is not
       initialized yet, we will send the response without the telephone numbers
       and update the host with an event once the cache is ready. */
...
/*-------------------------------------------------------------------------*/
  ...
  if (txn->qmi_txns_pending == 0)
  {
    ...
    action = qbi_svc_bc_ext_configure_nv_for_operator(txn);
    if (QBI_SVC_ACTION_SEND_QMI_REQ != action && QBI_SVC_ACTION_SEND_RSP != action)
    {
...
/*-------------------------------------------------------------------------*/
  ...
  action = qbi_svc_bc_sim_pin_q_req(txn);
  (void) qbi_svc_proc_action(txn, action);
...
/*-------------------------------------------------------------------------*/
  ...
  if (qmi_ind->card_status_valid)
  {
               ...
               primary_app_index))
    {
      ...
      action = qbi_svc_bc_sim_pin_q_req(ind->txn);
    }
    else
    {
...
/*-------------------------------------------------------------------------*/
  ...
  req = (qbi_svc_bc_pin_s_req_s *) txn->req.data;
  if (req->pin_op == QBI_SVC_BC_PIN_OP_CHANGE)
  {
    /* No indication expected for changing the PIN code, so skip straight to the
       response. */
    action = qbi_svc_bc_sim_pin_q_req(txn);
  }
  else if (!qbi_svc_ind_reg_dynamic(
...
/*-------------------------------------------------------------------------*/
  ...
  else
  {
    ...
    if (qbi_svc_bc_nas_register_state_get_current_cellular_class(qmi_txn->ctx) ==
        ...
        cache->home_provider.cdma_spn_len > 0)
    {
      QBI_LOG_I_0("Using EF-SPN from CSIM as ProviderName");
...
/*-------------------------------------------------------------------------*/
  ...
  if (!qbi_nv_store_cfg_item_read(ind->txn->ctx,
    QBI_NV_STORE_CFG_ITEM_EXECUTOR_SLOT_CONFIG, &exec_slot_cfg,
    sizeof(qbi_svc_bc_ext_exec_slot_config_s)))
  {
    QBI_LOG_E_0("Missing executor slot mapping info");
  ...
  qmi_req->slot = exec_slot_cfg.exec0_slot + UIM_SLOT_1_V01;
...
/*-------------------------------------------------------------------------*/
  ...
  if (qmi_ind.card_status_valid)
  {
    ...
    if (!qbi_nv_store_cfg_item_read(qmi_txn->ctx,
      QBI_NV_STORE_CFG_ITEM_EXECUTOR_SLOT_CONFIG, &exec_slot_cfg,
      sizeof(qbi_svc_bc_ext_exec_slot_config_s)))
    {
      QBI_LOG_E_0("Missing executor slot mapping info");
    ...
    slot_index = exec_slot_cfg.exec0_slot;
    if (new_ready_state == QBI_SVC_BC_READY_STATE_NOT_INITIALIZED && info->is_esim &&
      qmi_ind.card_status.card_info[slot_index].app_info_len == 0)
    {
