...
static qbi_svc_bc_nas_cache_s *qbi_svc_bc_nas_cache_get
(
  const qbi_ctx_s *ctx
);
...
static boolean qbi_svc_bc_nas_register_state_is_registered
(
  uint32 register_state
);
...
/*-------------------------------------------------------------------------*/
  ...
  else
  {
        ...
        (rsp->register_state == QBI_SVC_BC_REGISTER_STATE_DEREGISTERED))
    {
      ...
      QBI_LOG_I_1("RegState is DENIED/FORBIDDEN/DEREGISTERED with nw_error %d",
                  rsp->nw_error);
    }
...
/*-------------------------------------------------------------------------*/
  ...
  {
    if (qbi_svc_bc_connect_is_connected(txn->ctx))
    {
      QBI_LOG_I_0("Can't perform network scan while context is activated");
      txn->status = QBI_MBIM_STATUS_BUSY;
    }
    else if (qbi_svc_bc_nas_visible_providers_q_need_mode_pref_workaround(
               txn->ctx))
    {
