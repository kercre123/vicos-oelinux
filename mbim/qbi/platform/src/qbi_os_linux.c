...
/*-------------------------------------------------------------------------*/
  ...
  /*! @todo To be removed. This is a temporary workaround for closing
      control port for GPS interface. */
  qmi_req_gps = QBI_MEM_MALLOC_CLEAR(sizeof(dpm_close_port_req_msg_v01));
  if (qmi_req_gps != NULL)
  {
    QBI_MEMSET(&qmi_rsp_gps, 0, sizeof(qmi_rsp_gps));
    if (!qbi_os_linux_dpm_get_gps_port_info(
          ctx, qmi_req_gps->control_port_list[0].port_name,
    ...
    {
      qmi_req_gps->control_port_list_valid = TRUE;
      qmi_req_gps->control_port_list_len = 1;
      qmi_req_gps->data_port_list_valid = TRUE;
      ...
      success = qbi_os_linux_dpm_send_qmi_req(
        ...
        &qmi_rsp_gps, sizeof(qmi_rsp_gps));
      if (success && qmi_rsp_gps.resp.result != QMI_RESULT_SUCCESS_V01)
      {
        QBI_LOG_E_1("Port mapper close failed with error %d", qmi_rsp_gps.resp.error);
      }
    }
    QBI_MEM_FREE(qmi_req_gps);
  }
...
/*-------------------------------------------------------------------------*/
  ...
  /* Currently hard-coding control port name. This has been stable across
     multiple chipsets, so no changes are expected. */
  QBI_STRLCPY(control_port_name, "DATA40_CNTL", control_port_name_len);
...
/*-------------------------------------------------------------------------*/
  ...
  /*! @todo To be removed. This is a temporary workaround for openning
      control port for GPS interface. */
  qmi_req_gps = QBI_MEM_MALLOC_CLEAR(sizeof(dpm_open_port_req_msg_v01));
  if (qmi_req_gps != NULL)
  {
    QBI_MEMSET(&qmi_rsp_gps, 0, sizeof(qmi_rsp_gps));
    if (!qbi_os_linux_dpm_get_gps_port_info(
    ...
    else
    {
      qmi_req_gps->control_port_list_valid = TRUE;
      qmi_req_gps->control_port_list_len = 1;
      qmi_req_gps->hardware_data_port_list_valid = TRUE;
      qmi_req_gps->hardware_data_port_list_len = 1;

      QBI_LOG_D_0("Sending QMI_DPM_OPEN_PORT_REQ for GPS");
      success = qbi_os_linux_dpm_send_qmi_req(
        QMI_DPM_OPEN_PORT_REQ_V01, qmi_req_gps, sizeof(dpm_open_port_req_msg_v01),
        &qmi_rsp_gps, sizeof(qmi_rsp_gps));
      if (success && qmi_rsp_gps.resp.result != QMI_RESULT_SUCCESS_V01)
      {
        QBI_LOG_E_1("Port mapper open failed with error %d", qmi_rsp_gps.resp.error);
      }
    }
    QBI_MEM_FREE(qmi_req_gps);
  }
...
)
{
  boolean success = FALSE;
/*-------------------------------------------------------------------------*/
  if (qbi_hc_dpm_is_used(ctx) && !qbi_os_linux_dpm_close(ctx))
  {
    QBI_LOG_E_0("Failed to close port");
  }
  else
  {
    success = TRUE;
