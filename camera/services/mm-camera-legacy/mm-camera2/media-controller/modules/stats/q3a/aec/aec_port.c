  ...
  uint32_t *destLumaBuff, void *rawBuff)
{
  ...
  if (private->parse_RDI_stats.parse_VHDR_stats_callback != NULL) {
    ...
    CDBG_LOW("%s parse_VHDR_stats_callback not supported", __func__);
    rc = -1;
  }
  return rc;
}
  ...
  if (private->aec_get_data.valid_entries) {
    ...
    if (aec_msg) {
      ...
      if (aec_msg) {
        ...
        stats_get_data->aec_get.gtm_gain =
          aec_msg->u.aec_get_parm.u.exp_params.drc_gains.gtm_gain;
        stats_get_data->aec_get.ltm_gain =
          aec_msg->u.aec_get_parm.u.exp_params.drc_gains.ltm_gain;
        stats_get_data->aec_get.la_gain =
          aec_msg->u.aec_get_parm.u.exp_params.drc_gains.la_gain;
        stats_get_data->aec_get.gamma_gain =
          aec_msg->u.aec_get_parm.u.exp_params.drc_gains.gamma_gain;
...
static void aec_port_handle_vhdr_buf( aec_port_private_t *private, isp_buf_divert_t *hdr_stats_buff )
{
    ...
    CDBG_LOW("aec_msg=%p, HDR stats", aec_msg);
    ...
    aec_port_parse_RDI_stats_AE(private, (uint32_t *)aec_stats->yuv_stats.q3a_aec_stats.SY,
  ...
  mct_event_t *event)
{
  ...
  case MCT_EVENT_MODULE_BUF_DIVERT: {
    ...
    if(stats_buff->stats_type != HDR_STATS){
      /* Only AEC module handles VHDR buffer */
      break;
    }
  ...
  case MCT_EVENT_MODULE_SET_CHROMATIX_PTR: {
    ...
    if (aec_msg != NULL ) {
      ...
      aec_msg->u.aec_set_parm.u.init_param.adrc_chromatix =
        mod_chrom->chromatixAdrcPtr;
      ...
      property_get("persist.camera.adrc",value, "0");
      boolean adrc_enable = atoi(value) > 0 ? TRUE : FALSE;
      aec_tuning_params->adrc_enable = adrc_enable;
  ...
  case MCT_EVENT_MODULE_STREAM_CROP: {
    ...
    if (aec_msg != NULL ) {
      ...
      CDBG("vfe map: (%d %d %d %d) vfe_out: (%d %d)", stream_crop->x_map,
        ...
        stream_crop->dim_width, stream_crop->dim_height);
      rc = q3a_aecawb_thread_en_q_msg(private->thread_data, aec_msg);
    }
  }
    break;
  ...
  case MCT_EVENT_MODULE_ISP_ADRC_MODULE_MASK: {
    uint32_t isp_module_mask =
      *(uint32_t*)event->u.module_event.module_event_data;

    q3a_thread_aecawb_msg_t *aec_msg = aec_port_malloc_msg(MSG_AEC_SET,
      AEC_SET_PARAM_ADRC_ISP_MODULE_MASK);
    if (!aec_msg) {
      CDBG_ERROR("malloc failed for aec_msg");
      break;
    }
    if (isp_module_mask & (1 << ISP_ADRC_MOD_LA))
      aec_msg->u.aec_set_parm.u.adrc_isp_module_mask |= AEC_ADRC_ISP_MOD_LA;
    if (isp_module_mask & (1 << ISP_ADRC_MOD_LTM))
      aec_msg->u.aec_set_parm.u.adrc_isp_module_mask |= AEC_ADRC_ISP_MOD_LTM;
    if (isp_module_mask & (1 << ISP_ADRC_MOD_GTM))
      aec_msg->u.aec_set_parm.u.adrc_isp_module_mask |= AEC_ADRC_ISP_MOD_GTM;

    rc = q3a_aecawb_thread_en_q_msg(private->thread_data, aec_msg);
  }
    break;

  case MCT_EVENT_MODULE_ISP_ADRC_SPL_EFFECT: {
    boolean spl_effect_enable =
      (*(boolean*)event->u.module_event.module_event_data);

    q3a_thread_aecawb_msg_t *aec_msg = aec_port_malloc_msg(MSG_AEC_SET,
      AEC_SET_PARAM_ADRC_ISP_MODULE_AVB);
    if (!aec_msg) {
      CDBG_ERROR("malloc failed for aec_msg");
      break;
    }
    /* when spl effect enabled, isp is not available for adrc, need to disable it. */
    aec_msg->u.aec_set_parm.u.adrc_isp_module_available = !spl_effect_enable;
