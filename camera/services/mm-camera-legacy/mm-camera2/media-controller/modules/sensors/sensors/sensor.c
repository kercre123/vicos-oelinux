...
int32_t sensor_probe(int32_t fd, const char *sensor_name)
{
  ...
  if (sensor_name)
    if (strlen(sensor_name) <  MAX_SENSOR_NAME)
      strlcpy(slave_info->sensor_name, sensor_name, strlen(sensor_name) + 1);
...
static int8_t sensor_set_hdr_ae_bracket(void *sctrl, void *data)
{
  ...
  memset(&(ctrl->s_data->ae_bracket_info), 0,
    sizeof(sensor_ae_bracket_info_t));
...
static int8_t sensor_set_aec_init_settings(void *sctrl, void *data)
{
  ...
  if ((ctrl->s_data->cur_stream_mask & (1 << CAM_STREAM_TYPE_SNAPSHOT)) &&
    ...
    for (i=0; i < valid_entries; i++) {
      SLOW("g%d=%f, lc%d=%d", i,
        aec_data->sensor_gain[i], i, aec_data->linecount[i]);
...
static int32_t sensor_set_vfe_sof(void *sctrl, void *data)
{
  ...
  if (ctrl->s_data->ae_bracket_info.ae_bracket_config.mode ==
    ...
    if (lib->sensor_lib_ptr->sync_exp_gain &&
      ctrl->s_data->stored_gain != ctrl->s_data->prev_gain) {
      sensor_apply_exposure(sctrl);
    }
...
static int8_t sensor_set_frame_rate(void *sctrl, void *data)
{
  ...
  if ((ctrl->s_data->cur_stream_mask & (1 << CAM_STREAM_TYPE_PREVIEW) ||
    ...
    if ((ctrl->s_data->cur_stream_mask & (1 << CAM_STREAM_TYPE_RAW)) &&
         !ctrl->s_data->raw_burst_enable &&
        !(ctrl->s_data->cur_stream_mask & (1 << CAM_STREAM_TYPE_PREVIEW) ||
          ctrl->s_data->cur_stream_mask & (1 << CAM_STREAM_TYPE_VIDEO)) ) {
      /*if current gain or linecount is 0, we need set a default one*/
      if (ctrl->s_data->current_linecount == 0)
        ctrl->s_data->current_linecount = 500;
      if (ctrl->s_data->current_gain == 0)
        ctrl->s_data->current_gain = 16;
      SHIGH("%s: force apply exposure in RDI streaming, linecnt %d, gain %d, fps_div %d",
        __func__, ctrl->s_data->current_linecount,
        ctrl->s_data->current_gain, ctrl->s_data->current_fps_div);
      sensor_apply_exposure(sctrl);
    } else {
...
/*==========================================================
 * FUNCTION    - sensor_set_raw_exposure_manual
 *
 * DESCRIPTION:
 *==========================================================*/

static int32_t sensor_set_raw_manual_exposure(void * sctrl , void *data)
{
  sensor_ctrl_t *ctrl = (sensor_ctrl_t *)sctrl;
  sensor_exp_t exposure;
  ...
  if (ctrl->s_data->cur_stream_mask & (1 << CAM_STREAM_TYPE_RAW)) {
    manual_exp = (cam_manual_exposure_t *)data;
    exposure.luma_hdr = 0;
    exposure.fgain_hdr = 0;
    exposure.real_gain = manual_exp->gain;
    exposure.linecount = manual_exp->linecnt;

    SHIGH("%s: manual exposure gain %f, linecnt %d", __func__,
      exposure.real_gain, exposure.linecount);
    sensor_set_exposure(sctrl, exposure);
    sensor_apply_exposure(sctrl); /* apply exposure */
  }
...
static int32_t sensor_get_cur_chromatix_name(void *sctrl, void *data)
{
  ...
  if (ctrl->lib_params->sensor_lib_ptr->chromatix_array) {
    ...
    sensor_get->chromatix_name.adrc_chromatix =
      chromatix_array->sensor_lib_chromatix[cur_res].adrc_chromatix;
...
static int32_t sensor_get_capabilities(void *slib, void *data)
{
  ...
  for (i = 0; i < size; i++) {
    ...
    sensor_cap->exposure_ctrl_info.res_info[i].dim.width = sensor_cap->dim_fps_table[i].dim.width;
    sensor_cap->exposure_ctrl_info.res_info[i].dim.height = sensor_cap->dim_fps_table[i].dim.height;
    sensor_cap->exposure_ctrl_info.res_info[i].frame_length_lines = out_info[i].frame_length_lines;
    sensor_cap->exposure_ctrl_info.res_info[i].max_fps = out_info[i].max_fps;
  }
  sensor_cap->exposure_ctrl_info.res_num = size;
  sensor_cap->exposure_ctrl_info.frame_length_lines_offset = lib->sensor_lib_ptr->exp_gain_info->vert_offset;
  sensor_cap->exposure_ctrl_info.max_frame_length_lines = lib->sensor_lib_ptr->aec_info->max_linecount;
  sensor_cap->exposure_ctrl_info.min_gain = 1.0f;
  sensor_cap->exposure_ctrl_info.max_gain = lib->sensor_lib_ptr->aec_info->max_gain;
...
static int32_t sensor_get_resolution_info(void *sctrl, void *data)
{
  ...
  property_get("debug.camera.pdaf.enable", value, "1");
  enabled = atoi(value);
  ...
  if (sensor_out_info->meta_cfg.num_meta != 0) {
    sensor_out_info->parse_RDI_statistics.parse_VHDR_stats_callback =
      (void *)(lib->sensor_lib_ptr->parse_RDI_stats->parse_VHDR_stats);
  }
  ...
  /*Fill sensor postion: back/front*/
  sensor_out_info->position =
    lib->sensor_lib_ptr->sensor_init_params->position;
 ...
 * DESCRIPTION:
 *==========================================================*/
static int32_t sensor_set_raw_burst_enable(void *sctrl, void *data)
{
  sensor_ctrl_t *ctrl = (sensor_ctrl_t *)sctrl;
  int32_t *raw_burst_enable = (int32_t *)data;
  SLOW("raw burst enable %d", *raw_burst_enable);
  ctrl->s_data->raw_burst_enable = *raw_burst_enable;
  return SENSOR_SUCCESS;
}
...
{
  sensor_ctrl_t             *ctrl;
  sensor_pdaf_params_t      *s_pdaf;
  sensor_lib_params_t       *lib;
  int32_t                   rc = 0;
  uint32_t (*sensor_init_pdaf)(void*) = NULL;
  ...
  ctrl = (sensor_ctrl_t *)sctrl;
  lib = (sensor_lib_params_t *)ctrl->lib_params;
  if (lib->sensor_lib_ptr->parse_RDI_stats)
  {
    if (lib->sensor_lib_ptr->parse_RDI_stats->parse_PDAF_stats)
      rc = lib->sensor_lib_ptr->parse_RDI_stats->parse_PDAF_stats(NULL, data);
    else
      SLOW("No PD_stats parser valid");
  }
  else{
    SLOW("No RDI_stats parser valid");
  }
  SLOW("Exit");
  return rc;
}

static int32_t sensor_pdaf_init(void *sctrl, void *data){
  sensor_ctrl_t             *ctrl;
  ...
  ctrl->s_data->pdaf_mode = FALSE;
  *(void **)&sensor_init_pdaf = dlsym(lib->sensor_lib_handle,
    lib->sensor_lib_ptr->sensorlib_pdaf_api.init);
  if (!sensor_init_pdaf) {
    SERR("failed to open pdaf3 init library function");
    ctrl->s_data->pdaf_mode = FALSE;
  }
  else {
    rc = sensor_init_pdaf(s_pdaf);
    if (rc < 0) {
      SERR("PDAF Init Failed");
      ctrl->s_data->pdaf_mode = FALSE;
    }
    ctrl->s_data->pdaf_mode = TRUE;
  }
  SLOW("Exit");
  return rc;
}

static int32_t sensor_pdaf_deinit(void *sctrl, void *data){
  sensor_ctrl_t                 *ctrl;
  sensor_lib_params_t           *lib;
  int32_t                       rc = 0;
  uint32_t (*sensor_deinit_pdaf)(void) = NULL;
  ...
  *(void **)&sensor_deinit_pdaf = dlsym(lib->sensor_lib_handle,
    lib->sensor_lib_ptr->sensorlib_pdaf_api.deinit);
  if (!sensor_deinit_pdaf) {
    SERR("failed to open pdaf3 init library function");
  }
  else {
    rc = sensor_deinit_pdaf();
    if (rc < 0) {
      SERR("PDAF deInit Failed");
    }
  }
  ctrl->s_data->pdaf_mode = FALSE;
...
static int32_t sensor_pdaf_cal_defocus(void *sctrl, void *data)
{
  ...
  sensor_lib_params_t       *lib;
  int32_t                   rc = 0;
  uint32_t (*sensor_pdaf_calc_defocus)(void*, void*,
  ...
  ctrl = (sensor_ctrl_t *)sctrl;
  lib = (sensor_lib_params_t *)ctrl->lib_params;
  s_pdaf = (sensor_pdaf_params_t *)data;
  ...
  *(void **)&sensor_pdaf_calc_defocus = dlsym(lib->sensor_lib_handle,
  lib->sensor_lib_ptr->sensorlib_pdaf_api.calcdefocus);

  if(sensor_pdaf_calc_defocus)
  {
    rc = sensor_pdaf_calc_defocus(NULL, (void *)s_pdaf,
      lib->sensor_custom_API.pdlib_get_defocus);

    if(rc < 0)
      return SENSOR_FAILURE;
  }
  else {
    SERR("Calculate defocus not defined in sensor lib");
    return SENSOR_FAILURE;
  }
  ...
  sensor_submodule_event_type_t event, void *data)
{
  ...
  case SENSOR_SET_DIS_ENABLE:
    ...
    break;
  case SENSOR_SET_RAW_BURST_ENABLE:
    rc = sensor_set_raw_burst_enable(sctrl, data);
    break;
  ...
  case SENSOR_SET_ISO:
    ...
    break;
  case SENSOR_SET_RAW_MANUAL_EXPOSURE:
    rc = sensor_set_raw_manual_exposure(sctrl,data);
    break;
  ...
  case SENSOR_PDAF_INIT:
    rc = sensor_pdaf_init(sctrl, data);
    break;
  case SENSOR_PDAF_DEINIT:
    rc = sensor_pdaf_deinit(sctrl, data);
    break;
  default:
