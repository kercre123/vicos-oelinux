...
typedef enum {
  ...
  SENSOR_SET_RAW_BURST_ENABLE, /* int32_t * */
  ...
  SENSOR_SET_ISO,
  SENSOR_SET_RAW_MANUAL_EXPOSURE,
...
typedef struct {
  uint16_t XKnotNumSlopeOffset;
  uint16_t YKnotNumSlopeOffset;
  float  SlopeData[MAX_PDAF_KNOT_X*MAX_PDAF_KNOT_Y];
  float  OffsetData[MAX_PDAF_KNOT_X*MAX_PDAF_KNOT_Y];
  uint16_t XAddressKnotSlopeOffset[MAX_PDAF_KNOT_X];
  uint16_t YAddressKnotSlopeOffset[MAX_PDAF_KNOT_Y];
} pdafcalib_data_t;

enum pdaf_isp_type {
  ISP_PDAF_NONE,
  ISP_RDI_STATS,
  ISP_CAMIF_SUB_SAMPLE,
  MAX_ISP_PDAF_MODES,
};
...
typedef struct {
  ...
  pdaf_calibration_param_t        pdafc_1d;
  pdaf_2D_calibration_param_t     pdafc_2d;
...
typedef struct {
  ...
  /*RDI manual exposure control*/
  int32_t                        raw_manual_linecount;
  float                          raw_manual_gain;
