    ...
    uint32_t future_frame_id, sensor_frame_ctrl_type_t type, void *data)
{
  ...
  case SENSOR_FRAME_CTRL_EXP_META: {
    ...
    frame_ctrl_data->u.sensor_exp.sensor_gain =
    ...
    case SENSOR_FRAME_CTRL_EXP_META:
      ...
      if (stats_get) {
        stats_get->aec_get.sensor_gain[0] =
          sensor_frame_ctrl_data->u.sensor_exp.sensor_gain;
  ...
  sensor_bundle_info_t *bundle_info, mct_event_module_t *event_module)
{
  ...
  if (stats_update->flag & STATS_UPDATE_AEC) {
    ...
    if (stats_update->aec_update.est_state == AEC_EST_NO_LED_DONE&&bundle_info->s_bundle->state!=3)
    {
      ...
      bundle_info->s_bundle->state != 2 && bundle_info->s_bundle->torch_on == 0)
    {
    ...
    if (sizeof(stats_get_data_t) >
      ...
      if (stats_get &&
        ...
        SHIGH("g %f lux idx %f", stats_update->aec_update.real_gain,
          stats_update->aec_update.lux_idx);
        stats_get->aec_get.sensor_gain[0] = stats_update->aec_update.sensor_gain;
        stats_get->aec_get.real_gain[0] = stats_update->aec_update.real_gain;
        stats_get->aec_get.lux_idx = stats_update->aec_update.lux_idx;
  ...
  mct_port_t *port, mct_event_t *event, sensor_bundle_info_t *bundle_info)
{
  ...
  case MCT_EVENTS_MODULE_PDAF_ISP_INFO:{
    ...
    isp_pdaf_camif_info *isp_camif =
     (isp_pdaf_camif_info *)event_module->module_event_data;
    ...
    memset(&s_pdaf, 0, sizeof(s_pdaf));
    s_pdaf.eeprom_data = bundle_info->s_bundle->formatted_data;
    memcpy(&s_pdaf.isp_camif, isp_camif, sizeof(s_pdaf.isp_camif));
  ...
  /* same inteface for handling MCT
     parse PD status
     call defocus API
  */
  case MCT_EVENT_MODULE_STATS_PDAF_UPDATE: {
    ...
    memset(&s_pdaf, 0, sizeof(sensor_pdaf_params_t));

    pdaf_update = &stats_update->pdaf_update;
    ...
    /* parse PD stats
       for type 3 : there is no stats parse
    */
    SENSOR_SUB_MODULE_PROCESS_EVENT(bundle_info->s_bundle, SUB_MODULE_SENSOR,
  ...
  if (event->type == MCT_EVENT_MODULE_EVENT) {
    ...
    if (event->direction == MCT_EVENT_DOWNSTREAM) {
      ...
      ret = port_sensor_handle_upstream_module_event(module, port, event,
        &bundle_info);
      if (ret == FALSE) {
        SERR("event_name %d", event_module->type);
...
{
  #define SERR1(fmt, args...) fprintf(stderr, fmt, ##args)
