...
/** module_sensor_subdeinit: sensor sub module de-init function
 *
 *  @data: sensor bundle data for first sensor
 *  @user_data: sub module name to be removed from s_bundle
 *
 *  Return: 0 for success and negative error for failure
 *
 *  This function deallocates memory to hold module_sensor_prams_t struct
 *  for given sub module **/

static boolean module_sensor_subdeinit(void *data, void *user_data)
{
  int32_t rc = SENSOR_SUCCESS, i = 0;
  int32_t *sub_module = (int32_t *)user_data;
  module_sensor_bundle_info_t *s_bundle = (module_sensor_bundle_info_t *)data;
  if (!s_bundle) {
    SERR("failed");
    return FALSE;
  }
  if ((*sub_module >= SUB_MODULE_MAX) || (*sub_module < 0)) {
    SERR("failed");
    return FALSE;
  }
  i = *sub_module;
    ...
    module_sensor_bundle_info_t *s_bundle)
{
    ...
    if (s_bundle->sensor_info->subdev_id[SUB_MODULE_EEPROM] != -1)
    {
        ...
        /* calculate actuator sensitivity
...
/* need to check whether to set the data loaded from kernel
  ...
  if (s_bundle->sensor_info->subdev_id[SUB_MODULE_EEPROM] != -1) {
    ...
    SENSOR_SUB_MODULE_PROCESS_EVENT(s_bundle, SUB_MODULE_EEPROM,
      EEPROM_GET_FORMATTED_DATA, &formatted_data, rc);
    s_bundle->formatted_data = formatted_data;
  }
  ...
  mct_event_t *event, module_sensor_bundle_info_t *s_bundle)
{
  ...
  /* Check whether this is burst raw stream ON */
  if (stream_info->stream_type == CAM_STREAM_TYPE_RAW) {
      int32_t burst_enable =
        (stream_info->streaming_mode == CAM_STREAMING_MODE_BURST);
      rc = module_sensor_params->func_tbl.process(
                module_sensor_params->sub_module_private,
                SENSOR_SET_RAW_BURST_ENABLE, &burst_enable);
      if (rc < 0) {
        SERR("failed");
        return FALSE;
      }
  ...
  for (i = 0; i < res_cfg_table->size; i++) {
    ...
    case SENSOR_SET_START_STREAM: {
      ...
      } else if((stream_info->stream_type == CAM_STREAM_TYPE_RAW) &&
                !(s_bundle->stream_mask & (1 << CAM_STREAM_TYPE_PREVIEW) ||
                  s_bundle->stream_mask & (1 << CAM_STREAM_TYPE_VIDEO))) {
        /*For RDI only streaming, aec is not running,
          so try to get it from saved exposure init setting.
          if not set yet, then force setting default ones*/
        uint32_t init_linecnt;
        float init_gain;
        aec_get_t raw_aec_get;

        memset(&raw_aec_get, 0, sizeof(aec_get_t));

        init_linecnt = (s_bundle->raw_manual_linecount != 0) ? s_bundle->raw_manual_linecount : 500;
        init_gain = ((uint32_t)(s_bundle->raw_manual_gain * 256) > 0) ? s_bundle->raw_manual_gain : 1.0;

        raw_aec_get.valid_entries = 1;
        raw_aec_get.sensor_gain[0] = init_gain;
        raw_aec_get.linecount[0] = init_linecnt;

        SHIGH("Set RAW Exposure init setting, linecnt %d, gain %f", init_linecnt, init_gain);
        /* set initial exposure settings, before stream_on */
        rc = module_sensor_params->func_tbl.process(
                module_sensor_params->sub_module_private,
                SENSOR_SET_AEC_INIT_SETTINGS, (void*)(&raw_aec_get));
        if (rc < 0) {
          SERR("failed");
          return FALSE;
        }
    ...
    case SENSOR_LOAD_CHROMATIX: {
      ...
      SLOW("adrc chromatix: %s", sensor_get.chromatix_name.adrc_chromatix);
      SERR("chromatix name %s", sensor_get.chromatix_name.chromatix);
      ...
      if (s_bundle->sensor_info->subdev_id[SUB_MODULE_EEPROM] != -1) {
        /* downstream pdaf info inside of eeprom
           if there is pdaf pattern inside of eeprom module
           we ingore whether or not there is any actual eeprom hardware
         */
        int is_pdaf3_sensor = 0;
        rc = eeprom_module_params->func_tbl.process(
          eeprom_module_params->sub_module_private,
          EEPROM_GET_ISPDAF_SENSOR, &is_pdaf3_sensor);
        if (rc < 0) {
          SERR("EEPROM_GET_ISPDAF_SENSOR failed");
        }
        if( is_pdaf3_sensor ){
         rc = sensor_util_post_downstream_event(module, event->identity,
         MCT_EVENT_MODULE_SENSOR_PDAF_CONFIG, &(s_bundle->formatted_data->pdafInfo));
         if (rc == FALSE) {
           SERR("failed sending downstream even in pdaf type 3");
         }
        }
      ...
      module_chromatix.chromatixAdrcPtr = chromatix_params.adrc_chromatix_ptr;
   ...
   sensor_bundle_info_t *bundle)
{
  ...
  case CAM_INTF_PARM_RAW_MANUAL_EXPOSURE: {
    ...
    /*save raw manual exposure to s_bundle*/
    manual_exp = (cam_manual_exposure_t *)(event_control->parm_data);
    s_bundle->raw_manual_linecount = manual_exp->linecnt;
    s_bundle->raw_manual_gain = manual_exp->gain;
    SHIGH("save RAW_MANUAL_EXPOSURE: linecnt %d, gain %f",
        s_bundle->raw_manual_linecount,
        s_bundle->raw_manual_gain);
  }
  break;
  ...
  mct_event_t *event)
{
  ...
  case MCT_EVENT_CONTROL_STREAMOFF: {
    ...
    if (TRUE == module_sensor_is_ready_for_stream_off(module, event,
      ...
      if (rc < 0)
      {
        SERR("%s deinit pdaf fails",
             bundle_info.s_bundle->sensor_info->sensor_name);
      }
...
static boolean module_sensor_init_eeprom(void *data, void *user_data) {
    ...
    rc = TRUE;
    rc1 = func_tbl.process(s_bundle->eeprom_data, EEPROM_READ_DATA,
      NULL);
    if (rc1 < 0) {
        SERR("Failed EEPROM_READ_DATA");
        rc = FALSE;
    }
    rc2 = func_tbl.process(s_bundle->eeprom_data, EEPROM_CLOSE_FD,
      NULL);
    if (rc2 < 0) {
...
mct_module_t *module_sensor_init(const char *name)
{
  ...
  module_sensor_ctrl_t        *module_ctrl = NULL;
  enum sensor_sub_module_t    s_module_idx;
  ...
  ret = mct_list_traverse(module_ctrl->sensor_bundle, module_sensor_init_eeprom,
    s_module);
  if (ret == FALSE) {
    SERR("EEPROM init failed. So, remove EEPROM subdev from sensor module\n");
    s_module_idx = SUB_MODULE_EEPROM;
    mct_list_traverse(module_ctrl->sensor_bundle, module_sensor_subdeinit,
      (void *)&s_module_idx);
  }
