...
int isp_util_read_reg_val(isp_hw_t *isp_hw,void *dump_entry, uint32_t read_type, uint32_t read_len,
                        uint32_t reg_offset) {
  int rc = 0;
...
int isp_util_set_hdr_param(isp_t *isp, uint32_t session_id,
  cam_exp_bracketing_t *parm_data)
{
  int rc = 0;
  isp_session_t *session = isp_util_find_session(isp, session_id);
  uint32_t ae_bracketing_enable = FALSE;
  if (!session) {
    return -1;
  }
  cam_exp_bracketing_t *ae_bracket_config =(cam_exp_bracketing_t*)parm_data;

  if (ae_bracket_config->mode == CAM_EXP_BRACKETING_ON)
      ae_bracketing_enable = TRUE;

  if (session->vfe_ids & (1 << VFE0)) {
    if (isp->data.hw[VFE0].hw_ops) {
      rc = isp->data.hw[VFE0].hw_ops->set_params(
        isp->data.hw[VFE0].hw_ops->ctrl,
        ISP_HW_SET_PARAM_HDR,
        (void *)&ae_bracketing_enable, sizeof(ae_bracketing_enable));
    }
  }
  if (session->vfe_ids & (1 << VFE1)) {
    if (isp->data.hw[VFE1].hw_ops) {
  ...
  uint32_t session_id, uint32_t stream_id, int32_t *effect)
{
  ...
  // Update 3A about the special effect mode.
  if ( (*effect == CAM_EFFECT_MODE_EMBOSS) ||
...
int isp_util_set_adrc_hw_module_id_mask(isp_t *isp,uint32_t session_id,uint32_t stream_id)
{
  isp_stream_t *stream;
  mct_event_t mct_event;
  isp_hw_t *isp_hw;
  ...
  /* read register for HW module and module config and set the id_mask. */
  isp_pipeline_t *pipeline = isp_hw->pipeline.private_data;
  isp_adrc_hw_module.isp_adrc_hw_module_id_mask =
    pipeline->dep.util_adrc_hw_module_id_mask(isp_hw,session_id);
...
int isp_util_set_adrc_spl_effect(isp_t *isp,uint32_t session_id,uint32_t stream_id, boolean enable)
{
  isp_stream_t *stream;
  mct_event_t mct_event;
  isp_hw_t *isp_hw;
  ...
  memset(&mct_event, 0, sizeof(mct_event));
  mct_event.u.module_event.type = MCT_EVENT_MODULE_ISP_ADRC_SPL_EFFECT;
  mct_event.u.module_event.module_event_data = (void *)&enable;
  mct_event.type = MCT_EVENT_MODULE_EVENT;
  mct_event.identity = pack_identity(session_id, stream_id);
