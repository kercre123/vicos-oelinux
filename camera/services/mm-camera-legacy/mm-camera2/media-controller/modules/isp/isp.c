...
static int isp_add_meta_channel(isp_t *isp, isp_session_t *session,
  isp_stream_t *stream);
static int isp_add_camif_channel(isp_t *isp, isp_session_t *session,
  isp_stream_t *stream);
...
static int isp_create_channels_for_streamon(isp_t *isp, isp_session_t *session)
{
  ...
  for (i = 0; i < ISP_MAX_STREAMS; i++) {
    ...
    property_get("persist.camera.pdaf_snapshot", value, "0");
    enabled = atoi(value);
    if (enabled) {
      if (stream->stream_info.stream_type == CAM_STREAM_TYPE_SNAPSHOT) {
        rc = isp_add_camif_channel(isp, session, stream);
        if (rc < 0) {
          CDBG_ERROR("%s: error, cannot add camif channel, identity = 0x%X\n",
            __func__, stream->stream_info.identity);
          return rc;
        }
      }
  ...
  isp_session_t *session)
{
  ...
  /* Provide information to stats module on support ADRC HW module */
  isp_util_set_adrc_hw_module_id_mask(isp, session_id, stream_id);
...
*  Returns 0 for success and negative error for failure
**/
int isp_send_pdaf_streamoff_to_3a(isp_t *isp, isp_session_t *session,
  isp_stream_t *stream)
{
  int rc;
  ...
  mct_event.u.module_event.type = MCT_EVENT_MODULE_ISP_PDAF_STREAMOFF;
  mct_event.u.module_event.module_event_data = 0;
  ...
  mct_port_send_event_to_peer(stream->src_ports[ISP_SRC_PORT_3A], &mct_event);
  ISP_DBG(ISP_MOD_COM,"%s: X", __func__);
  return rc;
}
  ...
  isp_session_t *session)
{
  ...
  if (rc >= 0) {
    ...
    for (i = 0; i < num_streams; i++) {
      ...
      pthread_mutex_unlock(
        &isp->data.session_critical_section[session->session_idx]);
      if ((stream->stream_info.stream_type == CAM_STREAM_TYPE_PREVIEW) &&
        (stream->cfg.sensor_cfg.is_pdaf_supported ==TRUE)) {
        /*Ask 3A to release PDAF buffer*/
        rc = isp_send_pdaf_streamoff_to_3a(isp, session, stream);
      }
    }
  } else {
  ...
  uint32_t session_id, uint32_t stream_id, void *data)
{
  ...
  if (stats_get->flag & STATS_UPDATE_AEC) {
    ...
    session->saved_params.aec_stats_update.lux_idx =
      stats_get->aec_get.lux_idx;
    session->saved_params.aec_stats_update.gamma_gain =
      stats_get->aec_get.gamma_gain;
    session->saved_params.aec_stats_update.la_gain =
      stats_get->aec_get.la_gain;
    session->saved_params.aec_stats_update.ltm_gain =
      stats_get->aec_get.ltm_gain;
    session->saved_params.aec_stats_update.gtm_gain =
      stats_get->aec_get.gtm_gain;
  }
  ...
  uint32_t session_id, uint32_t stream_id, void *chromatix)
{
  ...
  session->chromatix.chromatixAdrcPtr = chromatix_param->chromatixAdrcPtr;
  if (session->active_count > 0) {
    rc = isp_ch_util_set_param(isp, session, stream_id,
...
/** isp_pdaf_channel_config
*    @isp: isp ctrl instance
*    @session_id: current senssion object
*    @stream_id: isp stream object
*    @pdaf_config: sensor pdaf information
*
*  This method saves pdaf info for camif raw stream
*
*  Returns 0 for success and negative error for failure
**/
int isp_pdaf_channel_config(isp_t *isp, uint32_t session_id,
  uint32_t stream_id, pdaf_info_t *pdaf_config)
{
  isp_session_t *session = NULL;
  isp_stream_t *stream = NULL;
  isp_bundle_pdaf_info_t *request_pdaf_config = NULL;
  ...
  request_pdaf_config = &session->bundle_pdaf_info;
  request_pdaf_config->user_stream_id = stream_id;
  request_pdaf_config->pdaf_config = *pdaf_config;
  return 0;
}

boolean isp_config_camif_raw_size(isp_bundle_pdaf_info_t *bundle_pdaf_info,
  sensor_out_info_t *sensor_out_info)
{
  ...
  uint32_t residual_height;
  int i = 0, j = 0;
  pdaf_info_t *pdaf_config;

  pdaf_config = &bundle_pdaf_info->pdaf_config;

  for (i = 0; i<pdaf_config->pdaf_pix_count; i++) {
    h_skip |=  (1<<(pdaf_config->pdaf_pix_coords[i].x % 16));
    v_skip |=  (1<<(pdaf_config->pdaf_pix_coords[i].y % 16));
  }
  /*Do left circular shift considering camif_crop first_pixel and first line*/
  shift_x = 16 - sensor_out_info->request_crop.first_pixel % 16;
  shift_y = 16 - sensor_out_info->request_crop.first_line % 16;
  h_skip = ((h_skip >> shift_x) | (h_skip << (16 - shift_x))) & 0xFFFF;
  v_skip = ((v_skip >> shift_y) | (v_skip << (16 - shift_y))) & 0xFFFF;

  num_h_skip = (uint32_t)isp_find_number_skip(h_skip);
  num_v_skip = (uint32_t)isp_find_number_skip(v_skip);

  pdaf_config->block_dim.width = (sensor_out_info->dim_output.width / 16)
    * num_h_skip;
  pdaf_config->block_dim.height = (sensor_out_info->dim_output.height / 16)
    * num_v_skip;

  residual_width = sensor_out_info->dim_output.width % 16;
  residual_height = sensor_out_info->dim_output.height % 16;

  pdaf_config->block_dim.width += isp_find_number_skip(h_skip &
    ALL_ONES_16(residual_width));
  pdaf_config->block_dim.height += isp_find_number_skip(v_skip &
    ALL_ONES_16(residual_height));

  /*In HW the pixel subsampling pattern starts from the MSB and is applied
  to the first pixel in a line. Reverse the calculated skip pattern*/
  h_skip = isp_reverse_skip(h_skip);
  v_skip = isp_reverse_skip(v_skip);
  bundle_pdaf_info->pix_skip_pattern = h_skip;
  bundle_pdaf_info->line_skip_pattern = v_skip;
  return TRUE;
}

int isp_find_number_skip(uint16_t skip_pattern)
{
  int count = 0;
...
uint16_t isp_reverse_skip(uint16_t skip_pattern)
{
...
/** isp_modify_plane_info_for_native_buf
 *
 * DESCRIPTION:
 *
 **/
static int isp_modify_plane_info_for_native_buf(
  mct_stream_info_t *stream_info, cam_format_t fmt)
{
  ...
  case CAM_FORMAT_BAYER_MIPI_RAW_8BPP_GBRG:
    stream_info->fmt = CAM_FORMAT_BAYER_QCOM_RAW_8BPP_GBRG;
    break;
  case CAM_FORMAT_BAYER_MIPI_RAW_8BPP_GRBG:
    stream_info->fmt = CAM_FORMAT_BAYER_QCOM_RAW_8BPP_GRBG;
    break;
  case CAM_FORMAT_BAYER_MIPI_RAW_8BPP_RGGB:
    stream_info->fmt = CAM_FORMAT_BAYER_QCOM_RAW_8BPP_RGGB;
    break;
  case CAM_FORMAT_BAYER_MIPI_RAW_8BPP_BGGR:
    stream_info->fmt = CAM_FORMAT_BAYER_QCOM_RAW_8BPP_BGGR;
    break;
  case CAM_FORMAT_BAYER_MIPI_RAW_10BPP_GBRG:
    stream_info->fmt = CAM_FORMAT_BAYER_QCOM_RAW_10BPP_GRBG;
    break;
  case CAM_FORMAT_BAYER_MIPI_RAW_10BPP_GRBG:
    stream_info->fmt = CAM_FORMAT_BAYER_QCOM_RAW_10BPP_GRBG;
    break;
  case CAM_FORMAT_BAYER_MIPI_RAW_10BPP_RGGB:
    stream_info->fmt = CAM_FORMAT_BAYER_QCOM_RAW_10BPP_RGGB;
    break;
  case CAM_FORMAT_BAYER_MIPI_RAW_10BPP_BGGR:
    stream_info->fmt = CAM_FORMAT_BAYER_QCOM_RAW_10BPP_BGGR;
    break;
  case CAM_FORMAT_BAYER_MIPI_RAW_12BPP_GBRG:
    stream_info->fmt = CAM_FORMAT_BAYER_QCOM_RAW_12BPP_GBRG;
    break;
  case CAM_FORMAT_BAYER_MIPI_RAW_12BPP_GRBG:
    stream_info->fmt = CAM_FORMAT_BAYER_QCOM_RAW_12BPP_GRBG;
    break;
  case CAM_FORMAT_BAYER_MIPI_RAW_12BPP_RGGB:
    stream_info->fmt = CAM_FORMAT_BAYER_QCOM_RAW_12BPP_RGGB;
    break;
  case CAM_FORMAT_BAYER_MIPI_RAW_12BPP_BGGR:
    stream_info->fmt = CAM_FORMAT_BAYER_QCOM_RAW_12BPP_BGGR;
    break;
  ...
  case CAM_FORMAT_BAYER_QCOM_RAW_12BPP_BGGR:
    ...
    break;
  default:
    CDBG_ERROR("%s: Invalid raw format %d",
      __func__, fmt);
    rc = -1;
    break;
  }
  ...
  CDBG_ERROR("%s: input fmt %d wxh = %d x %d",
...
*  Returns 0 for success and negative error for failure
**/
int isp_send_pdaf_camif_info_to_sensor(isp_t *isp, isp_session_t *session,
  isp_stream_t *stream, isp_pdaf_camif_info *camif_info)
{
  int rc;
  mct_event_t mct_event;

  memset(&mct_event, 0, sizeof(mct_event));
  mct_event.u.module_event.type = MCT_EVENTS_MODULE_PDAF_ISP_INFO;
  mct_event.u.module_event.module_event_data = (void *)camif_info;
  mct_event.type = MCT_EVENT_MODULE_EVENT;
  mct_event.identity = pack_identity(session->session_id, stream->stream_id);
  mct_event.direction = MCT_EVENT_UPSTREAM;
  /* broadcast sof upstream */
...
/** isp_add_camif_channel
*    @isp: isp ctrl instance
*    @session: current senssion object
*    @stream: isp stream object
*
*  Adds isp camif channel
*
*  Returns 0 for success and negative error for failure
**/
static int isp_add_camif_channel(isp_t *isp, isp_session_t *session,
  isp_stream_t *stream)
{
  int rc = 0, i = 0, enabled = 0;
  isp_channel_t *camif_channel;
  uint32_t camif_channel_id;
  mct_stream_info_t stream_info;
  mct_stream_info_t camif_stream_info;
  isp_bundle_pdaf_info_t *bundle_pdaf_info;
  pdaf_info_t *pdaf_config;
  isp_pdaf_camif_info camif_info;
  sensor_out_info_t *sensor_out_info;
  char value[PROPERTY_VALUE_MAX];
  ISP_DBG(ISP_MOD_COM,"%s: E", __func__);

  stream_info = stream->stream_info;
  camif_channel_id = stream->stream_id | ISP_PDAF_CHANNEL_BIT;

  for (i =0; i < ISP_MAX_STREAMS; i++) {
    if(session->channel[i].channel_id & ISP_PDAF_CHANNEL_BIT) {
      CDBG_HIGH("%s: already has camif channel for session %d, nop\n",
        __func__, session->session_id);
      return 0;
    }
  }
  ...
  camif_channel = NULL;
  sensor_out_info = &stream->cfg.sensor_cfg;
  bundle_pdaf_info = &session->bundle_pdaf_info;
  pdaf_config = &bundle_pdaf_info->pdaf_config;

  if (!sensor_out_info->is_pdaf_supported) {
    CDBG_ERROR("%s: is_pdaf_supported %d\n", __func__,
  ...
  if ((stream->stream_id !=
    UNPACK_STREAM_ID(bundle_pdaf_info->user_stream_id)) &&
    (enabled == 0)) {
    CDBG_ERROR ("%s: Can't create PDAF channel Expected stream id =%d\
      current stream id %d\n",__func__,
      UNPACK_STREAM_ID(bundle_pdaf_info->user_stream_id),
      stream->stream_id);
    return 0;
  ...
  isp_config_camif_raw_size(bundle_pdaf_info, sensor_out_info);
  camif_stream_info.dim.height = pdaf_config->block_dim.height;
  camif_stream_info.dim.width = pdaf_config->block_dim.width;
  isp_modify_plane_info_for_native_buf(&camif_stream_info, sensor_out_info->fmt);
  camif_stream_info.num_burst = 0;
  camif_stream_info.stream_type = CAM_STREAM_TYPE_RAW;
  camif_stream_info.streaming_mode = CAM_STREAMING_MODE_CONTINUOUS;

  /* add camif chanenl onto session*/
  camif_channel = isp_ch_util_add_channel(isp, session->session_id,
    camif_channel_id, stream->stream_idx, &camif_stream_info,
    ISP_CHANNEL_TYPE_IMAGE);
  if (camif_channel == NULL) {
    CDBG_ERROR("%s: error, no empty slot for camif channel, identity = 0x%x\n",
    __func__, stream->stream_info.identity);
  return -1;
  }
  ...
  /*sync bundle stream config into channel,
  set natvie buffer and emta use output mask*/
  camif_channel->cfg.vfe_output_mask = stream->cfg.vfe_output_mask;
  camif_channel->cfg.ispif_out_cfg = stream->cfg.ispif_out_cfg;
  camif_channel->cfg.ispif_out_cfg.is_split	= FALSE;
  camif_channel->cfg.sensor_cfg = stream->cfg.sensor_cfg;
  camif_channel->cfg.vfe_mask = stream->cfg.vfe_mask;
  camif_channel->divert_to_3a = TRUE;
  camif_channel->ext_stats_type = PD_STATS;
  camif_channel->meta_info	 = stream->meta_info;
  camif_channel->sink_port = stream->sink_port;
  camif_channel->src_ports[ISP_SRC_PORT_DATA] =
    stream->src_ports[ISP_SRC_PORT_DATA];
  camif_channel->src_ports[ISP_SRC_PORT_3A] =
    stream->src_ports[ISP_SRC_PORT_3A];
  camif_channel->state = ISP_CHANNEL_STATE_USER_CFG;
  camif_channel->use_native_buf = 1;

  stream->channel_idx_mask |=
    (1 << isp_ch_util_get_channel_idx(camif_channel));

  /*fill in pdaf camif info*/
  camif_info.dual_vfe = FALSE;
  camif_info.image_width = stream->cfg.sensor_cfg.request_crop.last_pixel
    - stream->cfg.sensor_cfg.request_crop.first_pixel + 1;
  camif_info.image_height = stream->cfg.sensor_cfg.request_crop.last_line
    - stream->cfg.sensor_cfg.request_crop.first_line + 1;
  camif_info.H_global_offset = pdaf_config->swin_pdaf_offset_x;
  camif_info.V_global_offset = pdaf_config->swin_pdaf_offset_y;
  camif_info.left_buffer_width = pdaf_config->block_dim.width;
  camif_info.left_buffer_stride =
    (camif_stream_info.buf_planes.plane_info.mp[0].stride/12) * 16;
  camif_info.right_buffer_width = 0;
  camif_info.right_buffer_stride = 0;
  camif_info.buffer_height = pdaf_config->block_dim.height;
  camif_info.overlap = 0;
  camif_info.right_image_offset = 0;
  rc = isp_send_pdaf_camif_info_to_sensor(isp, session, stream,
    &camif_info);
  ...
  uint32_t session_id, uint32_t stream_id, mct_event_t *event)
{
  ...
  case CAM_INTF_PARM_HDR: {
    rc = isp_util_set_hdr_param(isp, session_id,
         param->parm_data);
  }
    break;
