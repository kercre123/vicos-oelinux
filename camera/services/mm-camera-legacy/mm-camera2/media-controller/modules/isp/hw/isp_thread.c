  ...
  isp_thread_t *thread_data, int *thread_exit, boolean use_pipe)
{
  ...
  case MM_ISP_CMD_ACTION: {
    ...
    case ISP_HW_ACTION_CODE_INVALID_QUEUE: {
      thread_data->q_invalid = TRUE;
      sem_post(&thread_data->sig_sem);
      sem_posted = TRUE;
      break;
    }

    case ISP_HW_ACTION_CODE_VALID_QUEUE: {
      thread_data->q_invalid = FALSE;
      sem_post(&thread_data->sig_sem);
      sem_posted = TRUE;
      break;
    }
  ...
  case MM_ISP_CMD_SOF_UPDATE: {
    thread_data->return_code = 0;
    thread_data->action_cmd = NULL;
    struct msm_isp32_event_data sof_parm;
    ...
    cfg_cmd.cfg_data = (void*) &hw_frame_id;
    cfg_cmd.cmd_len = sizeof(hw_frame_id);
    cfg_cmd.num_cfg = 1;
    reg_cfg_cmd[0].cmd_type = VFE_HW_UPDATE_LOCK;

    rc = ioctl(isp_hw->fd, VIDIOC_MSM_VFE_REG_CFG, &cfg_cmd);
    if (rc < 0) {
        CDBG_ERROR("%s: HW update still on going", __func__);
