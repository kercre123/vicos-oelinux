...
static int isp_hw_notify_buf_divert_event(isp_hw_t *isp_hw,
  struct msm_isp32_event_data *isp_event_data)
{
  int rc = 0;
...
static int isp_hw_input_cfg(isp_hw_t *isp_hw, isp_intf_type_t intf)
{
  ...
  if (intf == ISP_INTF_PIX) {
    ...
    pix_cfg->camif_cfg.subsample_cfg.pixel_skip = camif_cfg->pixel_skip;
    pix_cfg->camif_cfg.subsample_cfg.line_skip = camif_cfg->line_skip;
  ...
  isp_hw_stream_t *stream, isp_intf_type_t intf)
{
  ...
  if (intf == ISP_INTF_PIX) {
    ...
    /* fill the pdaf sensor info here*/
    isp_hw->input.camif_cfg.pixel_skip =
      stream->cfg.pixel_skip;
    isp_hw->input.camif_cfg.line_skip =
      stream->cfg.line_skip;
  ...
  isp_hw_session_t *session, isp_hw_stream_t *stream)
{
  ...
  /* cmaif/Camif input cfg need to sit here, input format need to set to kernel
     before VIDIOC_MSM_ISP32_REQUEST_STREAM (ISP_AXI_SET_STREAM_CFG)*/
 ...
 *
 **/
int isp_hw_proc_hw_update(void *ctrl, struct msm_isp32_event_data *sof)
{
  int rc = 0;
...
int isp_hw_proc_update_params_at_sof(isp_hw_t *isp_hw,
  struct msm_isp32_event_data *sof)
{
 ...
 *  Return 0 on success and negative error on failure
 **/
static int isp_sof_update(void *ctrl, struct msm_isp32_event_data *sof_parm)
{
  int len, rc = 0;
  isp_hw_t *isp_hw = ctrl;
...
void isp_hw_proc_subdev_event(isp_hw_t *isp_hw, isp_thread_t *thread_data)
{
  ...
  struct msm_isp32_event_data *isp_event_data = NULL;
  ...
  if (rc >= 0) {
    isp_event_data = (struct msm_isp32_event_data *)v4l2_event.u.data;
    ...
    case ISP_EVENT_STATS_NOTIFY:{
      ...
      if (isp_hw->pipeline.num_active_streams == 0 || thread_data->q_invalid)
        break;

      stats_event.raw_stats_event =
        (struct msm_isp32_event_data *)v4l2_event.u.data;
    ...
    case ISP_EVENT_REG_UPDATE: {
      ...
      isp_t *isp = (isp_t *)isp_hw->notify_ops->parent;
      uint32_t frame_id;
      struct msm_isp32_event_data *reg_update_sof =
        (struct msm_isp32_event_data *)v4l2_event.u.data;
      current_session =
    ...
    case ISP_EVENT_WM_BUS_OVERFLOW: {
      ...
      if (error_event->u.error_info.error_mask == (1 << ISP_WM_BUS_OVERFLOW)) {
        ...
        bus_msg.msg = NULL;
        bus_msg.size = 0;
        bus_msg.sessionid = isp_hw->pipeline.session_id[error_event->input_intf];
        if (TRUE != mct_module_post_bus_msg(isp->module, &bus_msg))
...
static int isp_hw_proc_set_hdr(isp_hw_t *isp_hw,
  uint32_t *ae_bracketing_enable, uint32_t in_params_size)
{
  int rc = 0;
  if (sizeof(ae_bracketing_enable) != in_params_size) {
    CDBG_ERROR("%s: size mismatch\n", __func__);
    return -1;
  }

  rc = isp_pipeline_set_params(isp_hw->pipeline.private_data,
    ISP_PIX_SET_HDR, (void *)ae_bracketing_enable, in_params_size);
  ...
  uint32_t in_params_size)
{
  ...
  case ISP_HW_SET_PARAM_HDR: {
    rc = isp_hw_proc_set_hdr(isp_hw, (uint32_t *)in_params,
      in_params_size);
  }
    break;
  default: {
      ...
      if(action_code == ISP_HW_ACTION_CODE_STREAM_START) {
         if (isp_hw->pipeline.num_active_streams == 0) {
           CDBG_HIGH("%s: first start, hw thread for streamon/ack\n",
             __func__);
           thread = &isp_hw->thread_poll;
           action.action_code = ISP_HW_ACTION_CODE_VALID_QUEUE;
           action.data = NULL;
           action.data_size = 0;
           rc = isp_hw_pipe_thread_action(ctrl, thread, &action);
        ...
        if (isp_hw->pipeline.num_active_streams <= cnt &&
          ...
          thread = &isp_hw->thread_poll;
          action.action_code = ISP_HW_ACTION_CODE_INVALID_QUEUE;
          action.data = NULL;
          action.data_size = 0;
          rc = isp_hw_pipe_thread_action(ctrl, thread, &action);
          thread = &isp_hw->thread_hw;
