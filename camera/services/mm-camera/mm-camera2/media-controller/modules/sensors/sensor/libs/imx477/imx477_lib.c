...
#include "imx477_raw10_lib.h"
#else
#include "imx477_raw12_lib.h"
#endif
  ...
  struct camera_i2c_reg_array reg_array[], int size, int res)
{
  ...
  p_regData->pd_offset_x_h = find_reg_data(reg_array, size, PD_AREA_X_OFFSET_ADDR);
  p_regData->pd_offset_x_l = find_reg_data(reg_array, size, PD_AREA_X_OFFSET_ADDR+1);
  p_regData->pd_offset_y_h = find_reg_data(reg_array, size, PD_AREA_Y_OFFSET_ADDR);
  p_regData->pd_offset_y_l = find_reg_data(reg_array, size, PD_AREA_Y_OFFSET_ADDR+1);
  p_regData->pd_area_w_h = find_reg_data(reg_array, size, PD_AREA_WIDTH_ADDR);
  p_regData->pd_area_w_l = find_reg_data(reg_array, size, PD_AREA_WIDTH_ADDR+1);
  p_regData->pd_area_h_h = find_reg_data(reg_array, size, PD_AREA_HEIGHT_ADDR);
  p_regData->pd_area_h_l = find_reg_data(reg_array, size, PD_AREA_HEIGHT_ADDR+1);

  for(i = 0; i < PD_FLEX_WIN_NUM*4*2; i++) {
    p_regData->reg_addr_flex_win[i] = find_reg_data(reg_array, size, FLEX_AREA_XSTA0_ADDR + i);
  }
...
int32_t imx477_pdaf_calculate_defocus
  (void * arg1, signed long (*get_defocus)(void *input, void *output))
{
  ...
  ALOGE("PDAF calculate defocus, cur_res:%d",s_pdaf->cur_res);
  ...
  ALOGE("PDAF AREA:x_offset:%d,y_offset:%d, w:%d, h:%d",
  ...
  for(j = 0;j < pd_win_y;j++)
    for(i = 0;i < pd_win_x;i++) {
     ...
     ALOGE("PDAF INPUT[%d][%d]: window: X_STA %d X_END %d, Y_STA %d Y_END %d",
       i, j,
      ...
      s_pdaf->defocus[i + pd_win_x*j].defocus = output.Defocus >> 14;
      s_pdaf->defocus[i + pd_win_x*j].df_confidence =
        output.DefocusConfidence;
      s_pdaf->defocus[i + pd_win_x*j].df_conf_level =
        output.DefocusConfidenceLevel;

      ALOGE("PDAF OUTPUT win:%d, defocus: %d, conf level: %d, "
        "phase diff: %f, DCC: %f",
...
/**
 * FUNCTION: imx477_real_to_register_gain
 *
 * DESCRIPTION: Calcuate sensor analog gain register value
 **/
static unsigned int imx477_real_to_register_gain(float real_gain)
{
  unsigned int reg_gain = 0;

  if (real_gain < IMX477_MIN_AGAIN) {
    real_gain = IMX477_MIN_AGAIN;
    SHIGH("IMX477 Requested gain(%f) is lower than the sensor minimum(%d)",
      real_gain, IMX477_MIN_AGAIN);
  }
  else if (real_gain > IMX477_MAX_AGAIN) {
    real_gain = IMX477_MAX_AGAIN;
  }
...
/**
 * FUNCTION: imx477_register_to_real_gain
 *
 * DESCRIPTION: Calcuate sensor real gain value
 **/
static float imx477_register_to_real_gain(unsigned int reg_gain)
{
  float gain;

  if(reg_gain > IMX477_MAX_AGAIN_REG_VAL)
    reg_gain = IMX477_MAX_AGAIN_REG_VAL;
...
/**
 * FUNCTION: imx477_digital_gain_calc
 *
 * DESCRIPTION: Calcuate the sensor digital gain
 **/
static unsigned int imx477_digital_gain_calc(
 float real_gain, float sensor_real_gain)
{
  unsigned int reg_dig_gain = IMX477_MIN_DGAIN_REG_VAL;
  float real_dig_gain = IMX477_MIN_DGAIN;

  if(real_gain > IMX477_MAX_AGAIN) {
    real_dig_gain = real_gain / sensor_real_gain;
  }
  else {
    real_dig_gain = IMX477_MIN_DGAIN;
  }

  if(real_dig_gain > IMX477_MAX_DGAIN) {
    real_dig_gain = IMX477_MAX_DGAIN;
  }
...
/**
 * FUNCTION: imx477_fill_exposure_array
 *
  ...
  __attribute__((unused)) int is_hdr_enabled)
{
  ...
  SLOW("sensor_fill_exposure_array, cur_res:%d",1);
  ...
  if(luma_delta != 0) {
    ...
    reg_setting->reg_setting[reg_count].reg_addr = IMX477_EXP_RATIO_ADDR;
  ...
  unsigned short awb_gain_b, struct camera_i2c_seq_reg_setting* reg_setting)
{
  ...
  reg_setting->reg_setting[reg_count].reg_addr = IMX477_ABS_GAIN_R_WORD_ADDR;
  ...
  reg_setting->reg_setting[reg_count].reg_addr = IMX477_ABS_GAIN_B_WORD_ADDR;
