/**
 * Copyright (c) 2008-2011, 2014, 2017 Qualcomm Technologies, Inc.
 * All Rights Reserved.
...
int main(int argc, char* argv[])
{
    ...
    for (i = 0; i < concurrent_cnt; i++)
    {
        ...
        os_mutex_init_jpg(&thread_ctrl_blks[i].output_handler_args.mutex);
        os_cond_init_jpg(&thread_ctrl_blks[i].output_handler_args.cond);
        if (os_thread_create_jpg(&thread_ctrl_blks[i].thread, encoder_test, &thread_ctrl_blks[i]))
        {
...
OS_THREAD_FUNC_RET_T OS_THREAD_FUNC_MODIFIER encoder_test(OS_THREAD_FUNC_ARG_T arg)
{
    ...
    for (i = 0; i < p_args->back_to_back_count; i++)
    {
        ...
        {
            os_mutex_lock_jpg(&p_thread_arg->output_handler_args.mutex);
            while (p_thread_arg->encoding)
            {
                rc = os_cond_timedwait_jpg(&p_thread_arg->output_handler_args.cond,
                ...
                if (rc == JPEGERR_ETIMEDOUT)
                {
                    ...
                    os_mutex_unlock_jpg(&p_thread_arg->output_handler_args.mutex);
        ...
        else
        {
            ...
            while (p_thread_arg->encoding)
            {
                os_cond_wait_jpg(&p_thread_arg->output_handler_args.cond,
                                  &p_thread_arg->output_handler_args.mutex);
            }
            os_mutex_unlock_jpg(&p_thread_arg->output_handler_args.mutex);
        }
                           ...
                           void        *p_arg)
{
    ...
    if (event == JPEG_EVENT_DONE || event == JPEG_EVENT_ERROR)
    {
        ...
        os_cond_signal_jpg(&p_thread_arg->output_handler_args.cond);
        os_mutex_unlock_jpg(&p_thread_arg->output_handler_args.mutex);
    }
}
                            ...
                            uint8_t         last_buf_flag)
{
    ...
    os_mutex_lock_jpg(&p_output_args->mutex);
    if (!p_output_args->fout)
    {
        fprintf(stderr, "encoder_output_handler: invalid p_arg\n");
        os_mutex_unlock_jpg(&p_output_args->mutex);
        return 1;
    }
