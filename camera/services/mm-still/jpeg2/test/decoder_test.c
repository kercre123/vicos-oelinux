/**
 * Copyright (C) 2008-2011, 2017 Qualcomm Technologies, Inc.
...
int main(int argc, char* argv[])
{
    ...
    for (i = 0; i < concurrent_cnt; i++)
    {
        ...
        os_mutex_init_jpg(&thread_ctrl_blks[i].mutex);
        os_cond_init_jpg(&thread_ctrl_blks[i].cond);
        if (os_thread_create_jpg(&thread_ctrl_blks[i].thread, decoder_test, &thread_ctrl_blks[i]))
        {
    ...
    // Dump exif tag information if requested
        ...
        if(1 == p_args->thumbnail_flag) {
            ...
            if(header.thumbnail.height != 0 && header.thumbnail.width != 0) {
                ...
                os_mutex_lock_jpg(&p_thread_arg->mutex);
                while(p_thread_arg->decoding) {
                    os_cond_wait_jpg(&p_thread_arg->cond, &p_thread_arg->mutex);
                }
                os_mutex_unlock_jpg(&p_thread_arg->mutex);
        ...
        if (p_args->abort_time) {
            ...
            while (p_thread_arg->decoding)
            {
                rc = os_cond_timedwait_jpg(&p_thread_arg->cond, &p_thread_arg->mutex, p_args->abort_time);
                           ...
                           void        *p_arg)
{
    ...
    if (event != JPEG_EVENT_WARNING)
    {
        ...
        os_cond_signal_jpg(&p_thread_arg->cond);
        os_mutex_unlock_jpg(&p_thread_arg->mutex);
    }
}
