    ...
    if (!p_engine_local->p_dest_buffer) {
        ...
        if (JPEG_FAILED(rc) || !p_engine_local->p_dest_buffer) {
            JPEG_DBG_ERROR("jpege_engine_hw_output_handler: failed to get next"
              "output buffer\n"); os_mutex_lock_jpg(&(p_engine_local->mutex));
            p_engine_local->error_flag = 1;
            os_mutex_unlock_jpg(&(p_engine_local->mutex));
  ...
  struct jpege_hw_buf *buf)
{
  ...
  if ((buf->type == JPEG_EVT_SESSION_DONE) &&
    ...
    os_mutex_lock_jpg(&p_engine_local->frame_done_mutex);
    ...
    os_cond_signal_jpg(&p_engine_local->frame_done_cond);
    os_mutex_unlock_jpg(&p_engine_local->frame_done_mutex);
    return 0;
  }
  ...
  jpege_engine_event_handler_t  p_handler)
{
  ...
  os_mutex_init_jpg(&(p_engine->mutex));                    // Initialize the mutex
  os_cond_init_jpg(&(p_engine->cond));                      // Initialize the condition variable
  /* Initialize the mutex */
  os_mutex_init_jpg(&(p_engine->frame_done_mutex));
  /* Initialize the condition variable */
  os_cond_init_jpg(&(p_engine->frame_done_cond));
  ...
  jpege_engine_dst_t   *p_dest)
{
  ...
  os_mutex_lock_jpg(&(p_engine->mutex));
  if (p_engine->is_active) {
    os_mutex_unlock_jpg(&(p_engine->mutex));
  ...
  rc = os_thread_create_jpg(&p_engine->thread, jpege_engine_hw_encode,
    (void *)p_engine); // If there is a failure in creating the thread, clean up and exit
  if (rc) {
    os_mutex_unlock_jpg(&p_engine->mutex);
    JPEG_DBG_HIGH("jpege_engine_sw_start: os_thread_create_jpg() failed: %d\n", rc);
    return JPEGERR_EFAILED;
  }
  ...
  jpege_engine_hw_encode(OS_THREAD_FUNC_ARG_T arg)
{
 ...
 jpege_cmd_scale_cfg scale_cfg;
  ...
  os_mutex_lock_jpg(&(p_engine->frame_done_mutex));
  if (!p_engine->output_done) {
    rc = os_cond_timedwait_jpg(&p_engine->frame_done_cond,
      &(p_engine->frame_done_mutex), 20000);
  }
  os_mutex_unlock_jpg(&(p_engine->frame_done_mutex));

  JPEG_DBG_MED ("%s:%d] waiting for all done... rc %d\n", __func__, __LINE__,
    rc);
  if (rc) {
    JPEG_DBG_ERROR("jpege_engine_hw_start: os_cond_timedwait_jpg failed\n");
    p_engine->error_flag = true;
    goto err;
  }
...
err:
  ...
  if (p_engine->abort_flag) {
    p_engine->abort_flag = false;
    os_cond_signal_jpg(&p_engine->cond);
    os_mutex_unlock_jpg(&(p_engine->mutex));
  } else {
    if (p_engine->error_flag) {
      p_engine->error_flag = false;
  ...
  jpege_engine_obj_t *p_obj)
{
  ...
  if (p_engine->is_active) {
    ...
    while (p_engine->abort_flag) {
      os_cond_wait_jpg(&(p_engine->cond), &(p_engine->mutex));
    }
  }
  os_mutex_unlock_jpg(&(p_engine->mutex));
  return JPEGERR_SUCCESS;
  ...
  if (p_obj) {
    ...
    if (p_engine) {
      ...
      JPEG_DBG_MED("jpege_engine_sw_destroy: join %d %d",
        p_engine->thread, os_thread_self_jpg());
      if (os_thread_self_jpg() != p_engine->thread) {
        os_thread_join_jpg(&p_engine->thread, NULL);
      }

      os_mutex_destroy_jpg(&(p_engine->mutex));
      os_cond_destroy_jpg(&(p_engine->cond));
