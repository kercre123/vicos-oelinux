/*******************************************************************************
 * Copyright (c) 2008-2012, 2017 Qualcomm Technologies, Inc.
 * All Rights Reserved.
    ...
    void                   *p_user_data)
{
    ...
    (void)os_mutex_init_jpg(&(p_decoder->mutex));
    (void)os_cond_init_jpg(&(p_decoder->cond));
    ...
    if (os_thread_create_jpg(&p_decoder->thread,
                         jpegd_handle_input_req_thread,
                         (void *)(p_decoder)))
    {
        jpeg_postprocessor_destroy(&(p_decoder->processor));

        os_mutex_destroy_jpg(&(p_decoder->mutex));
        os_cond_destroy_jpg(&(p_decoder->cond));
        JPEG_FREE(p_decoder);
    ...
    uint32_t            list_count)
{
    ...
    os_mutex_lock_jpg(&(p_decoder->processor.mutex));
    ...
    for (i = 0; i < list_count; i++)
    {
        ...
        if (p_decoder->processor.output_buffers[q_index].is_in_q)
        {
            ...
            os_mutex_unlock_jpg(&(p_decoder->processor.mutex));
            return JPEGERR_EFAILED;
        }
    ...
    os_cond_signal_jpg(&(p_decoder->processor.cond));
    os_mutex_unlock_jpg(&(p_decoder->processor.mutex));
...
int jpegd_abort(jpegd_obj_t obj)
{
    ...
    os_mutex_lock_jpg(&p_decoder->mutex);
    if (p_decoder->state != JPEGD_IDLE)
    {
        ...
        os_mutex_lock_jpg(&(p_decoder->processor.mutex));   // Lock the post processor
        p_decoder->processor.abort_flag = true;         // Set post processor abort flag
        os_cond_signal_jpg(&(p_decoder->processor.cond));   // Signal the post processor (abort)
        os_mutex_unlock_jpg(&(p_decoder->processor.mutex)); // Unlock post processor
    }
    else
        os_mutex_unlock_jpg(&p_decoder->mutex);
...
void jpegd_destroy(jpegd_obj_t *p_obj)
{
    if (p_obj)
    {
        ...
        if (p_decoder)
        {
            ...
            os_cond_signal_jpg(&(p_decoder->cond));
            os_mutex_unlock_jpg(&(p_decoder->mutex));
    ...
    jpegd_handle_input_req_thread(OS_THREAD_FUNC_ARG_T p_arg)
{
    ...
    for (;;)
    {
        ...
        {
            os_cond_wait_jpg(&(p_decoder->cond), &(p_decoder->mutex));
        }
